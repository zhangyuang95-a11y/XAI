...
from backend.training import warehouse_r41_diagnostic_outer_collection_v13 as subject
...
def test_collection_freezes_the_v13_public_fold_assignments_without_repairs():
    ...
    status = development["legacy_family_only_split_status"]
    assert status["status"] == "supported_after_combined_promotion"
    assert status["all_partitions_supported"] is True
    assert [len(repairs[salt]["repairs"]) for salt in subject.CV_SALTS] == [0, 0]
    assert {
...
def _selector_report(lock: dict) -> dict:
    ...
    retained_semantic_sha = digest({"retained": "combined-development"})

    def source_mask(label: str, exclusions: int) -> dict:
        value = {
            "source_rows": 100, "retained_rows": 96, "removed_rows": 4,
            "source_unique_observations": 90,
            "retained_unique_observations": 86,
            "fresh_outer_unique_observations": exclusions,
            "retained_fresh_outer_observation_overlap": 0,
            "keep_mask_sha256": digest({"keep": label}),
            "retained_observation_hashes_sha256": digest({"retained": label}),
            "private_development_members_read_before_mask_frozen": False,
            "retained_rows_semantic_sha256": digest({"semantic": label}),
    ...
    validation_wins = {
        ...
        "base": source_mask("base", 64),
        "combined_promoted": source_mask("promoted", 62),
        "prior_unique_observations": 40,
        "historical_unique_observations": 30,
        "combined_promoted_unique_observations": 10,
        "fresh_v13_unique_observations": 32,
        "combined_promoted_projection_sha256": digest(["promoted"]),
        "combined_source_rows": 200,
        "combined_retained_rows": 192,
        "combined_retained_scene_count": 48,
    ...
    validation_hash_inputs = {
        ...
        "promotion_closeout_sha256": lock["bindings"][
            "promotion_closeout_sha256"],
        "combined_promoted_rows_sha256": lock["bindings"][
            "combined_promoted_rows_sha256"],
        "combined_promoted_rows_semantic_sha256": digest({"rows": "semantic"}),
        "combined_promoted_unique_observations": 10,
        "combined_promoted_observation_hashes_sha256": digest(["promoted"]),
    ...
    def legacy_support(salt: str, fold: int) -> dict:
        return {
            "minimum_per_partition": minimum_support,
            "partitions": {
                "fit": support(116, 14, 14, f"{salt}-{fold}-fit"),
                "validation": support(3, 3, 3, f"{salt}-{fold}-validation"),
            },
            "passed": True,
    ...
    legacy_status = {
        ...
        "status": "supported_after_combined_promotion",
        "assignment_sha256s": dict(subject.OFFICIAL_FOLD_ASSIGNMENT_SHA256S),
        "fold_support": {
            salt: {str(fold): legacy_support(salt, fold)
                   for fold in range(subject.CV_FOLDS)}
            for salt in subject.CV_SALTS
        },
        "all_partitions_supported": True,
        "action_labels_or_probabilities_used": False,
    }
    legacy_status["content_sha256"] = digest(legacy_status)
    ...
    def assignment_audit(salt: str) -> dict:
        ...
        legacy_sha = subject.OFFICIAL_FOLD_ASSIGNMENT_SHA256S[salt]
        value = {
    ...
    value = {
        ...
        "development": {
            ...
            "legacy_family_only_split_status": legacy_status,
...
def test_collection_contract_and_reader_api_are_unscored():
    ...
    assert subject.SCHEMA_VERSION == (
        "warehouse_r41_diagnostic_rcpd_v13_outer_collection_v1")
...
def test_candidate_lock_is_validated_before_any_outer_replay(tmp_path, monkeypatch):
    ...
    kwargs = dict(
        ...
        expected_projection_receipt_sha256="4" * 64,
            ...
            promotion_closeout_path="promoted-closeout",
            expected_promotion_closeout_sha256="6" * 64,
            permanent_promotion_closeout_registry="permanent-promotion",
            combined_promoted_rows_path="promoted-rows",
        program_path="program", selector_report_path="selector", output=tmp_path / "out")
    with pytest.raises(ValueError, match="locked v13 candidate"):
...
def test_candidate_bindings_include_registry_report_and_projection_receipt(
    ...
    names = (
        ...
        "projection_receipt",
        "development_rows", "combined_promoted_rows", "program", "selector_report",
    )
    paths = {}
    for name in names:
        path = tmp_path / name
...
def test_source_closure_binds_projection_registry_and_has_no_protected_reader():
    ...
    assert "backend/training/warehouse_r41_diagnostic_outer_collection_v13.py" in sources
    assert "backend/training/warehouse_r41_diagnostic_outer_hash_projection_v13.py" in sources
    assert "backend/training/warehouse_r41_diagnostic_rcpd_v13_outer_split.py" in sources
