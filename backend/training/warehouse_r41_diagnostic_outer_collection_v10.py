"""Collect the full fresh v10 outer rows only after candidate lock.
...
VERSION = "warehouse-r41-diagnostic-outer-collection.v10"
SCHEMA_VERSION = "warehouse_r41_diagnostic_rcpd_v10_outer_collection_v1"
STATUS = "collected_unscored"
CANDIDATE_LOCK_VERSION = "warehouse_r41_diagnostic_rcpd_v10_candidate_lock_v1"
SELECTOR_VERSION = "warehouse-r41-diagnostic-rcpd-v10-fit-selector.v1"
...
def contract() -> dict[str, Any]:
    return {
        "version": VERSION,
        "population": "fresh v10 identity-frozen development outer only",
...
def _validate_static_rows(arrays: Mapping[str, np.ndarray]) -> None:
    if set(arrays) != rows_v7._FIELDS:
        raise ValueError("V10 outer row array schema differs")
    ...
    if (count <= 0
        ...
        raise ValueError("V10 outer row shapes or dtypes differ")
    ...
    if (not np.isfinite(observations).all()
        ...
        raise ValueError("V10 outer numeric or action-authority rows differ")
    ...
    if not np.array_equal(
        ...
        raise ValueError("V10 outer public-observation hashes differ")
    for name in ("observation_hashes", "scene_fingerprints",
                 "source_state_hashes"):
        try:
            values = np.char.decode(arrays[name], "ascii").astype(str)
        except UnicodeDecodeError as error:
            raise ValueError("V10 outer row text is not ASCII") from error
        if any(_HEX.fullmatch(str(value)) is None for value in values):
            raise ValueError("V10 outer row SHA-256 field differs")
...
def load_authenticated_rows(
    ...
    path = _regular(rows_path, "v10 outer rows", maximum=MAX_NPZ_BYTES)
    if file_hash(path) != _sha(expected_rows_sha256, "v10 outer rows"):
        raise ValueError("Exact v10 outer row bytes required")
    ...
    try:
        with zipfile.ZipFile(path) as archive:
            ...
            if (len(names) != len(set(names)) or set(names) != expected_members
                ...
                raise ValueError("V10 outer row archive contents differ")
        with np.load(path, allow_pickle=False) as archive:
            if set(archive.files) != rows_v7._FIELDS:
                raise ValueError("V10 outer row array schema differs")
            arrays = {name: archive[name].copy() for name in archive.files}
    except (OSError, ValueError, zipfile.BadZipFile, KeyError) as error:
        raise ValueError("V10 outer rows are not a safe strict NPZ") from error
    if file_hash(path) != expected_rows_sha256:
        raise RuntimeError("V10 outer rows changed during strict load")
...
def _validate_candidate_lock_shape(value: Mapping[str, Any]) -> dict[str, str]:
    ...
    if (set(value) != _LOCK_FIELDS
        ...
        raise ValueError("Exact locked v10 candidate contract required")
...
def _validate_validation_wins(
    ...
    if (set(value) != _VALIDATION_WINS_FIELDS or not _content_valid(value)
        ...
        raise ValueError("Locked v10 selector validation-wins trace differs")
...
def _validate_selector_report(
    ...
    if (set(report) != _SELECTOR_FIELDS
        ...
        raise ValueError("Locked v10 selector report identity or bindings differ")
    ...
    if (not isinstance(contract_value, Mapping)
            ...
            or contract_value.get(
                "consumed_v9_outer_labels_or_probabilities_read") is not False
            or contract_value.get(
                "fresh_v10_outer_labels_or_probabilities_read") is not False
            ...
            or boundary.get("runtime_action_override") is not False
            or boundary.get("formal_ready") is not False):
        raise ValueError("Locked v10 selector information boundary differs")
    if (type(development.get("retained_row_count")) is not int
            or development["retained_row_count"] <= 0):
        raise ValueError("Locked v10 selector development row count differs")
    ...
    if (not isinstance(grid, Mapping)
        ...
        raise ValueError("Locked v10 selector selection differs")
    ...
    if (not isinstance(candidate_reports, list) or not candidate_reports
        ...
        raise ValueError("Locked v10 selector candidate registry differs")
    ...
    for candidate, expected_config_sha in zip(candidate_reports, config_hashes):
        if (not isinstance(candidate, Mapping)
            ...
            raise ValueError("Locked v10 selector candidate report differs")
        ...
        for salt_report, expected_salt in zip(salts, CV_SALTS):
            ...
            if (not isinstance(salt_report, Mapping)
                ...
                raise ValueError("Locked v10 selector two-salt CV trace differs")
            ...
            for fold in folds:
                ...
                if (set(fold) != _FOLD_REPORT_FIELDS
                    ...
                    raise ValueError("Locked v10 selector fold trace differs")
                if (fold["fit_rows"] + fold["validation_rows"]
                    ...
                    raise ValueError("Locked v10 selector fold partition differs")
                ...
                _gate_matches(
                    ...
                    label="Locked v10 selector informational fold",
        ...
        if (candidate.get("both_salts_pass_all_aggregate_gates") is not both_pass
            ...
            raise ValueError("Locked v10 selector candidate summary differs")
    ...
    if candidate_grid_file_sha256 != bindings["candidate_grid_sha256"]:
        raise ValueError("Locked v10 selector candidate-grid binding differs")
    ...
    if not eligible:
        raise ValueError("Locked v10 selector has no eligible candidate")
    chosen = eligible[0]
    expected_selection = {
    ...
    if (dict(selection) != expected_selection
        ...
        raise ValueError("Locked v10 selector chosen candidate differs")

    final_fit = report.get("final_fit")
    if (not isinstance(final_fit, Mapping)
            or set(final_fit) != _FINAL_FIT_FIELDS):
        raise ValueError("Locked v10 selector full-development refit differs")
    _gate_matches(final_fit.get("development_metrics"),
                  final_fit.get("development_gate"),
                  label="Locked v10 selector full-development", require_passed=True)
...
def authenticate_locked_candidate_selector(
    *, lock: Mapping[str, Any], selector_report_path: str | Path,
    ...
    path = _regular(selector_report_path, "locked v10 selector report")
    expected = _sha(expected_selector_report_sha256, "locked selector report")
    raw = path.read_bytes()
    if sha256(raw).hexdigest() != expected:
        raise ValueError("Exact locked v10 selector report bytes required")
    report = _strict_json_bytes(raw, "locked v10 selector report")
    if file_hash(path) != expected:
        raise RuntimeError("Locked v10 selector report changed during authentication")
...
def _validate_candidate_bindings(
    *, lock: Mapping[str, Any], paths: Mapping[str, Path],
    ...
    if any(bindings[name] != value for name, value in actual.items()):
        raise ValueError("V10 candidate lock artifact binding differs")
    ...
    if closure is not None:
        ...
        if (not normalized
            ...
            raise ValueError("V10 candidate source closure binding differs")
    else:
        report_binding = selector_report.get("bindings", {}).get(
            "source_closure_sha256")
        if report_binding != bindings["source_closure_sha256"]:
            raise ValueError("V10 candidate source closure is not authenticated")
    return bindings
...
def _resolve_snapshot(
    *, lock: Mapping[str, Any], actor_path: str | Path,
    ...
    originals = {
        ...
        "candidate_lock": _regular(candidate_lock_path, "v10 candidate lock"),
        ...
        "program": _regular(program_path, "locked v10 program"),
    ...
    snapshot = ImmutableInputSnapshot(
        ...
        prefix="warehouse-r41-v10-outer-collection-inputs-",
    )
...
def build(
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    candidate_file = _regular(candidate_lock_path, "v10 candidate lock")
    if file_hash(candidate_file) != _sha(
            expected_candidate_lock_sha256, "v10 candidate lock"):
        raise ValueError("Exact v10 candidate lock bytes required")
    candidate_lock = _strict_json(candidate_file, "v10 candidate lock")
    ...
    try:
        ...
        lock_fd = os.open(
            ...
            | getattr(os, "O_NOFOLLOW", 0), 0o600)
        locked = _strict_json(paths["candidate_lock"], "snapshotted v10 candidate lock")
        ...
        arrays, accounting, environment_steps = projection_api._replay_outer(
            ...
            progress_label="fresh_v10_outer_full_collection")
        ...
        temporary = Path(tempfile.mkdtemp(
            prefix=".warehouse-r41-v10-outer-collection-", dir=destination.parent))
        ...
        if (set(reopened) != set(arrays)
            ...
            raise RuntimeError("Staged v10 outer row archive changed")
...
def authenticate_saved_collection(
    *, report_path: str | Path, rows_path: str | Path,
    ...
    report_file = _regular(report_path, "v10 outer collection report")
    rows_file = _regular(rows_path, "v10 outer rows", maximum=MAX_NPZ_BYTES)
    projection_file = _regular(
        projection_copy_path, "v10 copied outer hash projection")
    expected = {
        "report": _sha(expected_report_sha256, "v10 outer collection report"),
        "rows": _sha(expected_rows_sha256, "v10 outer rows"),
        "projection": _sha(
            expected_projection_copy_sha256, "v10 copied hash projection"),
    }
    if (file_hash(report_file) != expected["report"]
        ...
        raise ValueError("Exact v10 outer collection artifact set required")
    report = _strict_json(report_file, "v10 outer collection report")
    sources = producer_sources()
    ...
    if (report.get("version") != VERSION
            ...
            or boundary.get("runtime_action_override") is not False
            or report.get("formal_ready") is not False):
        raise ValueError("V10 outer collection receipt differs")
    if file_hash(report_file) != expected["report"]:
        raise RuntimeError("V10 outer collection report changed during authentication")
    return deepcopy(report)
