...
from backend.warehouse_r41_diagnostic_public_tree_program_v8 import (
    AGGREGATION as PROGRAM_AGGREGATION,
...
VERSION = "warehouse-r41-diagnostic-online-release.v8"
...
_IDENTITY_FIELDS = frozenset((
    ...
    "protocol_content_sha256", "program_sha256", "program_content_sha256",
    "program_identity_sha256", "public_feature_contract_sha256",
    "public_feature_registry_sha256", "program_complexity_sha256",
...
def _validate_manifest(manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (identities.get("actor_sha256") != FIXED_ACTOR_SHA256
            or identities.get("source_full_manifest_version")
                != FULL_SCENE_MANIFEST_VERSION
    ...
    weights = identities.get("program_mix_weights")
    if (identities.get("program_action_names")
                != ["UP", "DOWN", "LEFT", "RIGHT", "WAIT"]
            or identities.get("program_classes") != [0, 1, 2, 3, 4]
            or identities.get("program_routes") != expected_routes
            or not isinstance(weights, Mapping)
            or set(weights) != set(PROGRAM_GROUPS)
            or any(type(value) not in (int, float)
                   or not 0.0 <= float(value) <= 1.0 for value in weights.values())
            or identities.get("program_aggregation") != PROGRAM_AGGREGATION):
        raise ValueError("Diagnostic v8 program registry identity differs")
    ...
    _reject_sensitive(manifest, "diagnostic portable manifest")
...
def _archive_bytes(manifest: Mapping[str, Any],
    ...
    with zipfile.ZipFile(stream, "w", compression=ARCHIVE_COMPRESSION,
        ...
        for name, raw in ((MANIFEST_NAME, (canonical(manifest) + "\n").encode()),
            ...
            archive.writestr(
                info, raw, compress_type=ARCHIVE_COMPRESSION,
...
def _read_archive(raw: bytes, *, expected_package_sha256: str,
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(raw), "r") as archive:
            ...
            if (archive.comment != b"" or len(names) != len(set(names))
            ...
            for info in infos:
                ...
                if (pure.is_absolute() or ".." in pure.parts
                        ...
                        or info.extra != b"" or info.comment != b""
...
def _runtime(paths: Mapping[str, Path],
    ...
    _reject_sensitive(runtime.actor.metadata, "diagnostic archived actor metadata")
...
def load_online_release(*, expected_package_sha256: str,
    ...
    try:
        ...
        program_identity = {
            "version": program.to_dict()["version"],
            "component_version": BOOSTED_TREE_VERSION,
            "actions": list(program.action_names),
            "classes": list(program.base_program.classes),
            "relations": program.relations.contract(),
            "routes": list(program.routes),
            "mix_weights": program.mix_weights,
            "aggregation": program.to_dict()["aggregation"],
        ...
        if (explainer.program_content_sha256 != identities["program_content_sha256"]
                ...
                or program.mix_weights != identities["program_mix_weights"]
                or program.to_dict().get("aggregation")
                    != identities["program_aggregation"]
                or digest(program.complexity())
                    != identities["program_complexity_sha256"]
                or digest(program.relations.contract())
                    != identities["public_feature_contract_sha256"]
                or digest(list(program.feature_names))
                    != identities["public_feature_registry_sha256"]
                or digest(program_identity) != identities["program_identity_sha256"]):
            raise ValueError("Diagnostic archived v8 program identity differs")
...
def assemble_from_admitted_components(*,
    ...
    with tempfile.TemporaryDirectory(prefix="warehouse-r41-diagnostic-assemble-") as tmp:
        ...
        identities = {
            ...
            "public_feature_registry_sha256": bindings[
                "public_feature_registry_sha256"],
            "program_complexity_sha256": bindings["program_complexity_sha256"],
            "program_action_names": list(explainer.program.action_names),
            "program_classes": list(explainer.program.base_program.classes),
            "program_routes": list(explainer.program.routes),
            "program_mix_weights": explainer.program.mix_weights,
        ...
        assembled_program_identity = {
            "version": explainer.program.to_dict()["version"],
            "component_version": BOOSTED_TREE_VERSION,
            "actions": list(explainer.program.action_names),
            "classes": list(explainer.program.base_program.classes),
            "relations": explainer.program.relations.contract(),
            "routes": list(explainer.program.routes),
            "mix_weights": explainer.program.mix_weights,
            "aggregation": explainer.program.to_dict()["aggregation"],
            "metadata": explainer.program.metadata,
            "complexity": explainer.program.complexity(),
        }
        if (identities["parent_explainer_signature"]
                ...
                or digest(explainer.program.relations.contract())
                    != identities["public_feature_contract_sha256"]
                or digest(list(explainer.program.feature_names))
                    != identities["public_feature_registry_sha256"]
                or digest(explainer.program.complexity())
                    != identities["program_complexity_sha256"]
                or digest(assembled_program_identity)
                    != identities["program_identity_sha256"]):
    ...
    parent = {
        ...
        "development_expansion_registry_sha256": bindings[
            "development_expansion_registry_sha256"],
        "development_expansion_content_sha256": bindings[
            "development_expansion_content_sha256"],
        ...
        "fresh_final_holdout_report_sha256": bindings[
            "fresh_final_holdout_report_sha256"],
        "fresh_final_v3_exclusion_sha256": bindings[
            "fresh_final_v3_exclusion_sha256"],
        "fresh_final_v3_exclusion_content_sha256": bindings[
            "fresh_final_v3_exclusion_content_sha256"],
        "final_once_identity_sha256": bindings["final_once_identity_sha256"],
        "final_once_campaign_key": bindings["final_once_campaign_key"],
        "final_once_permanent_anchor_sha256": bindings[
            "final_once_permanent_anchor_sha256"],
        "final_once_attempt_started_sha256": bindings[
            "final_once_attempt_started_sha256"],
        "final_once_candidate_authenticated_sha256": bindings[
            "final_once_candidate_authenticated_sha256"],
        ...
        "final_once_audit_completed_sha256": bindings[
            "final_once_audit_completed_sha256"],
        "final_once_attempt_completed_sha256": bindings[
        ...
        "public_feature_registry_sha256": bindings[
            "public_feature_registry_sha256"],
        "program_complexity_sha256": bindings["program_complexity_sha256"],
        "explanation_audit_inputs_sha256": bindings[
            "explanation_audit_inputs_sha256"],
        "explanation_audit_evidence_sha256": bindings[
