"""Select and lock one public-tree candidate for a replacement v12 fresh outer.

The selector treats the failed v8 outer and the failed v11 outer as
development data only after both permanent closeouts are authenticated.  The
replacement v12 outer contributes a label-blind ordered observation-hash
...
share a public observation, the newly promoted v11 row wins deterministically
...
from backend.training import warehouse_r41_diagnostic_rcpd_v12_outer_split as registry_api
...
VERSION = "warehouse-r41-diagnostic-rcpd-v12-fit-selector.v1"
STATUS = "locked_development_candidate_pending_fresh_outer"
LOCK_SCHEMA = "warehouse_r41_diagnostic_rcpd_v12_candidate_lock_v1"
...
OFFICIAL_FOLD_ASSIGNMENT_SHA256S = {
    "warehouse-r41-v9-blocked-cv-a-20260913": (
        "e7269e857d1a3c45cb13ffb361901b65ac9d60e94f8f979276144f03048eaa03"),
    "warehouse-r41-v9-blocked-cv-b-20260913": (
        "cbeaed633552b9c6d7f801b8f94f7ccef45700f9a0b47d58c59225837004798f"),
}
...
_PRIOR_PROJECTION_FIELDS = frozenset((
    ...
    "source_v11_projection_content_sha256", "outer_observation_hashes",
...
class NoEligibleCandidateError(RuntimeError):
    ...
    def __init__(self, candidate_reports: Sequence[Mapping[str, Any]]) -> None:
        ...
        super().__init__(
            "No v12 candidate passed both salted aggregate gate suites; "
...
def contract() -> dict[str, Any]:
    return {
        ...
        "development_sources": [
            ...
            "permanently closed failed-v11 outer rows",
        ],
        "candidate_grid_sha256": FROZEN_CANDIDATE_GRID_SHA256,
        "consumed_v9_outer_input": (
            "identity exclusion is bound by the v12 registry; labels and "
        ...
        "prior_outer_input": (
            "registry-bound union of consumed v8, v9, v10, and v11 one-way "
            "observation hashes only"
        ),
        "prior_outer_projection_bindings": [
            "file_sha256", "content_sha256",
        ],
        "fresh_outer_input": "ordered v12 label-blind observation SHA-256 projection only",
        ...
        "fresh_v12_outer_labels_or_probabilities_read": False,
...
def read_prior_outer_hash_projection(
    ...
    """Authenticate the registry-bound v8/v9/v10/v11 hash-only projection."""
...
def prepare_combined_development_rows(
    ...
    promoted_v11_path: str | Path, expected_promoted_v11_sha256: str,
    prior_outer_observation_hashes: Sequence[str],
    promoted_v11_observation_hashes: Sequence[str],
    fresh_outer_observation_hashes: Sequence[str],
    actor: NumPyNativeActor,
    early_member_audit: list[str] | None = None,
) -> tuple[dict[str, np.ndarray], dict[str, Any]]:
    """Promote v11 rows while freezing every keep mask before target access.

    ``prior_outer_observation_hashes`` is the registry-bound union through the
    consumed v11 outer.  Older development removes that full union.  The promoted
    v11 component removes only older historical hashes plus the new v12 outer,
    ...
    promoted = _regular(
        promoted_v11_path, "promoted failed-v11 outer rows",
    ...
    if not promoted_set or not promoted_set <= prior_set:
        raise ValueError("Promoted v11 hashes must be one bound prior component")
    ...
    if promoted_public_hashes != promoted_set:
        raise ValueError("Promoted v11 rows and hash projection differ")
    historical = sorted(prior_set - promoted_set)
    if promoted_set & set(historical):
        raise RuntimeError("Historical and promoted v11 projections overlap")
    ...
    if (file_hash(base) != expected_base_sha256
            or file_hash(promoted) != expected_promoted_v11_sha256):
    ...
    rows_api._validate_base_shapes(
        promoted_full, actor, "promoted failed-v11 development")
    if not bool(np.all(promoted_full["split_validation"])):
        raise ValueError("Promoted v11 source must be the consumed outer split")
    base_retained = {
    ...
    rows_api._validate_base_shapes(combined, actor, "combined v12 development")
    audit: dict[str, Any] = {
    ...
    while True:
        ...
        if not candidates:
            raise fit_api.ReplacementSupportError(
                "V12 public same-family swap cannot satisfy frozen support")
...
def _replacement_support_audit(
    ...
    if (route.shape != selected.shape or scenes.shape != route.shape
        ...
        raise ValueError("V12 replacement support registry differs")
    ...
    for salt in salts:
        ...
        for fold in range(FOLD_COUNT):
            ...
            if (predicted.shape != (int(np.sum(validation)), len(fit_api.ACTIONS))
                ...
                raise ValueError("V12 fold prediction probabilities differ")
    ...
    value: dict[str, Any] = {
        ...
        "selection": {
            "selected_config_sha256": _sha(
                selected_config_sha256, "selected v12 fit configuration"),
...
def _resolve_snapshot(
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    promoted_v11_rows_path: str | Path, candidate_grid_path: str | Path,
    ...
    promoted_closeout_original = _regular(
        promoted_v11_closeout_path, "promoted v11 outer closeout")
    promoted_permanent = _directory(
        permanent_v11_outer_registry, "permanent v11 outer registry")
    promoted_closeout = promoted_closeout_api.read_saved_closeout(
        ...
        permanent_attempt_registry=promoted_permanent)
    promoted_rows_sha = _sha(
        promoted_closeout.get("consumed_outer", {}).get("rows_sha256"),
        "promoted failed-v11 outer rows")
    originals = {
    ...
    expected = {
        ...
        "promoted_v11_rows": promoted_rows_sha,
    ...
    snapshot = ImmutableInputSnapshot(
        ...
        prefix="warehouse-r41-v12-fit-selector-inputs-",
    )
...
def build(
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    try:
        ...
        promoted_rows_sha = _sha(
            promoted_outer["rows_sha256"], "promoted v11 rows")
        promoted_rows_semantic_sha = _sha(
            promoted_outer["rows_semantic_sha256"],
            "promoted v11 rows semantic digest")
        ...
        if (prior_projection_file_sha256
            ...
            raise ValueError(
                "Prior outer projection does not match the v12 registry")
        ...
        if (designation.get("bindings", {}).get("actor_sha256") != actor.artifact_sha256
                ...
                or registry_bindings.get(
                    ...
                    != file_hash(paths["promoted_v11_closeout"])
                or registry_bindings.get(
                ...
                or registry_bindings.get(
                    ...
                    != promoted_outer["identities_sha256"]
            ...
            raise ValueError("V12 selector evidence bindings differ")
        ...
        validation_hash_inputs = {
            ...
            "promoted_v11_rows_sha256": promoted_rows_sha,
            "promoted_v11_rows_semantic_sha256": promoted_rows_semantic_sha,
            "promoted_v11_unique_observations": len(promoted_hashes),
        ...
        arrays, overlap_audit = prepare_combined_development_rows(
            ...
            promoted_v11_path=paths["promoted_v11_rows"],
            expected_promoted_v11_sha256=promoted_rows_sha,
            prior_outer_observation_hashes=prior_hashes,
            promoted_v11_observation_hashes=promoted_hashes,
        ...
        if relations.base_feature_names != actor_feature_names:
            raise ValueError("V12 relation base features differ from the frozen Actor")
        ...
        for salt in CV_SALTS:
            ...
            if (assignment_audit["repaired_assignment_sha256"]
                ...
                raise RuntimeError("Official v12 fold-assignment regression differs")
        ...
        if (program.base_feature_names != actor_feature_names
            ...
            raise RuntimeError("Locked v12 program relation/Actor binding differs")
        ...
        if (not np.array_equal(
            ...
            raise RuntimeError("Locked v12 program round-trip action parity differs")
        if final_gate.get("passed") is not True:
            raise RuntimeError("Full-development v12 refit failed the nine hard gates")

        temporary = Path(tempfile.mkdtemp(
            prefix=".warehouse-r41-v12-fit-selector-", dir=destination.parent))
        ...
        report: dict[str, Any] = {
            ...
            "development": {
                ...
                "failed_v11_outer_permanently_closed": True,
                "failed_v11_outer_reclassified_as_development": True,
