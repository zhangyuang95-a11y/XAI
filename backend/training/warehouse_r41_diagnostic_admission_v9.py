"""Fail-closed admission for the warehouse r4.1 diagnostic v9 release.

Only the previously disclosed behaviour-performance gate is waived.  The
locked v9 program must pass its irrevocable development outer and protected
final audit before this module can admit any deployment derivative.
"""
...
def source_closure() -> dict[str, str]:
    """Bind the complete transitive v9 admission implementation."""
...
def _validate_components_snapshot(
    ...
    json_values = {
        ...
        if name not in {"actor", "compact_program"}
    }
    ...
    if (completion.get("status") != final_api.STATUS_PASSED
        ...
        raise ValueError("Passing immutable v9 protected-final completion required")
    ...
    if (current_hashes != initial_hashes or source_closure() != sources
        ...
        raise RuntimeError("V9 admission input or source changed during validation")
...
def validate_components(
    ...
    with snapshot_api.ImmutableInputSnapshot(
            ...
            prefix="warehouse-r41-diagnostic-v9-admission-") as frozen:
...
def read_saved_admission(
    ...
    _reject_sensitive(value, "saved v9 admission")
    return deepcopy(value)
