"""Irrevocably score one locked v12 candidate program on one fresh development outer.
...
VERSION = "warehouse-r41-diagnostic-rcpd-v12-outer-once.v1"
LOCK_SCHEMA = "warehouse_r41_diagnostic_rcpd_v12_candidate_lock_v1"
COLLECTION_SCHEMA = "warehouse_r41_diagnostic_rcpd_v12_outer_collection_v1"
LOCK_STATUS = "locked"
...
def _candidate_lock(
    ...
    lock_path, _, value = _strict_json(
        path, "v12 candidate lock", expected_sha256=expected_sha256)
    ...
    if (value.get("schema_version") != LOCK_SCHEMA
        ...
        raise ValueError("V12 candidate lock semantics differ")
    try:
        strict_bindings = collection_api._validate_candidate_lock_shape(value)
    except ValueError as error:
        raise ValueError("V12 candidate lock semantics differ") from error
...
def _verify_lock_files(bindings: Mapping[str, str], **paths: str | Path) -> dict[str, Path]:
    expected_keys = {
        ...
        "promoted_v11_closeout": "promoted_v11_closeout_sha256",
        ...
        "promoted_v11_rows": "promoted_v11_rows_sha256",
        "program": "program_sha256", "selector_report": "selector_report_sha256",
    }
    if set(paths) != set(expected_keys):
        raise ValueError("Complete v12 candidate-lock file set required")
    result = {}
    for name, binding in expected_keys.items():
        maximum = MAX_NPZ_BYTES if name in {
            "actor", "development_rows", "promoted_v11_rows"} else MAX_JSON_BYTES
...
def _validate_candidate_source_closure(
    ...
    if closure is None:
        raise ValueError(
            "V12 candidate source closure mapping is required for runtime authentication")
    if (not closure
    ...
    if missing or changed:
        ...
        raise ValueError(
            "V12 candidate runtime source closure differs: " + "; ".join(details))
...
def _registry_bundle(
    ...
    *, bindings: Mapping[str, str],
    ...
    value = _strict_json_bytes(
        registry_path.read_bytes(), "fresh v12 outer registry")
    report = _strict_json_bytes(
        registry_report_path.read_bytes(), "fresh v12 outer registry report")
    ...
    if (set(value) != _REGISTRY_FIELDS
        ...
        raise ValueError("Fresh v12 outer registry semantics differ")
    public_identities = []
    ...
    for index, (scene, identity) in enumerate(zip(scenes, identities)):
        if (not isinstance(scene, Mapping) or not isinstance(identity, Mapping)
            ...
            raise ValueError("Fresh v12 outer registry identities differ")
    ...
    if (set(report) != _REGISTRY_REPORT_FIELDS
        ...
        raise ValueError("Fresh v12 outer registry/report identity differs")
    if (file_hash(registry_path) != bindings["fresh_outer_registry_sha256"]
        ...
        raise RuntimeError("Fresh v12 outer registry bundle changed during read")
...
def _preflight_outer_rows(
    *, rows_path: Path, rows_sha256: str, projection: Mapping[str, Any],
    ...
    promoted_v11_rows_path: Path, promoted_v11_rows_sha256: str,
    promoted_v11_closeout_sha256: str,
    promoted_v11_rows_semantic_sha256: str,
    ...
    promoted = _safe_row_projection(
        promoted_v11_rows_path, expected_sha256=promoted_v11_rows_sha256,
        fields=frozenset(("observation_hashes",)),
        label="promoted failed-v11 outer rows")
    ...
    bound_promoted = list(promoted_v11_observation_hashes)
    ...
    expected_hash_inputs = {
        ...
        "promoted_v11_closeout_sha256": promoted_v11_closeout_sha256,
        "promoted_v11_rows_sha256": promoted_v11_rows_sha256,
        "promoted_v11_rows_semantic_sha256": (
            promoted_v11_rows_semantic_sha256),
        "promoted_v11_unique_observations": len(bound_promoted),
        "promoted_v11_observation_hashes_sha256": digest(bound_promoted),
    ...
    expected_top = {
        ...
        "promoted_v11_unique_observations": len(bound_promoted),
        "fresh_v12_unique_observations": len(fresh_unique),
        "promoted_v11_projection_sha256": digest(bound_promoted),
    ...
    if (not isinstance(validation_wins, Mapping)
            ...
            or not isinstance(validation_wins.get("promoted_v11"), Mapping)
            or any(validation_wins["base"].get(name) != value
                   for name, value in base_audit.items())
            or any(validation_wins["promoted_v11"].get(name) != value
    ...
    return {
        ...
        "promoted_v11_source_row_count": len(promoted_ordered),
        "promoted_v11_retained_row_count": len(promoted_retained),
        ...
        "promoted_v11_validation_wins_keep_mask_sha256": (
...
def build(
    *, candidate_lock_path: str | Path, expected_candidate_lock_sha256: str,
    ...
    promoted_v11_rows_path: str | Path, program_path: str | Path,
    ...
    if promoted_outer["rows_sha256"] != bindings["promoted_v11_rows_sha256"]:
    ...
    preflight = _preflight_outer_rows(
        ...
        development_rows_sha256=bindings["development_rows_sha256"],
        promoted_v11_rows_path=paths["promoted_v11_rows"],
        promoted_v11_rows_sha256=bindings["promoted_v11_rows_sha256"],
        promoted_v11_closeout_sha256=bindings[
            "promoted_v11_closeout_sha256"],
        promoted_v11_rows_semantic_sha256=promoted_outer[
            "rows_semantic_sha256"],
        promoted_v11_observation_hashes=promoted_outer[
    ...
    try:
        ...
        if (producer_sources() != sources
                ...
                or file_hash(paths["promoted_v11_closeout"])
                    != bindings["promoted_v11_closeout_sha256"]
