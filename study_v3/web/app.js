...
function task2Introduction(){if(release?.share_preview)return lang==='zh'?'体验版：第一个解释节点会引导你点击 AI 头上的“？”完成提问，回答显示在右侧。后续可以在右侧选择“我理解”或“还不理解”，也可直接继续操作。你是蓝色角色，AI 是橙色角色。':'Demo: at the first explanation point, click the “?” above the AI to ask a question. Answers and later understanding checks appear on the right; you may also keep playing without responding. You are blue; the AI is orange.';if(['guided-question-choice-v3','first-node-question-guide-v4','sidebar-early-question-guide-v5'].includes(view.automatic_explanation_protocol))return lang==='zh'?'Task 2 的第一个解释节点会引导你点击 AI 头上的“？”提问，回答显示在右侧。后续关键节点可选择直接继续或查看解释；你也可以在提问框追问。提问和阅读不消耗游戏步数。':'At the first explanation point in Task 2, click the “?” above the AI to ask a question. The answer appears on the right. At later decision points, choose to continue or see an explanation. You can also ask follow-up questions. Asking and reading do not use game turns.';return view.automatic_explanations_enabled?(lang==='zh'?'Task 2 中，AI 会在预设节点自动提供解释。你也可以主动追问；提问不消耗游戏回合。':'In Task 2, your AI teammate explains at preset points. You can also ask follow-up questions without using game turns.'):tr('task2Intro');}
function understandingNotice(){if(view?.optional_prompts&&view?.understanding_rating_enabled)return `<p class="hint">${lang==='zh'?'Task 2 每完成约 20% 的进度会在右侧显示 1–5 分理解度问题；可以回答，也可以直接继续游戏。':'Every 20% of Task 2, a 1–5 understanding question appears on the right. You may answer or keep playing.'}</p>`;return view?.understanding_rating_enabled?`<p class="hint">${lang==='zh'?'Task 2 每完成约 20% 的回合进度时，将暂停并请你用 1–5 分评价对 AI 队友的理解；任务结束时也会评分。评分不会消耗步数。':'Every 20% of the Task 2 turn budget, and at the end, the game pauses for a 1–5 rating of how well you understand your AI teammate. Rating does not use game turns.'}</p>`:'';}
...
function promptsBlockPlay(){return !view?.optional_prompts&&!!(pendingUnderstanding()||pendingAutomatic());}
function ratingBlocksPlay(){return !view?.optional_prompts&&!!pendingUnderstanding();}
function optionalPromptNote(){return view?.optional_prompts?`<p class="hint">${lang==='zh'?'可选择回答，也可以直接继续操作。未回答会记为跳过。':'You may respond or keep playing. Unanswered prompts are recorded as skipped.'}</p><button type="button" class="button secondary full" data-skip-prompts>${lang==='zh'?'暂不回答，继续':'Continue without answering'}</button>`:'';}
...
function syncUnderstandingDialog(){
 ...
 if(!view.optional_prompts)stopHeld();if(!view.optional_prompts&&!document.hidden&&focusedUnderstanding!==rating.id){focusedUnderstanding=rating.id;$('understandingTitle').tabIndex=-1;$('understandingTitle').focus({preventScroll:true});}
...
function syncAutomaticDialog(){
 const guide=questionGuide();if(guide&&!document.hidden){
  if(!view.optional_prompts)stopHeld();if(!view.optional_prompts&&focusedQuestionGuide!==guide.id){focusedQuestionGuide=guide.id;($('aiQuestionButton')||$('guidedQuestionExample'))?.focus({preventScroll:true});}
