"""Durable complete current-Actor branch collection through its genuine stable runtime.
...
from . import warehouse_family_stable_runtime_samples as samples
...
from backend import warehouse_stable_runtime as runtime_api
from .warehouse_family_branch_runtime_collection import context_matrix
from backend.warehouse_family_explanation import actor_parameter_sha256

VERSION = 'warehouse-family-current-stable-teacher-collection.v1'
...
def _context(actor, protocol, actor_sha, protocol_sha, scenarios):
    ...
    runtime=runtime_api.StableRuntime(paths['actor'],protocol=declared,expected_actor_sha256=actor_sha,
...
def _plan(runtime, scenario_manifest, inputs):
    ...
    sources={**runtime.sources,str(Path(__file__).relative_to(ROOT)):file_hash(__file__),
        ...
        "backend/training/warehouse_family_branch_runtime_collection.py":file_hash(ROOT/"backend/training/warehouse_family_branch_runtime_collection.py")}
...
def read_data(output):
    ...
    if (plan.get('version')!=VERSION or plan.get('producer')!=VERSION
        ...
        raise ValueError('Only the genuine current stable-runtime collection is supported')
    for relative,expected in plan['source_files'].items():
        if file_hash(ROOT/relative)!=expected:raise ValueError('Stable collection source changed')
