"""Expanded-development RCPD for the frozen r4.1 diagnostic Actor.

The fit uses all 192 previously registered development scenes and dense
counterfactual endpoints at every public critical frame.  A separately frozen
64-scene development supplement is excluded from fitting and uses the frozen
every-fifth-frame intervention schedule.  A seven-member ensemble of explicit
...
This is development evidence and never reads an explanation-final trajectory
or label.  Candidate admission requires every one of four deterministic whole
scene folds to meet the direction gate, which prevents an aggregate pass from
hiding a weak scene family partition.
"""
...
from backend import warehouse_r41_diagnostic_model_tree_ensemble_v2 as ensemble_api
from backend.warehouse_r41_diagnostic_model_tree import (
    R41DiagnosticModelTreeProgram,
    VERSION as MEMBER_VERSION,
)
from backend.warehouse_r41_diagnostic_model_tree_ensemble_v2 import (
    ENVELOPE_VERSION,
...
VERSION = "warehouse-r41-diagnostic-rcpd.v7"
...
def contract() -> dict[str, Any]:
    return {
        ...
        "source_splits": {
            "fit": ["registered train", "registered conflict_validation"],
            "development_validation": "frozen development supplement",
        },
        "holdout_split": "final_test",
        "partners": list(PARTNERS),
        "evaluated_role": "robot_2",
        "partition": (
            "expanded development scene split; validation wins on any exact float32 "
            "public-observation overlap and the duplicate training row is excluded"
        ),
        "intervention_schedule": {
            "train": "every critical pre-action frame",
            "development_supplement": "critical pre-action frame divisible by five",
            "actions": list(ACTIONS),
        },
        "program_family": (
            "mean-probability ensemble of seven explicit axis-routing trees "
        ...
        "candidate_selection_disclosure": (
            "all seven-member subsets of eleven pre-registered router seeds are "
            "compared on the fresh development supplement only; scenes are assigned "
            "to four folds by a fixed salted SHA-256 fingerprint; candidates must pass "
            "all aggregate gates and every fold's overall and per-group direction gate; "
            "selection then maximizes the worst fold and group "
            "intervention-direction fidelity, then aggregate minimum "
            "group direction, overall direction, and ordinary fidelity; final-test "
            "states and labels are not accessed"
        ),
        ...
        "fit_access": {
            "fit_rows": "all 192 registered development scenes",
        ...
        "weights": {
            ...
            "critical_group_balance": "base registered weighting",
            "scene_balance": "selection-time whole-scene folds",
        ...
        "development_scene_fold_count": SCENE_FOLD_COUNT,
        "deterministic_fit_workers": FIT_WORKERS,
        "source_public_row_archive_version": source_api.VERSION,
        "development_supplement_version": supplement_api.VERSION,
...
def producer_sources() -> dict[str, str]:
    # The extractor uses runtime/environment code, never the HTTP server.
    # Legacy training imports reach the server for unrelated CLI wiring; keep
    # that deployment-only boundary from invalidating scientific evidence.
    return local_source_hashes((
        Path(__file__), Path(source_api.__file__), Path(supplement_api.__file__),
        Path(member_api.__file__),
        Path(ensemble_api.__file__),
    ), exclude_paths=(
...
def _write(path: Path, raw: bytes) -> None:
    ...
    if path.exists() or path.is_symlink() or path.parent.is_symlink():
        raise ValueError("Diagnostic v7 output path is unsafe")
...
def _write_npz(path: Path, arrays: Mapping[str, np.ndarray]) -> None:
    if path.exists():
        raise ValueError("Diagnostic v7 row archive already exists")
...
def _load_npz(path: Path) -> dict[str, np.ndarray]:
    if (not path.is_file() or path.is_symlink() or path.resolve() != path.absolute()
        ...
        raise ValueError("Diagnostic v7 row archive is unsafe or oversized")
    with zipfile.ZipFile(path) as archive:
        ...
        if ({item.filename for item in infos} != expected or len(infos) != len(expected)
            ...
            raise ValueError("Diagnostic v7 row archive contents differ")
    with np.load(path, allow_pickle=False) as archive:
        if set(archive.files) != _FIELDS:
            raise ValueError("Diagnostic v7 row archive schema differs")
...
def _load_supplement(path: Path, *, actor_path: Path,
    ...
    if (bindings.get("actor_sha256") != file_hash(actor_path)
            ...
            or bindings.get("contract_sha256") != digest(supplement_api.contract())
        ...
        for partner_index, partner in enumerate(PARTNERS):
            ...
            while not env.done:
                ...
                if anchor:
                    ...
                    for player_action in ACTIONS:
                        ...
                        if transition["submitted_actions"]["robot_2"] != \
                            ...
                            raise RuntimeError("Diagnostic v7 branch overrode the Actor")
                        if branch.done:
                ...
                if digest(env.snapshot()) != source_sha:
                    raise RuntimeError("Diagnostic v7 collection changed source state")
                ...
                if (transition["submitted_actions"]["robot_2"] != actions["robot_2"]
                        or transition["submitted_actions"]["robot_2"]
                           != transition["policy_actions"]["robot_2"]):
                    raise RuntimeError("Diagnostic v7 trajectory overrode the Actor")
...
def _rows_to_arrays(train_rows: Sequence[Mapping[str, Any]],
    ...
    arrays["weights"] = _expected_weights(arrays).astype(np.float32)
    accounting = {
...
def _expected_weights(arrays: Mapping[str, np.ndarray]) -> np.ndarray:
    ...
    if not np.isfinite(result).all() or np.any(result <= 0):
        raise ValueError("Diagnostic v7 robust sample weights are invalid")
    return result
...
def _validate_arrays(arrays: Mapping[str, np.ndarray], *, actor: NumPyNativeActor,
    ...
    if set(arrays) != _FIELDS:
        raise ValueError("Diagnostic v7 evidence array set differs")
    ...
    if (count <= 0 or arrays["observations"].shape != (count, actor.obs_dim)
        ...
        raise ValueError("Diagnostic v7 evidence shapes or dtypes differ")
    ...
    if (not np.isfinite(observations).all() or not np.isfinite(probabilities).all()
        ...
        raise ValueError("Diagnostic v7 numeric or action-authority evidence differs")
    ...
    if not np.array_equal(expected_hashes, arrays["observation_hashes"]):
        raise ValueError("Diagnostic v7 public-observation hashes differ")
    ...
    if (not np.allclose(probabilities, actor_probabilities, rtol=8e-6, atol=4e-6)
        ...
        raise ValueError("Diagnostic v7 rows differ from the frozen Actor")
    split = arrays["split_validation"]
    ...
    if (set(map(str, scenes[~split])) != train_fingerprints
        ...
        raise ValueError("Diagnostic v7 registered scene partition differs")
    ...
    if set(map(str, episodes)) != expected_episodes:
        raise ValueError("Diagnostic v7 episode matrix differs")
    ...
    if train_hashes & validation_hashes:
        raise ValueError("Diagnostic v7 exact observation crossed the scene split")
    kinds = _decode(arrays["kinds"])
    anchors = _decode(arrays["anchor_ids"])
    branches = _decode(arrays["branch_actions"])
    physical = _decode(arrays["physical_hashes"])
    ...
    if (not np.all(ordinary | intervention)
        ...
        raise ValueError("Diagnostic v7 row-kind schema differs")
    ...
    if not np.array_equal(anchors[ordinary], expected_anchor):
        raise ValueError("Diagnostic v7 intervention schedule differs")
    ...
    for episode in expected_validation_episodes:
        ...
        if (not len(ordered)
            ...
            raise ValueError("Diagnostic v7 trajectory is incomplete")
    for episode in expected_train_episodes:
        ...
        if (len(indices) and (len(np.unique(frames[indices])) != len(indices)
            ...
            raise ValueError("Diagnostic v7 filtered training trajectory differs")
    ...
    if any(len(values) != 1 for values in anchor_splits.values()):
        raise ValueError("Diagnostic v7 intervention anchor crossed the split")
    if not np.array_equal(arrays["weights"], _expected_weights(arrays)):
        raise ValueError("Diagnostic v7 train-only weights differ")
...
def _fit_logistic(x: np.ndarray, y: np.ndarray, weights: np.ndarray, *, c: float):
    ...
    if (any(issubclass(item.category, ConvergenceWarning) for item in caught)
        ...
        raise RuntimeError("Diagnostic v7 leaf classifier did not converge")
...
def _fit_member(arrays: Mapping[str, np.ndarray], *, actor: NumPyNativeActor,
    ...
    if (router.get_depth() > ROUTER_DEPTH or router.get_n_leaves() > ROUTER_LEAVES
        ...
        raise RuntimeError("Diagnostic v7 router bounds differ")
    models = []
...
def _metrics_from_probabilities(probabilities: np.ndarray,
    ...
    if (probabilities.shape != (len(arrays["observations"]), len(ACTIONS))
        ...
        raise ValueError("Diagnostic v7 candidate probabilities differ")
...
def _scene_folds(arrays: Mapping[str, np.ndarray]) -> np.ndarray:
    """Assign a whole scene to one deterministic development fold."""
    scenes = _decode(arrays["scene_fingerprints"])
    values = {
        str(scene): int(sha256(
            ("warehouse-r41-v7-scene-fold:" + str(scene)).encode("ascii")
        ).hexdigest()[:8], 16) % SCENE_FOLD_COUNT
        for scene in np.unique(scenes)
    }
    return np.asarray([values[str(scene)] for scene in scenes], dtype=np.uint8)


def _fold_robustness(probabilities: np.ndarray,
                     arrays: Mapping[str, np.ndarray], *,
                     folds: np.ndarray | None = None) -> dict[str, Any]:
    validation = arrays["split_validation"]
    if folds is None:
        folds = _scene_folds(arrays)
    if folds.shape != validation.shape:
        raise ValueError("Diagnostic v7 scene-fold vector differs")
    rows = []
    for fold in range(SCENE_FOLD_COUNT):
        metrics = _metrics_from_probabilities(
            probabilities, arrays, validation & (folds == fold))
        direction = metrics["effective_intervention_direction"]
        if (direction["pairs"] <= 0
                or any(direction["by_group"][group]["pairs"] <= 0
                       for group in GROUPS)):
            raise ValueError("Diagnostic v7 scene fold lacks intervention evidence")
        rows.append({
            "fold": fold,
            "scene_count": metrics["overall"]["scenes"],
            "overall_fidelity": metrics["overall"]["fidelity"],
            "nonwait_fidelity": metrics["nonwait"]["fidelity"],
            "direction_fidelity": direction["fidelity"],
            "direction_by_group": {
                group: deepcopy(direction["by_group"][group])
                for group in GROUPS
            },
        })
    return {
        "assignment": (
            "sha256('warehouse-r41-v7-scene-fold:' + scene_fingerprint) modulo four"
        ),
        "folds": rows,
        "minimum_group_direction_fidelity": min(
            row["direction_by_group"][group]["fidelity"]
            for row in rows for group in GROUPS),
        "minimum_overall_direction_fidelity": min(
            row["direction_fidelity"] for row in rows),
        "minimum_ordinary_fidelity": min(
            min(row["overall_fidelity"], row["nonwait_fidelity"])
            for row in rows),
    }


def _candidate_gate(metrics: Mapping[str, Any],
                    robustness: Mapping[str, Any]) -> dict[str, Any]:
    """Require aggregate gates and every whole-scene fold to clear direction."""
    aggregate = _gate(metrics)
    checks = dict(aggregate["checks"])
    for row in robustness["folds"]:
        fold = int(row["fold"])
        checks[f"fold_{fold}_direction"] = (
            row["scene_count"] >= 10
            and row["direction_fidelity"] >= MIN_DIRECTION)
        for group in GROUPS:
            group_row = row["direction_by_group"][group]
            checks[f"fold_{fold}_direction_{group}"] = (
                group_row["scenes"] >= 10
                and group_row["fidelity"] >= MIN_DIRECTION)
    return {"checks": checks, "passed": all(checks.values())}
...
def _fit_candidate(actor: NumPyNativeActor, arrays: Mapping[str, np.ndarray], *,
    ...
    base_metadata = {
        ...
        "program_family": "seven_axis_router_sparse_public_linear_softmax_ensemble",
        "member_seed_candidates": list(MEMBER_SEED_CANDIDATES),
    ...
    fitted_members = Parallel(n_jobs=FIT_WORKERS, backend="loky")(
        delayed(_fit_member)(
            arrays, actor=actor, seed=seed, metadata=base_metadata)
        for seed in MEMBER_SEED_CANDIDATES
    )
    for seed, (member, diagnostics) in zip(
            MEMBER_SEED_CANDIDATES, fitted_members):
    ...
    compared = []
    validation = arrays["split_validation"]
    scene_folds = _scene_folds(arrays)
    for seeds in combinations(MEMBER_SEED_CANDIDATES, MEMBER_COUNT):
        ...
        robustness = _fold_robustness(probabilities, arrays, folds=scene_folds)
        direction = metrics["effective_intervention_direction"]
        ordinary_minimum = min(
            metrics["overall"]["fidelity"], metrics["nonwait"]["fidelity"],
            *(metrics["critical"][group]["fidelity"] for group in GROUPS))
        score = [
            robustness["minimum_group_direction_fidelity"],
            robustness["minimum_overall_direction_fidelity"],
        ...
        compared.append({
            ...
            "score": score,
            "fold_robustness": robustness,
            "gate": _candidate_gate(metrics, robustness),
            "metrics": metrics,
        })
    compared.sort(key=lambda row: (
        not row["gate"]["passed"],
        *(-value for value in row["score"]),
    ...
    if not np.array_equal(predictions, restored.predict_batch(arrays["observations"])):
        raise RuntimeError("Diagnostic v7 compact serialization changed an action")
    metrics = _metrics(program, arrays, validation)
    robustness = _fold_robustness(
        program.predict_proba_batch(arrays["observations"]), arrays,
        folds=scene_folds)
    gate = _candidate_gate(metrics, robustness)
    if (metrics != selected["metrics"] or robustness != selected["fold_robustness"]
            or gate != selected["gate"]):
        raise RuntimeError("Diagnostic v7 selected candidate replay differs")
    return program, {
        "profile": {"member_seeds": list(selected_seeds),
            ...
            "coefficient_qmax": 127, "scene_fold_count": SCENE_FOLD_COUNT,
            "critical_group_balance": "base_registered_weighting",
            "scene_balance": "selection_time_whole_scene_folds"},
        "selection": {
            ...
            "selected_fold_robustness": deepcopy(selected["fold_robustness"]),
...
def extract(*, actor_path: str | Path, protocol_path: str | Path,
    ...
    if ((output / "program.json").stat().st_size > MAX_PROGRAM_BYTES
            or load_program_envelope(_read(output / "program.json", "program")).to_dict()
               != program.to_dict()):
        raise RuntimeError("Diagnostic v7 release program budget or round trip differs")
    ...
    if train_steps + validation_steps != expected_steps:
        raise RuntimeError("Diagnostic v7 environment-step accounting differs")
    if (producer_sources() != sources or file_hash(actor_path) != bindings["actor_file_sha256"]
            ...
            or file_hash(supplement_path)
               != bindings["development_supplement_file_sha256"]
            or file_hash(designation_path) != bindings["designation_sha256"]):
        raise RuntimeError("Frozen diagnostic v7 inputs changed during extraction")
    _write_json(output / "report.json", report)
    read_saved_report(output,
        expected_report_sha256=file_hash(output / "report.json"),
        actor_path=actor_path, protocol_path=protocol_path,
        manifest_path=manifest_path,
        development_supplement_path=supplement_path,
...
def read_saved_report(output: str | Path, *, expected_report_sha256: str,
    ...
    if not output.is_dir() or output.is_symlink() or output.resolve() != output:
        raise ValueError("Diagnostic v7 evidence directory is missing or unsafe")
    ...
    if (_HEX.fullmatch(str(expected_report_sha256)) is None
        ...
        raise ValueError("Diagnostic v7 report hash differs")
    actor_path = _regular(actor_path, "Actor")
    protocol_path = _regular(protocol_path, "training protocol")
    manifest_path = _regular(manifest_path, "diagnostic manifest")
    supplement_path = _regular(
    ...
    if inputs != {"version": VERSION, "bindings": bindings,
                  ...
                  "formal_ready": False}:
        raise ValueError("Diagnostic v7 input provenance differs")
    report = _read(report_path, "Diagnostic v7 report")
    artifacts = report.get("evidence_artifacts")
    expected_artifacts = {"inputs.json", "rows.npz", "candidates.json"}
    if not isinstance(artifacts, dict) or set(artifacts) != expected_artifacts:
        raise ValueError("Diagnostic v7 evidence artifact set differs")
    for name, expected in artifacts.items():
        ...
        if (not path.is_file() or path.is_symlink() or path.parent != output
            ...
            raise ValueError("Diagnostic v7 evidence artifact changed")
    ...
    program, candidate = _fit_candidate(
        actor, arrays, binding=binding,
        source_full_manifest_bindings=runtime.source_full_manifest_bindings)
    stored = _read(output / "candidates.json", "Diagnostic v7 candidates")
    if stored != {"version": VERSION, "binding_sha256": binding,
                  "selection_status": "frozen_development_candidate",
                  "selection_disclosure": contract()["candidate_selection_disclosure"],
                  "candidate": candidate}:
        raise ValueError("Diagnostic v7 candidate differs from train-only refit")
    if program_path is None:
        raise ValueError("Diagnostic v7 candidate requires its program")
    supplied = _regular(program_path, "Diagnostic v7 program")
    if supplied != output / "program.json" or supplied.stat().st_size > MAX_PROGRAM_BYTES:
        raise ValueError("Diagnostic v7 program path or size differs")
    saved_program = load_program_envelope(_read(supplied, "Diagnostic v7 program"))
    if saved_program.to_dict() != program.to_dict():
        raise ValueError("Diagnostic v7 program differs from authenticated refit")
    train_hashes = set(map(bytes, arrays["observation_hashes"][~arrays["split_validation"]]))
    validation_hashes = set(map(bytes, arrays["observation_hashes"][arrays["split_validation"]]))
    ...
    if (not isinstance(row_accounting, dict)
        ...
        raise ValueError("Diagnostic v7 row accounting differs")
    expected_steps = row_accounting["raw_collection_environment_steps"]
    execution = report.get("execution", {})
    if (type(execution.get("environment_steps")) is not int
            or execution["environment_steps"] != expected_steps):
        raise ValueError("Diagnostic v7 execution accounting differs")
    expected_report = {
        "version": VERSION,
        "status": "passed" if candidate["gate"]["passed"] else "failed",
        "explanation_eligible": bool(candidate["gate"]["passed"]),
        ...
        "program_transport_payload_sha256": _read(
            supplied, "Diagnostic v7 program")["payload_sha256"],
    ...
    if digest(report) != digest(expected_report):
        raise ValueError("Diagnostic v7 report differs from immutable evidence")
    if require_passed and expected_report["status"] != "passed":
        raise ValueError("Diagnostic v7 did not meet its registered explanation gates")
    return report
...
__all__ = [
    ...
    "SCENE_FOLD_COUNT", "FIT_WORKERS", "_FIELDS",
