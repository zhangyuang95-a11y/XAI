...
VERSION = "warehouse-r41-diagnostic-online-runtime-portable.v1"
PORTABLE_PROTOCOL_VERSION = "warehouse-r41-diagnostic-portable-protocol.v1"
...
class R41DiagnosticOnlineAlignmentRuntime(R41OnlineAlignmentRuntime):
    ...
    def __init__(
        ...
        *,
        ...
        protocol_container = _read_object(
            self._training_protocol_path, "Training protocol")
        if protocol_container.get("version") != PORTABLE_PROTOCOL_VERSION:
            raise ValueError("Portable training protocol required")
        wrapper = deepcopy(protocol_container)
        wrapper_claimed = wrapper.pop("content_sha256", None)
        protocol = protocol_container.get("runtime_protocol")
        if (set(protocol_container) != {
                "version", "source_protocol_file_sha256",
                "source_protocol_content_sha256", "runtime_protocol",
                "content_sha256"}
                or wrapper_claimed != digest(wrapper)
                or not isinstance(protocol, Mapping)
                or not re.fullmatch(r"[0-9a-f]{64}", str(
                    protocol_container.get(
                        "source_protocol_content_sha256", "")))):
            raise ValueError("Portable training protocol differs")
        protocol = deepcopy(dict(protocol))
        protocol_content_sha256 = protocol_container[
            "source_protocol_content_sha256"]
        ...
        actual = {
            ...
            "training_protocol_content_sha256": protocol_content_sha256,
    ...
    def verify_binding(self):
        ...
        if identities != self._external_file_identities:
            ...
            if (protocol_container.get("version") != PORTABLE_PROTOCOL_VERSION
                    or wrapper_claimed != digest(wrapper)):
                raise ValueError("Portable training protocol changed")
            protocol_sha256 = protocol_container.get(
                "source_protocol_content_sha256")
            manifest = _read_object(self._manifest_path, "Conflict manifest")
            content = deepcopy(manifest)
            ...
            current = {
                "actor_file_sha256": file_hash(self._actor_path),
                "training_protocol_file_sha256": file_hash(self._training_protocol_path),
                "training_protocol_content_sha256": protocol_sha256,
                "manifest_file_sha256": file_hash(self._manifest_path),
