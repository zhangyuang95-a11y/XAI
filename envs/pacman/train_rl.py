...
def symbolic_artifact_paths(checkpoint_path: Path) -> tuple[Path, Path, Path]:
    if checkpoint_path.name == "dqn_pacman.pt":
        return (
            Path("models/dqn_pacman_symbolic.joblib"),
            Path("artifacts/policy_code.py"),
            Path("artifacts/policy_summary.json"),
        )
    stem = checkpoint_path.stem
    return (
        checkpoint_path.with_name(f"{stem}_symbolic.joblib"),
        Path("artifacts") / f"{stem}_policy_code.py",
        Path("artifacts") / f"{stem}_policy_summary.json",
    )
...
def train(args: argparse.Namespace) -> Path:
    ...
    game_viewer.close()
    visualizer.close()
    if not getattr(args, "distill_symbolic_policy", True):
        print("[distill] skipped symbolic policy distillation")
        return save_path

    try:
        from .symbolic_policy_adapter import distill_symbolic_policy
    except (ImportError, ModuleNotFoundError) as exc:
        print(f"[distill] skipped symbolic policy distillation; dependency unavailable: {exc}")
        return save_path

    symbolic_path, policy_code_path, policy_summary_path = symbolic_artifact_paths(save_path)
    symbolic_policy = distill_symbolic_policy(
        save_path,
        output_path=symbolic_path,
        policy_code_path=policy_code_path,
        summary_path=policy_summary_path,
        rollout_episodes=args.symbolic_rollout_episodes,
        seed=args.seed,
    )
    print(f"[distill] symbolic policy -> {symbolic_path}")
    print(f"[distill] policy code     -> {policy_code_path}")
    print(f"[distill] policy summary  -> {policy_summary_path}")
    print(
        f"[distill] holdout_action_agreement={symbolic_policy.metrics['holdout_action_agreement']:.2%} "
        f"pairwise={symbolic_policy.metrics['holdout_pairwise_agreement']:.2%} "
        f"top_leaf_coverage={symbolic_policy.metrics['top_leaf_coverage']:.2%} "
        f"avg_rule_length={symbolic_policy.metrics['average_rule_length']:.2f}"
    )
    return save_path
