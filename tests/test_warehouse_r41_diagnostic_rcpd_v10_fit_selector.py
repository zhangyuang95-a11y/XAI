...
from backend.training import warehouse_r41_diagnostic_rcpd_v10_fit_selector as subject
...
def test_v10_selector_uses_replacement_registry_without_consumed_v9_targets():
    contract = subject.contract()
    assert subject.VERSION == "warehouse-r41-diagnostic-rcpd-v10-fit-selector.v1"
    assert subject.registry_api.VERSION == \
        "warehouse-r41-diagnostic-rcpd-v10-fresh-outer-registry.v1"
    assert subject.projection_api.VERSION == \
        "warehouse-r41-diagnostic-outer-hash-projection.v10"
    ...
    assert contract["fresh_v10_outer_labels_or_probabilities_read"] is False
    assert "v10 registry" in contract["consumed_v9_outer_input"]


def test_v10_selector_rejects_a_replacement_candidate_grid_before_file_access():
    wrong = "0" * 64
...
def test_v10_validation_wins_mask_precedes_private_development_read(
