...
function pendingAutomatic(){return view?.can_ask?(view.automatic_explanations||[]).find(c=>c.requires_confirmation&&!c.confirmed):null;}
...
function syncAutomaticDialog(){
 const guide=questionGuide();if(guide&&!document.hidden){
  stopHeld();if(focusedQuestionGuide!==guide.id){focusedQuestionGuide=guide.id;($('aiQuestionButton')||$('guidedQuestionExample'))?.focus({preventScroll:true});}
  updateControlLocks();return;
 }
 const panel=$('automaticDialog'),card=pendingAutomatic();if(!panel||!card||document.hidden)return;
 stopHeld();const key=card.id+':'+card.requested;
 if(focusedAutomatic!==key){focusedAutomatic=key;($('understoodExplanationButton')||$('confirmExplanationButton'))?.focus({preventScroll:true});}
 updateControlLocks();
}
function observeAutomatic(){
 automaticObserver?.disconnect();automaticObserver=null;if(pendingUnderstanding())return;
 ...
 if(card.id!==lastAutomaticId){stopHeld();lastAutomaticId=card.id;}
...
function questionPanel(){
 ...
 const zh=lang==='zh',g=pendingUnderstanding()?null:questionGuide(),available=!release?.share_preview||release.share_qa_available;
...
function bind(){if(prolificEntry&&!view?.instance_id)window.ProlificEntry.bind(async next=>{view=next;domain=next.domain;participantId=next.participant_id;await render();},render);if($('startForm'))$('startForm').onsubmit=async e=>{e.preventDefault();if(busy||(!preview&&!release?.study_ready))return;busy=true;updateEntryStatus();adminKey=$('adminInput')?.value||'';try{view=await api('/api/study/session',{domain,participant_id:$('participantInput').value,language:lang,consent:$('consentInput').checked,mode:preview?'preview':'pilot',group:$('groupInput').value},adminKey?{Authorization:'Bearer '+adminKey}:{});participantId=view.participant_id;adminKey='';render();}catch(err){if(err.code==='study_not_ready'){release={...release,study_ready:false};entryFailed=false;entryCheckedAt=-Infinity;}report(err);}finally{busy=false;updateEntryStatus();scheduleEntryCheck();}};
 ...
 if($('aiQuestionButton'))$('aiQuestionButton').onclick=()=>{
  stopHeld();if(!view?.can_ask||pendingUnderstanding()||busy||animationPending)return;
...
async function loadFrame(run,turn){
 stopHeld();if(pendingUnderstanding()||pendingAutomatic()||busy||animationPending||asking||!view?.instance_id)return;
...
document.addEventListener('keydown',e=>{
 if(pendingUnderstanding()||pendingAutomatic()){if(e.repeat&&[' ','Enter'].includes(e.key))e.preventDefault();return;}
