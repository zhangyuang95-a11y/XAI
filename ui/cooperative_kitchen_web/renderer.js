(function (root) {
  ...
  function pot(ctx, x, y, state, lang, recipe = {}) {
    ...
    const cookingSteps = recipe.cooking_steps || recipe.cookingSteps || 4;
    for (let index = 0; index < ingredients; index += 1) circle(ctx, x - 12 + index * 12, y + 7, 3.6, index < state.ingredients ? '#f0cd84' : '#a6b3c3', '#d8e0e9', .7);
    if (cooked) {
      ...
      roundRect(ctx, x - 22, y - 30, 44 * Math.max(0, Math.min(1, (cookingSteps - state.remaining) / cookingSteps)), 5, 2, COLOR.green);
    ...
    else write(ctx, `${labels[lang].P} · ${state.ingredients}/${ingredients}`, x, y + 33, 11, COLOR.muted, 600);
  }
  ...
  function station(ctx, kind, row, col, state, lang) {
    ...
    if (kind === 'P') { pot(ctx, cx, cy - 4, state.pot, lang, state.recipe); return; }
    if (kind === 'I') {
    ...
    const counterIndex = Object.keys(state.counters || {}).sort((a,b) => Number(a.split(',')[0]) - Number(b.split(',')[0]) || Number(a.split(',')[1]) - Number(b.split(',')[1])).indexOf(`${row},${col}`);
    const suffix = shared && counterIndex >= 0 ? ` ${counterIndex + 1}` : '';
  ...
  function draw(canvas, state, options = {}) {
    ...
    const map = state.map;
    if (!Array.isArray(map) || !map.length || !map[0]?.length) return;
    const width = SIZE * map[0].length;
    const height = SIZE * map.length;
    ...
    canvas.style.aspectRatio = `${map[0].length} / ${map.length}`;
    ...
    for (let row = 0; row < map.length; row += 1) {
      for (let col = 0; col < map[row].length; col += 1) {
        ...
        ctx.fillStyle = col < map[row].length / 2 ? '#f6f9fd' : '#fbf9f6'; ctx.fillRect(x, y, SIZE, SIZE);
    ...
    canvas.setAttribute?.('aria-label', `${labels[lang].map} · ${state.turn}/${state.maxSteps} · ${state.orders}/${state.targetOrders}`);
  }
