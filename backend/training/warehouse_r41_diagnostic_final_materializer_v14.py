"""Materialise the one protected v14 warehouse final split after its claim.

This program is intentionally launched only by
``warehouse_r41_diagnostic_final_once_v14``.  Its first filesystem read is the
...
identity and every public observation used through the v13 outer or burned
v13 timeout prefix is excluded.  Selection uses an independent v14 salt,
fixed family quotas, and the exact observation-only projection of the later final
...
from backend.training import warehouse_r41_diagnostic_final_observation_projection_v14 as final_projection_api
from backend.training import warehouse_r41_diagnostic_final_timeout_closeout_public_v14 as timeout_closeout_api
from backend.training import warehouse_r41_diagnostic_final_candidate_universe_v14 as universe_api
from backend.training import warehouse_r41_diagnostic_final_timing_calibration_v14 as timing_api
...
VERSION = "warehouse-r41-diagnostic-final-materializer.v14"
MATERIAL_VERSION = "warehouse-r41-diagnostic-final-material.v14"
MATERIAL_STATUS = "materialized_after_irrevocable_final_claim"
CONFIG_VERSION = VERSION + ".config.v4"
CONFIG_ENV = "WAREHOUSE_R41_V14_FINAL_MATERIALIZER_CONFIG"
FINAL_ONCE_VERSION = "warehouse-r41-diagnostic-final-once.v14"
ANCHOR_VERSION = FINAL_ONCE_VERSION + ".attempt-anchor.v1"
ANCHOR_STATUS = "final_attempt_irrevocably_claimed"
FINAL_SCENE_COUNT = 64
FINAL_SCENE_OFFSET = 1_400_000
...
PRIVATE_SALT_COMMITMENT = (
    "08f70acfd23b4eb2fd832b0c155462df49a03a18841764f9026bf227224fa77a"
)
PRIVATE_SALT_PATH = Path(
    "/Users/zhangyuang/.config/policylens/"
    "warehouse_r41_diagnostic_v14_holdout_salt.bin")
...
_CONFIG_PATH_FIELDS = frozenset((
    ...
    "timeout_closeout", "permanent_timeout_closeout_registry",
    "candidate_universe", "timing_calibration",
))
...
def contract() -> dict[str, Any]:
    return {
        ...
        "v13_timeout_closeout_authenticated": True,
        "all_v13_ranked_final_inputs_retired": True,
        "fresh_v14_candidate_universe_authenticated": True,
        "public_timing_calibration_authenticated": True,
...
def _authenticate_claim(value: str | Path) -> tuple[Path, dict[str, Any]]:
    ...
    path, anchor, _ = _read_json_exact(value, "permanent v14 final claim")
    ...
    if (path.name != "attempt_anchor.json"
            ...
            or bindings.get("private_salt_domain_sha256")
                != sha256(PRIVATE_SALT_DOMAIN).hexdigest()
            or bindings.get("timeout_closeout_content_sha256")
                != inputs.get("timeout_closeout_content_sha256")
            or bindings.get("candidate_universe_content_sha256")
                != inputs.get("candidate_universe_content_sha256")
            or bindings.get("candidate_universe_identity_sha256")
                != inputs.get("candidate_universe_identity_sha256")
            or bindings.get("timing_calibration_content_sha256")
                != inputs.get("timing_calibration_content_sha256")
        ...
        raise ValueError("Permanent v14 final claim semantics differ")
...
def _load_config() -> tuple[Path, dict[str, Path]]:
    ...
    config_path, value, _ = _read_json_exact(supplied, "v14 final materializer config")
    ...
    if (set(value) != {"version", "paths", "content_sha256"}
        ...
        raise ValueError("V14 final materializer configuration differs")
    resolved = {
...
def _authenticate_timeout_closeout_companions(
    *, closeout_path: Path, closeout: Mapping[str, Any],
    permanent_registry: Path,
) -> tuple[dict[str, Any], dict[str, Any]]:
    """Read the full, receipt-bound retired identity/hash companions."""
    bindings = closeout.get("bindings")
    if not isinstance(bindings, Mapping):
        raise ValueError("V13 timeout closeout bindings differ")
    universe_file = closeout_path.parent / timeout_closeout_api.IDENTITY_NAME
    projection_file = closeout_path.parent / timeout_closeout_api.PROJECTION_NAME
    _, burned_universe, _ = _read_json_exact(
        universe_file, "burned v13 candidate universe",
        expected_sha256=bindings["burned_candidate_universe_file_sha256"])
    burned_universe = timeout_closeout_api._validate_universe(burned_universe)
    _, burned_projection, _ = _read_json_exact(
        projection_file, "burned v13 observation hashes",
        expected_sha256=bindings["burned_observation_hashes_file_sha256"])
    burned_projection = timeout_closeout_api._validate_projection(
        burned_projection, burned_universe)
    permanent_campaign = _directory(
        permanent_registry / str(closeout["closeout_claim_key"]),
        "permanent v13 timeout-closeout campaign")
    _, permanent_burned_universe, _ = _read_json_exact(
        permanent_campaign / timeout_closeout_api.IDENTITY_NAME,
        "permanent burned v13 candidate universe",
        expected_sha256=bindings["burned_candidate_universe_file_sha256"])
    _, permanent_burned_projection, _ = _read_json_exact(
        permanent_campaign / timeout_closeout_api.PROJECTION_NAME,
        "permanent burned v13 observation hashes",
        expected_sha256=bindings["burned_observation_hashes_file_sha256"])
    if (permanent_burned_universe != burned_universe
            or permanent_burned_projection != burned_projection
            or burned_universe.get("content_sha256")
                != bindings["burned_candidate_universe_content_sha256"]
            or burned_projection.get("content_sha256")
                != bindings["burned_observation_hashes_content_sha256"]
            or closeout.get("burned_candidate_universe", {}).get(
                "ranking_input_identities_sha256")
                != burned_universe["ranking_input_identities_sha256"]
            or closeout.get("burned_observation_projection", {}).get(
                "unique_observation_hashes_sha256")
                != burned_projection["unique_observation_hashes_sha256"]):
        raise ValueError("V13 timeout closeout companion binding differs")
    return burned_universe, burned_projection
...
def _authenticate_public_inputs(
    *, claim_path: Path, anchor: Mapping[str, Any], paths: Mapping[str, Path],
    ...
    permanent = _directory(
        paths["permanent_final_registry"], "permanent v14 final registry")
    ...
    timeout_closeout_path = outer_api._authenticate_file(
        paths["timeout_closeout"], "v13 timeout closeout",
        expected_sha256=inputs["timeout_closeout_sha256"])
    permanent_timeout = _directory(
        paths["permanent_timeout_closeout_registry"],
        "permanent v13 timeout-closeout registry")
    timeout_closeout = timeout_closeout_api.read_saved_closeout_public(
        timeout_closeout_path,
        expected_closeout_sha256=inputs["timeout_closeout_sha256"],
        permanent_closeout_registry=permanent_timeout)
    burned_universe, burned_projection = (
        _authenticate_timeout_closeout_companions(
            closeout_path=timeout_closeout_path, closeout=timeout_closeout,
            permanent_registry=permanent_timeout))
    candidate_universe = universe_api.read_saved_universe(
        paths["candidate_universe"],
        expected_universe_sha256=inputs["candidate_universe_sha256"],
        timeout_closeout_path=timeout_closeout_path,
        expected_timeout_closeout_sha256=inputs["timeout_closeout_sha256"],
        permanent_timeout_closeout_registry=permanent_timeout)
    timing_calibration = timing_api.read_saved_calibration(
        paths["timing_calibration"],
        expected_calibration_sha256=inputs["timing_calibration_sha256"],
        timeout_closeout_path=timeout_closeout_path,
        expected_timeout_closeout_sha256=inputs["timeout_closeout_sha256"],
        permanent_timeout_closeout_registry=permanent_timeout)
    universe_bindings = candidate_universe.get("bindings", {})
    if (timeout_closeout.get("content_sha256")
            != inputs["timeout_closeout_content_sha256"]
            or candidate_universe.get("content_sha256")
                != inputs["candidate_universe_content_sha256"]
            or candidate_universe.get("ordered_identity_sha256")
                != inputs["candidate_universe_identity_sha256"]
            or timing_calibration.get("content_sha256")
                != inputs["timing_calibration_content_sha256"]
            or universe_bindings.get("actor_sha256")
                != lock_bindings["actor_sha256"]
            or universe_bindings.get("protocol_sha256")
                != lock_bindings["protocol_sha256"]
            or universe_bindings.get("runtime_manifest_sha256")
                != lock_bindings["runtime_manifest_sha256"]):
        raise ValueError("V14 timeout, universe, or timing binding differs")
    ...
    return {
        ...
        "burned_candidate_universe": burned_universe,
        "burned_observation_projection": burned_projection,
...
def _candidate_scene_population(
    ...
    """Authenticate the already-public, salt-free v14 candidate universe."""
    raw_scenes = candidate_universe.get("candidate_scenes")
    raw_identities = candidate_universe.get("candidate_identities")
    if not isinstance(raw_scenes, list) or not isinstance(raw_identities, list):
        raise ValueError("V14 candidate universe is missing")
    scenes = [deepcopy(dict(row)) for row in raw_scenes
              if isinstance(row, Mapping)]
    identities = [deepcopy(dict(row)) for row in raw_identities
                  if isinstance(row, Mapping)]
    mapping = {str(row.get("fingerprint")): row for row in scenes}
    expected = {(row.get("seed"), row.get("fingerprint"),
                 row.get("family_id"), row.get("batch_index"))
                for row in identities}
    actual = {(row.get("seed"), row.get("fingerprint"),
               row.get("family_id"), row.get("batch_index"))
              for row in scenes}
    if (len(scenes) != universe_api.SCENE_COUNT
            or len(identities) != universe_api.SCENE_COUNT
            or len(mapping) != len(scenes) or actual != expected
            or candidate_universe.get("ordered_identity_sha256")
                != digest(identities)):
        raise ValueError("V14 candidate scene and identity projections differ")
    ...
    *, runtime: Any, identities: Sequence[Mapping[str, Any]],
    ...
    for family in FAMILY_IDS:
        ...
        for identity in _ordered_candidates(identities, family=family, salt=salt):
            ...
            frozen["id"] = f"diagnostic_v14_fresh_final_{scene_index:04d}"
...
def _read_committed_salt(path: Path) -> bytes:
    """Open and authenticate the independent v14 salt after the claim."""
    if Path(path).expanduser().absolute() != PRIVATE_SALT_PATH:
        raise ValueError("Only the frozen private v14 salt path is allowed")
    candidate, raw, _actual = _read_once(
        path, "private v14 holdout salt", maximum=32)
    info = candidate.stat(follow_symlinks=False)
    if (len(raw) != 32 or stat.S_IMODE(info.st_mode) != 0o600
            or sha256(PRIVATE_SALT_DOMAIN + raw).hexdigest()
                != PRIVATE_SALT_COMMITMENT):
        raise ValueError("Private v14 salt does not match its frozen commitment")
    return raw
...
def _prepare_selection(
    *, paths: Mapping[str, Path], authenticated: Mapping[str, Any],
    ...
    (_historical_identities, excluded_seeds, excluded_fingerprints, source_row_scenes,
    ...
    burned_universe = authenticated.get("burned_candidate_universe")
    burned_projection = authenticated.get("burned_observation_projection")
    if (not isinstance(burned_universe, Mapping)
            or not isinstance(burned_projection, Mapping)
            or not isinstance(
                burned_universe.get("ranking_input_identities"), list)
            or not isinstance(
                burned_projection.get("unique_observation_hashes"), list)):
        raise ValueError("V13 timeout exclusion closure differs")
    for row in burned_universe["ranking_input_identities"]:
        if not isinstance(row, Mapping):
            raise ValueError("V13 burned identity differs")
        excluded_seeds.add(int(row["seed"]))
        excluded_fingerprints.add(str(row["fingerprint"]))
    excluded_fingerprints.update(authenticated["development_scenes"])
    identities, scene_by_fingerprint = _candidate_scene_population(
        authenticated["candidate_universe"])
    new_seeds = {int(row["seed"]) for row in identities}
    new_fingerprints = {str(row["fingerprint"]) for row in identities}
    if (new_seeds & excluded_seeds or new_fingerprints & excluded_fingerprints
            or new_fingerprints & set(source_row_scenes)):
        raise ValueError("V14 candidate identity overlaps prior exposure")
    ...
    forbidden = (
        ...
        | prior_hashes
        | set(map(str, burned_projection["unique_observation_hashes"])))
    ...
    return {
        ...
        "burned_v13_candidate_fingerprints": {
            str(row["fingerprint"])
            for row in burned_universe["ranking_input_identities"]},
        "runtime": runtime,
    }
    ...
    *, anchor: Mapping[str, Any], authenticated: Mapping[str, Any],
    ...
    if ({scene["fingerprint"] for scene in scenes}
            & (authenticated["development_scenes"]
               ...
               | prepared["burned_v13_candidate_fingerprints"])):
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--claim", required=True,
                        help="Permanent v14 final-once attempt_anchor.json")
