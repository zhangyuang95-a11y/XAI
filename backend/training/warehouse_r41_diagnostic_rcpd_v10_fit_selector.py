"""Select and lock one public-tree candidate for a replacement v10 fresh outer.
...
selection.  The replacement v10 outer contributes a label-blind ordered
...
VERSION = "warehouse-r41-diagnostic-rcpd-v10-fit-selector.v1"
...
def contract() -> dict[str, Any]:
    return {
        ...
        "consumed_v9_outer_input": (
            "identity exclusion is bound by the v10 registry; labels and "
            "probabilities are not selector inputs"
        ),
        "fresh_outer_input": "ordered v10 label-blind observation SHA-256 projection only",
        ...
        "fresh_v10_outer_labels_or_probabilities_read": False,
    ...
    for salt in salts:
        ...
        for fold in range(FOLD_COUNT):
            ...
            if (predicted.shape != (int(np.sum(validation)), len(fit_api.ACTIONS))
                ...
                raise ValueError("V10 fold prediction probabilities differ")
    ...
    value: dict[str, Any] = {
        ...
        "selection": {
            "selected_config_sha256": _sha(
                selected_config_sha256, "selected v10 fit configuration"),
...
def _resolve_snapshot(
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    snapshot = ImmutableInputSnapshot(
        ...
        prefix="warehouse-r41-v10-fit-selector-inputs-",
    )
    ...
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    try:
        ...
        if (designation.get("bindings", {}).get("actor_sha256") != actor.artifact_sha256
            ...
            raise ValueError("V10 selector evidence bindings differ")
        ...
        if relations.base_feature_names != actor_feature_names:
            raise ValueError("V10 relation base features differ from the frozen Actor")
        ...
        if (program.base_feature_names != actor_feature_names
            ...
            raise RuntimeError("Locked v10 program relation/Actor binding differs")
        ...
        if (not np.array_equal(
            ...
            raise RuntimeError("Locked v10 program round-trip action parity differs")
        if final_gate.get("passed") is not True:
            raise RuntimeError("Full-development v10 refit failed the nine hard gates")

        temporary = Path(tempfile.mkdtemp(
            prefix=".warehouse-r41-v10-fit-selector-", dir=destination.parent))
