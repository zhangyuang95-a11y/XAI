"""Real held-out explanation acceptance for the frozen r4.1 online runtime.
...
from backend.warehouse_r41_online_explanation import (
    R41OnlineAlignmentExplainer as OnlineAlignmentExplainer,
)
from backend.warehouse_r41_online_runtime import R41OnlineAlignmentRuntime as OnlineAlignmentRuntime
from env.warehouse.navigation import ACTIONS
from env.warehouse_native.partners import partner_action


VERSION = "warehouse-r41-online-explanation-audit.v1"
...
def audit(*, actor_path: str | Path, protocol_path: str | Path, program_path: str | Path,
    ...
    from backend.training.warehouse_r41_conflict_scenarios import (
        MANIFEST_VERSION, validate_conflict_manifest,
    )
    validate_conflict_manifest(scenarios, replay=True)
    if (scenarios.get("version") != MANIFEST_VERSION
            or not isinstance(split, list) or len(split) != EXPLANATION_SCENES
            or runtime.config.horizon != 120
            or scenarios.get("content_sha256")
                != runtime.actor.metadata.get("conflict_manifest_semantic_sha256")):
        raise ValueError("Exact frozen r4.1 final-test explanation split required")
...
def read_saved_report(output: str | Path, *, expected_report_sha256: str,
    ...
    if (report["base_rows"] != len(ordinary)
            or report["episodes"] != EXPLANATION_SCENES * len(PARTNERS)
...
__all__ = ["VERSION", "PARTNERS", "GROUPS", "QUESTIONS", "EXPLANATION_SCENES", "contract",
