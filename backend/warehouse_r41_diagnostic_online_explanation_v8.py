"""Readable, frame-bound explanations for the r4.1 diagnostic v8 program.
...
VERSION = "warehouse-r41-diagnostic-online-readable-answers.v8"
...
def explanation_sources() -> dict[str, str]:
    """Return the complete source closure used by the v8 renderer."""
    ...
    for path in paths:
        if not path.is_file() or path.is_symlink():
            raise ValueError("r4.1 v8 explanation source is missing: " + str(path))
...
def _validated_program_trace(trace: Any) -> dict[str, Any]:
    ...
    if not isinstance(trace, Mapping):
        raise ValueError("v8_program_trace_malformed")
...
class R41DiagnosticOnlineAlignmentExplainer:
    """Evidence renderer bound to the exact runtime and explicit v8 program."""

    def __init__(self, program_path: str | Path, *, expected_program_sha256: str,
                 runtime: Any, allow_test_fixture: bool = False):
        ...
        try:
            ...
            self.program = R41DiagnosticPublicTreeProgramV8.from_dict(payload)
        except (TypeError, ValueError, json.JSONDecodeError) as error:
            raise ValueError("RCPD v8 public-tree program differs") from error
        ...
        if (tuple(self.program.base_feature_names) != expected_features
            ...
            raise ValueError("Program source, v8 public schema, or action authority differs")
    ...
    def _answer(self, request: Mapping[str, Any], frame_record: Mapping[str, Any],
        ...
        if program_action != program_trace["prediction"] or program_action not in EXACT_ACTIONS:
            raise ValueError("v8_program_trace_prediction_differs")
...
DiagnosticOnlineExplainer = R41DiagnosticOnlineAlignmentExplainer

__all__ = [
    "VERSION", "RCPD_VERSION", "EXACT_ACTIONS", "QUICK_QUESTIONS",
    "R41DiagnosticOnlineAlignmentExplainer", "DiagnosticOnlineExplainer",
    "explanation_sources",
]
