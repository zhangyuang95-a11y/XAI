...
(() => {
  ...
  function visibleAnswers(view){
    ...
    return (Array.isArray(view.answers)?view.answers:[]).filter(a=>!a.run_id || a.run_id===view.run_id).slice(0,1).map(a=>({
      ...
      evidence_detail:typeof a.evidence_detail==="string"?a.evidence_detail:"",
    }));
  }
  ...
  function confirmedMotion(before,after,kind,replay){
    const tutorial=kind==="tutorial_advance" && phase(before)==="instructions" && phase(after)==="instructions"
      && Number.isInteger(before.tutorial?.frame_index) && after.tutorial?.frame_index===before.tutorial.frame_index+1;
    if((kind!=="action" && !tutorial) || replay!==null || !before || !after
  ...
  Object.assign(WORDS.zh,{instructions:"任务说明与演示",startStudy:"同意并查看说明",tutorialTitle:"AI–AI 协作教学演示",tutorialText:"先观看两个 AI 完成一次固定协作配送，了解取货、交付、充电和碰撞规则。演示不计入正式任务成绩。",tutorialUnscored:"演示记录与 Task 1 的三局成绩完全分开。",playTutorial:"播放",pauseTutorial:"暂停",previousTutorialFrame:"上一帧",nextTutorialFrame:"下一帧",restartTutorial:"从头播放",skipTutorial:"跳过演示并开始 Task 1",beginTask1:"开始 Task 1",tutorialFrame:"教学演示帧",aiOne:"AI · 1"});
  Object.assign(WORDS.en,{instructions:"Instructions & demo",startStudy:"Consent and view instructions",tutorialTitle:"AI–AI cooperation tutorial",tutorialText:"Watch two AI robots complete one fixed collaborative delivery to learn pickup, delivery, charging, and collision rules. The demo does not count toward task scores.",tutorialUnscored:"Tutorial records are fully separate from the three Task 1 rounds.",playTutorial:"Play",pauseTutorial:"Pause",previousTutorialFrame:"Previous frame",nextTutorialFrame:"Next frame",restartTutorial:"Restart",skipTutorial:"Skip demo and begin Task 1",beginTask1:"Begin Task 1",tutorialFrame:"Tutorial frame",aiOne:"AI · 1"});
  Object.assign(WORDS.zh,{explanation:"系统问答",system:"系统",whyPenalty:"刚才为什么扣分？",chargerPenalty:"充电桩占用处罚"});
  Object.assign(WORDS.en,{explanation:"System questions",system:"System",whyPenalty:"Why did we just lose points?",chargerPenalty:"Shared-charger occupancy penalty"});
  const ui={view:null,entry:"study",language:readStorage(localStorage,LANGUAGE_KEY)==="en"?"en":"zh",busy:false,error:null,replay:null,history:null,historyToken:0,historyLoading:null,refreshing:false,userNavigationEpoch:0,viewportEpoch:0,initialized:false,seen:new Set(),seenQueue:new Set(),seenTimer:null,seenPenalties:new Set(),penaltyTimer:null,questionnaireDirty:false,questionnairePreview:null,feedbackRun:null,answerSignature:null,motion:null,motionToken:0,demoPlaying:false,demoToken:0,pendingBeginTask1:false};
  ...
  function paint(){
    ...
    const stages=["consent","instructions","task1","task2","questionnaire","completed"],workflowStage=p==="registration"?"consent":p==="task1_complete"?"task1":p,index=stages.indexOf(workflowStage);
    $("workflow").classList.toggle("hidden",!(isStudy || ui.entry==="study"));document.querySelectorAll("[data-stage]").forEach(n=>{n.classList.toggle("active",n.dataset.stage===workflowStage);n.classList.toggle("done",isStudy && stages.indexOf(n.dataset.stage)<index);});
    $("stepValue").textContent=s?`${s.frame} / ${v.horizon || 120}`:"—";
    const showMetrics=!!v?.run_id || p==="instructions";for(const [id,key] of [["scoreValue","score"],["deliveriesValue","deliveries"],["stepsValue","steps"],["collisionsValue","collisions"],["shutdownsValue","shutdowns"]])$(id).textContent=showMetrics?format(m[key]):"—";
    $("emptyMessage").classList.toggle("hidden",!!v?.map && (!!v.run_id || p==="instructions" && !!f?.state) || p==="questionnaire" && !!f?.state);$("emptyMessage").textContent=v?.study_version_mismatch?tr("studyVersionChanged"):["questionnaire","completed","consent"].includes(p)?tr(p):v?(!playable(v)?tr(failed?"failedGate":"modelGate"):tr(ui.entry==="study"?"consent":"startFreeplay")):tr("connecting");
    ...
    const tutorial=v?.tutorial,tutorialActive=p==="instructions" && !!tutorial;if(!tutorialActive && ui.demoPlaying){ui.demoPlaying=false;ui.demoToken++;}
    $("instructionsPanel").classList.toggle("hidden",!tutorialActive);
    if(tutorialActive){const total=Math.max(1,Number(tutorial.total_frames)||1),current=Math.max(0,Number(tutorial.frame_index)||0),maximum=Math.max(current,Number(tutorial.max_played_index)||0);$("tutorialFrameLabel").textContent=`${current+1} / ${total}`;$("tutorialSlider").max=String(maximum);$("tutorialSlider").value=String(current);$("tutorialPlayButton").textContent=ui.demoPlaying?tr("pauseTutorial"):tr("playTutorial");$("beginTask1Button").textContent=tutorial.complete?tr("beginTask1"):tr("skipTutorial");disable($("tutorialPreviousButton"),locked || current<=0 || !allowed(v,"tutorial_select"));disable($("tutorialNextButton"),locked || current>=total-1 || !allowed(v,"tutorial_advance"));disable($("tutorialRestartButton"),locked || current<=0 || !allowed(v,"tutorial_restart"));disable($("tutorialSlider"),locked || maximum<=0 || !allowed(v,"tutorial_select"));disable($("beginTask1Button"),!allowed(v,"begin_task1") || locked && !ui.demoPlaying);disable($("tutorialPlayButton"),!ui.demoPlaying && (locked || !allowed(v,tutorial.complete?"tutorial_restart":"tutorial_advance")));}
    ...
    drawWarehouse(f,ui.motion?.positions);drawQuestionMarkers(f);document.body.dataset.stage=p;document.body.dataset.version=String(v?.version ?? 0);document.body.dataset.frame=String(s?.frame ?? 0);document.body.dataset.replay=String(ui.replay!==null);document.body.dataset.historyLoading=String(ui.historyLoading!==null);document.body.dataset.tutorialPlaying=String(ui.demoPlaying);document.body.dataset.frontendVersion=FRONTEND_VERSION;document.body.dataset.explanationAllowed=String(explanationsAllowed(v));
  }
  function renderExplanations(view,locked){
    ...
    $("questionFrame").textContent=`${tr("system")} · ${tr("frame")} ${shown()?.state?.frame ?? 0}`;
    const hasPenalty=(shown()?.events || []).some(event=>event?.event==="charger_occupancy_penalty");$("penaltyQuestionButton").classList.toggle("hidden",!hasPenalty);
    for(const b of document.querySelectorAll("[data-question]"))disable(b,locked);disable($("askButton"),locked);disable($("questionInput"),locked);
    const answers=visibleAnswers(view),signature=JSON.stringify([answers,ui.language]);
    if(signature!==ui.answerSignature){ui.answerSignature=signature;$("answerList").replaceChildren(...answers.map(a=>{const box=document.createElement("article");box.className="answer";const q=document.createElement("strong");q.textContent=a.question;const body=document.createElement("p");body.textContent=["pending","queued","running"].includes(a.status)?tr("questionPending"):a.text || tr("answerUnavailable");const meta=document.createElement("small");meta.textContent=`${tr("system")} · ${tr("answerFrame")} ${a.frame ?? "—"}`;box.append(q,body,meta);if(a.evidence_detail){const details=document.createElement("details");details.className="evidence-detail";const summary=document.createElement("summary");summary.textContent=tr("evidenceDetails");const evidence=document.createElement("pre");evidence.textContent=a.evidence_detail;details.append(summary,evidence);box.append(details);}return box;}));}
  ...
  function penaltyEvent(view){return (view?.events || []).find(event=>event?.event==="charger_occupancy_penalty") || null;}
  function playPenaltyFeedback(view){const event=penaltyEvent(view);if(!event)return;const key=`${view?.run_id || "tutorial"}:${event.event_id || view?.state?.frame}`;if(ui.seenPenalties.has(key))return;ui.seenPenalties.add(key);const flash=$("penaltyFlash"),toast=$("penaltyToast");if(ui.penaltyTimer)clearTimeout(ui.penaltyTimer);flash.classList.remove("active");toast.classList.remove("active");toast.hidden=false;void flash.offsetWidth;flash.classList.add("active");toast.classList.add("active");ui.penaltyTimer=setTimeout(()=>{flash.classList.remove("active");toast.classList.remove("active");toast.hidden=true;ui.penaltyTimer=null;},1100);}
  ...
  async function execute(payload,retry=false){
    if(ui.busy || ui.historyLoading!==null || (!retry && transport.pending()) || !canSubmit(ui.view,payload.kind,retry))return null;
    let accepted=null;ui.busy=true;ui.error=null;render();
    try{
      const before=ui.view,submitted=retry?transport.pending()?.body:payload;
      const result=retry?await transport.retry():await transport.submit({operation_id:crypto.randomUUID(),expected_version:ui.view.version,...payload},ui.view.session_id);
      const next=normalizeView(result);if(payload.kind==="questionnaire")ui.questionnaireDirty=false;
      await animateConfirmedAction(before,next,submitted?.kind);adopt(next);if(submitted?.kind==="action")playPenaltyFeedback(next);accepted=next;
    }catch(e){
      ui.motionToken++;ui.motion=null;ui.error=["study_version_mismatch","study_version_changed"].includes(e.message)?"studyVersionChanged":e.message==="participant_id_taken"?"participantTaken":e.message==="invalid_participant_id"?"participantInvalid":e.status===409?"conflict":"requestFailed";
      if(!transport.pending())try{adopt(await transport.request("/api/view"));}catch{ui.error="requestFailed";}
    }finally{ui.motionToken++;ui.motion=null;ui.busy=false;render();}
    return accepted;
  }
  async function playTutorial(){
    if(ui.demoPlaying){ui.demoPlaying=false;ui.demoToken++;render();return;}
    if(phase(ui.view)!=="instructions" || !ui.view?.tutorial)return;
    const token=++ui.demoToken;ui.pendingBeginTask1=false;ui.demoPlaying=true;render();
    try{
      if(ui.view.tutorial.complete){const restarted=await execute({kind:"tutorial_restart"});if(!restarted || token!==ui.demoToken)return;}
      while(ui.demoPlaying && token===ui.demoToken && phase(ui.view)==="instructions" && !ui.view?.tutorial?.complete){
        const advanced=await execute({kind:"tutorial_advance"});if(!advanced)return;
        if(ui.pendingBeginTask1 && phase(ui.view)==="instructions"){
          ui.pendingBeginTask1=false;await execute({kind:"begin_task1"});return;
        }
      }
    }finally{if(token===ui.demoToken){ui.demoPlaying=false;ui.pendingBeginTask1=false;render();}}
  }
  async function beginTask1(){
    if(phase(ui.view)!=="instructions" || !allowed(ui.view,"begin_task1"))return;
    ui.demoPlaying=false;ui.demoToken++;
    if(ui.busy){ui.pendingBeginTask1=true;render();return;}
    await execute({kind:"begin_task1"});
  }
  ...
  function sameCell(a,b){return Array.isArray(a) && Array.isArray(b) && a[0]===b[0] && a[1]===b[1];}
  function taskOrdinal(task,index){const n=String(task?.task_id || "").match(/(\d+)$/);return n?Number(n[1]):index+1;}
  function actionWord(action){return actionLabel(action).replace("—",ui.language==="zh"?"等待":"wait");}
  function aiReasonAllowed(view){return explanationsAllowed(view) && study(view) && ["task1","task1_complete"].includes(phase(view));}
  function nearestTaskByDistance(map,origin,tasks,key){
    let best=null;for(const [index,task] of tasks.entries()){const target=task?.[key];if(!target)continue;const d=gridDistance(map,origin,target);if(d===null)continue;if(!best || d<best.distance)best={task,index,distance:d};}return best;
  }
  function gridDistance(map,start,goal){
    if(!Array.isArray(start) || !Array.isArray(goal) || !map)return null;
    const rows=Number(map.rows),cols=Number(map.cols);if(!rows || !cols)return null;
    const blocked=new Set((map.shelves || []).map(p=>`${p[0]},${p[1]}`)),source=[Math.round(start[0]),Math.round(start[1])],target=[Math.round(goal[0]),Math.round(goal[1])];
    if(source[0]===target[0] && source[1]===target[1])return 0;
    const queue=[[source,0]],seen=new Set([`${source[0]},${source[1]}`]),dirs=[[1,0],[-1,0],[0,1],[0,-1]];
    while(queue.length){const [[r,c],d]=queue.shift();for(const [dr,dc] of dirs){const nr=r+dr,nc=c+dc,key=`${nr},${nc}`;if(nr<0 || nr>=rows || nc<0 || nc>=cols || blocked.has(key) || seen.has(key))continue;if(nr===target[0] && nc===target[1])return d+1;seen.add(key);queue.push([[nr,nc],d+1]);}}
    return null;
  }
  ...
  function drawWarehouse(view,visualPositions=null){const canvas=$("warehouseCanvas"),width=canvas.clientWidth,height=canvas.clientHeight,ratio=window.devicePixelRatio || 1;if(!width || !height)return;const pixelWidth=Math.round(width*ratio),pixelHeight=Math.round(height*ratio);if(canvas.width!==pixelWidth)canvas.width=pixelWidth;if(canvas.height!==pixelHeight)canvas.height=pixelHeight;const ctx=canvas.getContext("2d");ctx.setTransform(ratio,0,0,ratio,0,0);ctx.clearRect(0,0,width,height);canvas.dataset.animationRunning=String(ui.motion!==null);canvas.dataset.animationProgress=String(ui.motion?.progress ?? 1);if(!view?.map){positionAiReasonBubble(null);return;}const map=view.map,rows=map.rows,cols=map.cols,size=Math.min((width-34)/cols,(height-34)/rows),ox=(width-cols*size)/2,oy=(height-rows*size)/2;const cell=p=>[ox+p[1]*size,oy+p[0]*size];ctx.fillStyle="#f8fafc";ctx.fillRect(ox,oy,cols*size,rows*size);ctx.strokeStyle="#dce4ef";ctx.lineWidth=1;for(let r=0;r<=rows;r++){ctx.beginPath();ctx.moveTo(ox,oy+r*size);ctx.lineTo(ox+cols*size,oy+r*size);ctx.stroke();}for(let c=0;c<=cols;c++){ctx.beginPath();ctx.moveTo(ox+c*size,oy);ctx.lineTo(ox+c*size,oy+rows*size);ctx.stroke();}for(const p of map.shelves || []){const[x,y]=cell(p);ctx.fillStyle="#9aa8ba";ctx.fillRect(x+2,y+2,size-4,size-4);}ctx.textAlign="center";ctx.textBaseline="middle";if(map.charger_position){const[x,y]=cell(map.charger_position);ctx.fillStyle="#6558e8";ctx.fillRect(x+4,y+4,size-8,size-8);ctx.fillStyle="#fff";ctx.font=`bold ${size*.43}px sans-serif`;ctx.fillText("⚡",x+size/2,y+size/2);}const tasks=view.state?.tasks || [];for(const [i,t] of tasks.entries()){ctx.font=`bold ${size*.25}px sans-serif`;for(const [key,label] of [["pickup_position",`A${i+1}`],["delivery_position",`B${i+1}`]]){if(key==="pickup_position" && t.status!=="available")continue;if(!t[key])continue;const[x,y]=cell(t[key]);ctx.fillStyle=taskColor(t.task_id,i);ctx.beginPath();ctx.arc(x+size/2,y+size/2,size*.3,0,Math.PI*2);ctx.fill();ctx.fillStyle="#fff";ctx.fillText(label,x+size/2,y+size/2);}}
    ...
    positionAiReasonBubble(view,visualPositions,{size,ox,oy});
  }
  ...
  $("tutorialPlayButton").addEventListener("click",()=>void playTutorial());
  $("tutorialRestartButton").addEventListener("click",()=>{if(phase(ui.view)==="instructions")void execute({kind:"tutorial_restart"});});
  $("tutorialPreviousButton").addEventListener("click",()=>{const index=Number(ui.view?.tutorial?.frame_index ?? 0);if(index>0)void execute({kind:"tutorial_select",frame_index:index-1});});
  $("tutorialNextButton").addEventListener("click",()=>{if(phase(ui.view)==="instructions")void execute({kind:"tutorial_advance"});});
  ...
  const QUICK_INTENTS={whyAction:"action_reason",whyWait:"wait_reason",whyCollision:"collision_reason",humanInfluence:"human_influence",currentGoal:"task_direction",chargingNeed:"charging_need",whyPenalty:"penalty_reason"};document.querySelectorAll("[data-question]").forEach(b=>b.addEventListener("click",()=>ask(tr(b.dataset.question),QUICK_INTENTS[b.dataset.question] || null)));$("askButton").addEventListener("click",()=>ask($("questionInput").value,null));
