...
function clearChat(){stopHeld();chatEpoch++;if(view){view.questions=[];view.automatic_explanations=[];view.can_ask=false;}draftQuestion='';asking=false;const box=$('chatLog');if(box)box.replaceChildren();const input=$('questionInput');if(input)input.value='';if($('chatPanel'))$('chatPanel').hidden=true;automaticObserver?.disconnect();if($('automaticExplanation'))$('automaticExplanation').remove();}
...
function automaticPanel(){
 const card=latestAutomatic();if(!card)return '';
...
function observeAutomatic(){
 ...
 const card=latestAutomatic(),element=$('automaticExplanation');
...
async function render({animate=false,displayState=null}={}){
 ...
 const template=document.createElement('template');template.innerHTML=html;reconcileChildren($('app'),template.content);restoreEntryDraft(entryDraft);bind();observeAutomatic();
