"""Traceable v8 public-tree composition with a NumPy-only runtime.

The program accepts only the frozen 197-value public observation.  It derives
the registered 349 public relation features deterministically, evaluates a
...
VERSION = "warehouse-r41-diagnostic-public-tree-program.v8"
...
class R41DiagnosticPublicTreeProgramV8:
    ...
    def __init__(self, payload: Mapping[str, Any]):
        if not isinstance(payload, Mapping) or set(payload) != _TOP_FIELDS:
            raise ValueError("Public-tree v8 top-level schema differs")
        if payload.get("version") != VERSION:
            raise ValueError("Public-tree v8 version differs")

        relations_payload = payload.get("relations")
        if not isinstance(relations_payload, Mapping):
            raise ValueError("Public-tree v8 relations contract differs")
        base_names = relations_payload.get("base_feature_names")
        try:
            self.relations = R41DiagnosticPublicRelationsV8(base_names)
        except (TypeError, ValueError) as exc:
            raise ValueError("Public-tree v8 relations contract differs") from exc
        if (len(self.relations.base_feature_names) != 197
                or len(self.relations.feature_names) != 349
                or dict(relations_payload) != self.relations.contract()):
            raise ValueError("Public-tree v8 requires the exact 197-to-349 relation contract")
        ...
        if (not isinstance(action_names, list) or len(action_names) < 2
            ...
            raise ValueError("Public-tree v8 action registry differs")
        self.action_names = tuple(action_names)
        if payload.get("aggregation") != _aggregation():
            raise ValueError("Public-tree v8 aggregation contract differs")
        ...
        if not isinstance(specialists, list) or len(specialists) != len(GROUPS):
            raise ValueError("Public-tree v8 specialist registry differs")
        ...
        for index, item in enumerate(specialists):
            ...
            if (not isinstance(item, Mapping) or set(item) != _SPECIALIST_FIELDS
                ...
                raise ValueError("Public-tree v8 specialist order differs")
        ...
        if not isinstance(metadata, dict):
            raise ValueError("Public-tree v8 metadata must be an object")
    ...
    def _validate_program(
        ...
        if program.feature_names != self.feature_names:
            raise ValueError(label + " program must consume exactly 349 public features")
        if program.action_names != self.action_names:
            raise ValueError(label + " action registry differs")

    @classmethod
    def from_dict(cls, payload: Mapping[str, Any]) -> "R41DiagnosticPublicTreeProgramV8":
        return cls(payload)

    @classmethod
    def from_json(cls, payload: str) -> "R41DiagnosticPublicTreeProgramV8":
        if type(payload) is not str:
            raise ValueError("Public-tree v8 JSON payload must be text")
        try:
            decoded = json.loads(payload)
        except (TypeError, ValueError) as exc:
            raise ValueError("Public-tree v8 JSON payload is invalid") from exc
        return cls(decoded)
    ...
    def from_programs(
        ...
        *,
        ...
        """Assemble already-fitted/exported candidates into the v8 runtime."""

        relations = R41DiagnosticPublicRelationsV8(base_feature_names)
        if len(relations.base_feature_names) != 197 or len(relations.feature_names) != 349:
            raise ValueError("Public-tree v8 requires exactly 197 public base features")
        programs = {
        ...
        if not isinstance(mix_weights, Mapping) or set(mix_weights) != set(GROUPS):
            raise ValueError("Public-tree v8 requires one mix weight for every group")
        ...
        if not isinstance(route_values, Mapping) or set(route_values) != set(GROUPS):
            raise ValueError("Public-tree v8 requires one public route for every group")
        normalized_routes = {}
        for group in GROUPS:
    ...
    def _mapping_vector(self, observation: Mapping[str, Any]) -> np.ndarray:
        if not isinstance(observation, Mapping):
            raise ValueError("Public-tree v8 observation must be a mapping")
        if set(observation) != set(self.base_feature_names):
            raise ValueError("Public-tree v8 requires exactly 197 raw public features")
        try:
            vector = np.asarray([
                observation[name] for name in self.base_feature_names
            ], dtype=np.float32)
        except (TypeError, ValueError) as exc:
            raise ValueError("Public-tree v8 public observation differs") from exc
        if vector.shape != (197,) or not np.isfinite(vector).all():
            raise ValueError("Public-tree v8 public observation must be finite")
        return vector
    ...
    def _predict_expanded(self, expanded: np.ndarray) -> np.ndarray:
        ...
        if (not np.isfinite(probabilities).all()
                or not np.allclose(probabilities.sum(axis=1), 1.0,
                                   rtol=0.0, atol=2e-15)):
            raise ValueError("Public-tree v8 final probabilities are invalid")
...
def assemble_public_tree_program_v8(
    base_feature_names: Sequence[str],
    base_program: R41DiagnosticBoostedTreeProgram | Mapping[str, Any],
    narrow_passage: R41DiagnosticBoostedTreeProgram | Mapping[str, Any],
    shared_pickup: R41DiagnosticBoostedTreeProgram | Mapping[str, Any],
...
__all__ = [
    "VERSION", "GROUPS", "AGGREGATION", "R41DiagnosticPublicTreeProgramV8",
