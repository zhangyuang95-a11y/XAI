"""Genuine stable Actor admission; inherited verified contexts and public physics.

The historical registry and branch runtime stay unchanged. Only this module's
exact StableRuntime type is accepted by its verifier. New stable metadata is
...
from backend.training import warehouse_family_stable_evaluation as admission
...
RUNTIME_VERSION = "warehouse-family-stable-direct-runtime.v1"
VERIFY_VERSION = "warehouse-family-stable-runtime-verification.v1"
FAMILY = "stable_feedback197"
...
class StableRuntime(inherited.BranchRuntime):
    ...
    def __init__(self, actor_path, *, protocol, expected_actor_sha256, expected_protocol_sha256,
        ...
        if digest(protocol) != expected_protocol_sha256: raise ValueError("Stable protocol external anchor differs")
        self._actor_path = Path(actor_path).expanduser().resolve()
        if file_hash(self._actor_path) != expected_actor_sha256: raise ValueError("Stable Actor bytes differ")
        ...
        if self.config != collaborative_study_config(): raise ValueError("Stable runtime keeps the original public configuration")
        if expected_bindings is not None:
            ...
            for key, value in expected_bindings.items():
                ...
                if digest(actual) != digest(value):
                    raise ValueError("Stable runtime external binding differs: " + key)
        ...
        self.contract_report = {"version": RUNTIME_VERSION, "signature": self.signature,
            ...
            "training_stage": "stable_feedback", "shutdown_arm": self.actor.metadata["shutdown_arm"],
    ...
    def _full_verify_binding(self):
        if (type(self.actor) is not NumPyNativeActor or digest(asdict(self.config)) != self._configuration_sha256
            ...
            raise ValueError("Stable runtime Actor, configuration, source or signature changed")
...
def verify(runtime, *, allow_test_fixture=False, expected_family=None, expected_signature=None):
    ...
    if type(runtime) is not StableRuntime or type(allow_test_fixture) is not bool or runtime.test_fixture is not allow_test_fixture:
        raise ValueError("Only the exact new StableRuntime and matching fixture scope are supported")
    if expected_family is not None and expected_family != FAMILY: raise ValueError("Stable runtime family differs")
    signature = runtime.verify_binding()
    if expected_signature is not None and expected_signature != signature: raise ValueError("Stable runtime signature differs")
...
def fresh_instance(runtime, *, allow_test_fixture=False, expected_family=None, expected_signature=None):
    ...
    result = StableRuntime(runtime._actor_path, protocol=runtime.protocol,
    ...
    if verify(result, allow_test_fixture=allow_test_fixture) != before:
        raise ValueError("Private stable runtime differs from its source")
    return result
