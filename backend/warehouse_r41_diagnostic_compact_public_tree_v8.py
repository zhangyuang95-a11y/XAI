"""Compact, explicit transport for the r4.1 v8 public-tree program.
...
VERSION = "warehouse-r41-diagnostic-compact-public-tree.v1"
MAGIC = b"PLR41CT1"
...
_SOURCE_FIELDS = frozenset(("file_sha256", "content_sha256"))
...
def _base_arrays(payload: Mapping[str, Any]) -> tuple[dict[str, np.ndarray], list[dict[str, Any]]]:
    ...
    for component_index, component in enumerate(components):
        ...
        arrays[prefix + "threshold_indices"] = np.asarray(thresholds, dtype="<u2")
...
def _patch_mismatches(
    source: R41DiagnosticPublicTreeProgramV8,
    compact: R41DiagnosticPublicTreeProgramV8,
...
def encode_program(
    ...
    """Encode one canonical v8 JSON program and prove audited action parity."""
    ...
    try:
        source = R41DiagnosticPublicTreeProgramV8.from_dict(source_payload)
    ...
    while True:
        ...
        compact = R41DiagnosticPublicTreeProgramV8.from_dict(
...
def _parse(raw: bytes) -> tuple[dict[str, Any], dict[str, np.ndarray]]:
    ...
    for component_index in range(COMPONENT_COUNT):
        ...
        exact_dtypes.update({
            ...
            prefix + "threshold_indices": np.dtype("<u2"),
...
def program_json_bytes(program: R41DiagnosticPublicTreeProgramV8) -> bytes:
    return (canonical(program.to_dict()) + "\n").encode("utf-8")


__all__ = [
    "VERSION", "MAGIC", "COMPRESSION", "QUANTIZATION", "TOPOLOGY",
    "THRESHOLDS", "MAX_COMPRESSED_BYTES", "CompactProgramError",
