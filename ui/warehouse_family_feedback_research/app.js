...
(() => {
  ...
  const ui={view:null,entry:"study",language:readStorage(localStorage,LANGUAGE_KEY)==="en"?"en":"zh",busy:false,error:null,replay:null,history:null,historyToken:0,historyLoading:null,refreshing:false,userNavigationEpoch:0,viewportEpoch:0,initialized:false,seen:new Set(),seenQueue:new Set(),seenTimer:null,questionnaireDirty:false,questionnairePreview:null,feedbackRun:null,answerSignature:null,motion:null,motionToken:0,demoPlaying:false,demoToken:0,pendingBeginTask1:false};
  ...
  function renderExplanations(view,locked){
    ...
    if(signature!==ui.answerSignature){ui.answerSignature=signature;$("answerList").replaceChildren(...answers.map(a=>{const box=document.createElement("article");box.className="answer";const q=document.createElement("strong");q.textContent=a.question;const body=document.createElement("p");body.textContent=["pending","queued","running"].includes(a.status)?tr("questionPending"):a.text || tr("answerUnavailable");const meta=document.createElement("small");meta.textContent=`${tr("ai")} · ${tr("answerFrame")} ${a.frame ?? "—"}`;box.append(q,body,meta);if(a.evidence_detail){const details=document.createElement("details");details.className="evidence-detail";const summary=document.createElement("summary");summary.textContent=tr("evidenceDetails");const evidence=document.createElement("pre");evidence.textContent=a.evidence_detail;details.append(summary,evidence);box.append(details);}return box;}));}
  ...
  async function execute(payload,retry=false){
    ...
    try{
      ...
      await animateConfirmedAction(before,next,submitted?.kind);adopt(next);accepted=next;
    }catch(e){
