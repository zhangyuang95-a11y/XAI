"""Retained-beta stage Actor admission and real observed197 RCPD collection.
...
``fit_feedback`` invokes the revised thirteen-candidate search; neither identifies the
...
VERSION = "warehouse-native-own-shutdown-stage-rcpd.v2"
...
def extraction_config():
    """Observed197 collection format with an explicitly revised RCPD search."""
    from . import warehouse_native_expanded_rcpd as expanded
    return {"version": VERSION, "data_version": DATA_VERSION,
        "actor_protocols": dict(admission.ACTOR_PROTOCOLS),
        "data_contract": data_api.extraction_config(),
        "fit_contract": expanded.contract(),
...
def required_bindings(protocol):
    """Actual stage fields plus an external parameter-semantic extraction anchor."""
    return tuple(dict.fromkeys((*admission.required_bindings(protocol), "actor_parameters_sha256")))


def validate_actor(actor, *, expected_bindings, protocol, allow_test_fixture=False, config=None):
    """Use the genuine frozen stage admission; this never grants capability."""
    config = config or collaborative_study_config()
    if type(actor) is not NumPyNativeActor:
        raise ValueError("Only an actual stage NumPy Actor may label extraction rows")
    if (type(protocol) is not dict or type(expected_bindings) is not dict
            or not set(required_bindings(protocol)) <= set(expected_bindings)):
        raise ValueError("Independent stage/condition/parameter bindings are required")
    if not compact._sha(expected_bindings["actor_parameters_sha256"]):
        raise ValueError("Invalid external parameter semantic hash")
    # Native continuation exports omit this semantic field, while genuine
    # feedback exports contain it. The outer source reader must verify the
    # former against the actual checkpoint; do not fabricate NP metadata.
    known = {k:v for k,v in expected_bindings.items()
             if k != "actor_parameters_sha256" or k in actor.metadata}
    admitted = admission.validate_actor(actor, known, protocol, config=config,
                                         allow_test_fixture=allow_test_fixture)
    return {"metadata": admitted["metadata"],
        "feature_names": list(admitted["metadata"]["feature_names"]),
        "configuration": admitted["configuration"],
        "actor_training_clock": admitted["cumulative_joint_steps"],
        "weights_sha256": admitted["weights_sha256"],
        "training_stage": admitted["training_stage"],
...
def _validate_data(data, fixture):
    ...
    version = bindings.get("experiment_version")
    stage = "feedback" if version == admission.feedback.VERSION else "continuation"
    expected_keys = set(REQUIRED_BINDINGS)
    if stage == "feedback": expected_keys.add("feedback_branch")
    if (not expected_keys <= set(bindings) or version not in admission.ACTOR_PROTOCOLS
            or metadata.get("experiment_version") != version or receipt.get("training_stage") != stage
            or bindings["shutdown_arm"] not in admission.previous.ARMS
            or not compact._same(bindings["own_shutdown_beta"], admission.previous.ARMS[bindings["shutdown_arm"]])
            or metadata.get("feature_names") != data["feature_names"]
            or metadata.get("test_fixture") is not fixture):
        raise ValueError("Actual retained-beta stage identity must remain attached to the data")
    if stage == "feedback":
        if (bindings.get("feedback_branch") not in ("control", "feedback")
                or metadata.get("feedback_enabled") is not (bindings["feedback_branch"] == "feedback")
                or metadata.get("actor_parameters_sha256") != bindings["actor_parameters_sha256"]):
            raise ValueError("Actual feedback condition/parameter binding differs")
    elif metadata.get("feedback_enabled") is not False or "feedback_branch" in metadata:
        raise ValueError("Continuation data cannot contain a feedback condition")
...
def merge_episodes(episodes, *, allow_test_fixture=False):
    ...
    for data in material:
        ...
        if not compact._same({k: first[k] for k in ("actor_metadata", "weights_sha256", "configuration", "training_stage")},
                             {k: other[k] for k in ("actor_metadata", "weights_sha256", "configuration", "training_stage")}):
    ...
    contract = {"metadata": first["actor_metadata"], "weights_sha256": first["weights_sha256"],
        ...
        "training_stage": first["training_stage"]}
...
def fit_feedback(train, selection, *, step, feature_names, feedback_config=None,
    ...
    """Real thirteen-candidate search; never grants independent explanations."""
    from . import warehouse_native_expanded_rcpd as expanded
    from dataclasses import asdict
    if feedback_config is not None and digest(asdict(feedback_config) if hasattr(feedback_config,'__dataclass_fields__') else feedback_config)!=digest(asdict(expanded.feedback_config())):
        raise ValueError('Stage extraction must use the explicit expanded search configuration')
    for data in (train, selection):
        _validate_data(data, allow_test_fixture)
    left, right = train["collector_receipt"], selection["collector_receipt"]
    if not compact._same({k: left[k] for k in ("actor_metadata", "weights_sha256", "configuration", "training_stage")},
                         {k: right[k] for k in ("actor_metadata", "weights_sha256", "configuration", "training_stage")}):
        raise ValueError("Train and selection evidence must retain the same frozen Actor")
    sources = execution_sources()
    result = expanded.fit(train, selection, step=step, feature_names=feature_names,
        prior_manager_state=prior_manager_state,
        allow_test_fixture=allow_test_fixture)
