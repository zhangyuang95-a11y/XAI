...
from backend.training import warehouse_r41_diagnostic_rcpd_v12_fit_selector as subject
...
def test_v12_selector_uses_replacement_registry_without_consumed_v9_targets():
    contract = subject.contract()
    assert subject.VERSION == "warehouse-r41-diagnostic-rcpd-v12-fit-selector.v1"
    assert subject.registry_api.VERSION == \
        "warehouse-r41-diagnostic-rcpd-v12-fresh-outer-registry.v1"
    assert subject.projection_api.VERSION == \
        "warehouse-r41-diagnostic-outer-hash-projection.v12"
    # V12 freezes one development-only configuration before fresh outer scoring.
    ...
    assert contract["fresh_v12_outer_labels_or_probabilities_read"] is False
    assert "v12 registry" in contract["consumed_v9_outer_input"]
    assert contract["prior_outer_projection_bindings"] == [
        "file_sha256", "content_sha256"]


def test_v12_candidate_lock_binds_prior_validation_projection():
...
def test_v12_selector_rejects_a_replacement_candidate_grid_before_file_access():
    wrong = "0" * 64
    with pytest.raises(ValueError, match="preregistered v12 candidate grid"):
        subject._resolve_snapshot(
            ...
            promoted_v11_closeout_path="missing",
            expected_promoted_v11_closeout_sha256=wrong,
            permanent_v11_outer_registry="missing",
            ...
            development_rows_path="missing", promoted_v11_rows_path="missing",
            candidate_grid_path="missing",
            expected_candidate_grid_sha256=wrong,
        )


def test_v12_validation_wins_mask_precedes_private_development_read(
...
def test_v12_promotes_v11_rows_with_public_precedence_before_target_read(
    tmp_path: Path, monkeypatch,
):
    base = _arrays(6)
    promoted = _arrays(4)
    promoted["observations"][:, 0] += 20
    # One public observation is shared across sources.  Promoted-v11 wins.
    ...
    combined, audit = subject.prepare_combined_development_rows(
        ...
        promoted_v11_path=promoted_path,
        expected_promoted_v11_sha256=file_hash(promoted_path),
...
def test_v12_prior_outer_projection_is_strict_and_label_free(tmp_path: Path):
...
def test_v12_validation_union_includes_prior_and_fresh_hashes():
...
def test_v12_validation_union_removes_burned_v10_1505_unique_1902_rows():
...
def test_v12_support_stratified_assignment_is_deterministic_and_supported():
...
def test_v12_failed_public_support_is_ineligible_without_calling_fit(monkeypatch):
    arrays = _arrays(6)
    families = {
...
def test_v12_fit_support_disagreement_fails_closed(monkeypatch):
    arrays = _arrays(6)
    families = {
        value.decode(): "family-a"
        for value in arrays["scene_fingerprints"]
...
def test_v12_grid_continues_after_an_ineligible_candidate(monkeypatch):
    calls: list[str] = []
...
def test_v12_all_ineligible_has_deterministic_complete_summary_digest():
    report = {
