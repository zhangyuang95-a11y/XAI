...
run(`render=async options=>renders.push(options);toast=message=>notices.push(message);report=e=>{throw e};api=(path,payload)=>new Promise(resolve=>calls.push({path,payload,resolve}));
 view={instance_id:'practice',revision:1,stage:'demo',mode:'preview',group:'A',can_ask:false,actions:['up','down','left','right','wait'],task_runs:[],public_help:[{id:'controls',en:'WASD and E operate the front station.',zh:'WASD 与 E 操作前方工位。'},{id:'menu',en:'Five dishes; deadlines 100, 140, 240, 280, 360.',zh:'五道菜，截止回合 100、140、240、280、360。'},{id:'score',en:'Serve +100; each turn −1; discard ingredient −5; discard dish −20.',zh:'上菜 +100，每回合 −1，丢配料 −5，丢成品 −20。'}],rule_metadata:{orders_per_task:5,score:{served:100,step:-1,single_component_discard:-5,combined_dish_discard:-20}},tutorial:{index:0,version:'practice-v1',playing:false,completed:false,progress:{},goal:{en:'Move, face a counter, and wait.',zh:'移动、面向工位并等待。'},highlights:['prep'],state:{domain:'kitchen',turn:0,max_turns:360,score:{task_score:0},orders:[{recipe:'egg_tomato',deadline:100,status:'pending'}],interaction:{available:false,label_en:'Face a station first.',label_zh:'先面向工位。'}}}};view.state=view.tutorial.state;`);
...
(async()=>{
 ...
 // Public help and menu numbers are provided by the saved scoring rules.
 const menu=run('kitchenMenu(view.state)');assert(menu.includes('Serve on time +100'));assert(menu.includes('Each turn -1'));assert(menu.includes('Trash ingredient -5'));assert(menu.includes('Trash dish -20'));assert(menu.includes('Due 100'));assert(!menu.includes('+30'));assert(menu.includes('Menu · 5 dishes'));
 ...
 run(`view.tutorial.index=5;view.tutorial.total_segments=6`);const last=run('demoPage()');assert(last.includes('Step 6 / 6'));assert(!last.includes('class="panel kitchen-menu"'));assert(last.includes('deadlines 100, 140'));assert(last.includes('Serve +100'));assert(last.includes('discard ingredient −5'));assert(last.includes('discard dish −20'));
