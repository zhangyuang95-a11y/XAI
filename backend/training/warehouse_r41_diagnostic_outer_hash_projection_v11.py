"""Publish the label-blind ordered observation-hash projection for v11 outer.

The fresh outer scene identities must already be frozen by the v11 registry.
...
VERSION = "warehouse-r41-diagnostic-outer-hash-projection.v11"
...
def _resolve_snapshot(
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    registry = _regular(registry_path, "fresh v11 outer registry")
    registry_report = _regular(registry_report_path, "fresh v11 outer registry report")
    ...
    expected = {
        ...
        "registry": _sha(expected_registry_sha256, "fresh v11 registry"),
        "registry_report": _sha(
            expected_registry_report_sha256, "fresh v11 registry report"),
    }
    ...
    snapshot = ImmutableInputSnapshot(
        ...
        prefix="warehouse-r41-v11-outer-projection-inputs-",
    )
...
def _validate_registry(
    *, paths: Mapping[str, Path], component_originals: Mapping[str, Path],
    ...
    registry = _strict_json(paths["registry"], "fresh v11 outer registry")
    report = _strict_json(paths["registry_report"], "fresh v11 outer registry report")
    ...
    if (registry.get("version") != registry_api.VERSION
        ...
        raise ValueError("Exact identity-frozen fresh v11 outer registry required")
    ...
    for index, (scene, identity) in enumerate(zip(scenes, identities)):
        if not isinstance(scene, Mapping) or not isinstance(identity, Mapping):
            raise ValueError("Fresh v11 outer scene identity differs")
        ...
        if (any(type(public[key]) is not int for key in ("batch_index", "seed"))
            ...
            raise ValueError("Fresh v11 outer materialised identity differs")
    ...
    recovery_statistics_valid = (
        ...
        and exclusion_counts.get("consumed_v10_outer_identities")
            == registry_api.FRESH_OUTER_SCENE_COUNT
        and exclusion_counts.get("union_candidate_identities_excluded") == 593
    ...
    if (not isinstance(bindings, Mapping)
        ...
        raise ValueError("Fresh v11 outer registry/report binding differs")
...
def _validate_registry_hash_screen_replay(
    ...
    """Reproduce the per-scene commitments used by the v11 hash screen."""
    statistics = registry.get("statistics")
    ...
    if (not isinstance(screen, Mapping) or not _content_valid(screen)
            ...
            or not isinstance(scenes, list) or len(scenes) != SCENE_COUNT):
        raise ValueError("Fresh v11 registry hash-screen audit differs")
    ...
    if len(observation_hashes) != len(scene_fingerprints):
        raise ValueError("Fresh v11 registry hash-screen replay differs")
    ...
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    try:
        ...
        arrays, _accounting, environment_steps = _replay_outer(
            ...
            progress_label="fresh_v11_outer_hash_projection")
        ...
        temporary = Path(tempfile.mkdtemp(
            prefix=".warehouse-r41-v11-outer-projection-", dir=destination.parent))
...
def read_saved_projection(
    *, projection_path: str | Path, receipt_path: str | Path,
    ...
    projection_file = _regular(projection_path, "v11 outer hash projection")
    receipt_file = _regular(receipt_path, "v11 outer projection receipt")
    if (file_hash(projection_file) != _sha(
            expected_projection_sha256, "v11 outer hash projection")
            or file_hash(receipt_file) != _sha(
                expected_receipt_sha256, "v11 outer projection receipt")):
        raise ValueError("Exact v11 outer projection artifact pair required")
    projection = validate_projection(_strict_json(
        projection_file, "v11 outer hash projection"))
    receipt = _strict_json(receipt_file, "v11 outer projection receipt")
