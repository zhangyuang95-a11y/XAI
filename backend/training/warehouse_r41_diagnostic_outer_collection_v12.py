"""Collect the full fresh v12 outer rows only after candidate lock.
...
VERSION = "warehouse-r41-diagnostic-outer-collection.v12"
SCHEMA_VERSION = "warehouse_r41_diagnostic_rcpd_v12_outer_collection_v1"
...
_REQUIRED_LOCK_BINDINGS = frozenset((
    ...
    "development_rows_sha256", "promoted_v11_rows_sha256",
    "promoted_v11_closeout_sha256", "candidate_grid_sha256", "program_sha256",
...
_DEVELOPMENT_FIELDS = frozenset((
    ...
    "failed_v11_outer_permanently_closed",
    "failed_v11_outer_reclassified_as_development", "validation_wins",
...
_COMBINED_VALIDATION_WINS_FIELDS = frozenset((
    "precedence", "base", "promoted_v11", "prior_unique_observations",
    "historical_unique_observations", "promoted_v11_unique_observations",
    "fresh_v12_unique_observations", "promoted_v11_projection_sha256",
...
_VALIDATION_HASH_INPUT_FIELDS = frozenset((
    ...
    "promoted_v11_closeout_sha256", "promoted_v11_rows_sha256",
    "promoted_v11_rows_semantic_sha256",
    "promoted_v11_unique_observations",
    "promoted_v11_observation_hashes_sha256",
...
def contract() -> dict[str, Any]:
    return {
        "version": VERSION,
        "population": "fresh v12 identity-frozen development outer only",
...
def _validate_static_rows(arrays: Mapping[str, np.ndarray]) -> None:
    if set(arrays) != rows_v7._FIELDS:
        raise ValueError("V12 outer row array schema differs")
    ...
    if (count <= 0
        ...
        raise ValueError("V12 outer row shapes or dtypes differ")
    ...
    if (not np.isfinite(observations).all()
        ...
        raise ValueError("V12 outer numeric or action-authority rows differ")
    ...
    if not np.array_equal(
        ...
        raise ValueError("V12 outer public-observation hashes differ")
    for name in ("observation_hashes", "scene_fingerprints",
                 "source_state_hashes"):
        try:
            values = np.char.decode(arrays[name], "ascii").astype(str)
        except UnicodeDecodeError as error:
            raise ValueError("V12 outer row text is not ASCII") from error
        if any(_HEX.fullmatch(str(value)) is None for value in values):
            raise ValueError("V12 outer row SHA-256 field differs")
...
def load_authenticated_rows(
    ...
    path = _regular(rows_path, "v12 outer rows", maximum=MAX_NPZ_BYTES)
    if file_hash(path) != _sha(expected_rows_sha256, "v12 outer rows"):
        raise ValueError("Exact v12 outer row bytes required")
    ...
    try:
        with zipfile.ZipFile(path) as archive:
            ...
            if (len(names) != len(set(names)) or set(names) != expected_members
                ...
                raise ValueError("V12 outer row archive contents differ")
        with np.load(path, allow_pickle=False) as archive:
            if set(archive.files) != rows_v7._FIELDS:
                raise ValueError("V12 outer row array schema differs")
            arrays = {name: archive[name].copy() for name in archive.files}
    except (OSError, ValueError, zipfile.BadZipFile, KeyError) as error:
        raise ValueError("V12 outer rows are not a safe strict NPZ") from error
    if file_hash(path) != expected_rows_sha256:
        raise RuntimeError("V12 outer rows changed during strict load")
...
def _validate_candidate_lock_shape(value: Mapping[str, Any]) -> dict[str, str]:
    ...
    if (set(value) != _LOCK_FIELDS
        ...
        raise ValueError("Exact locked v12 candidate contract required")
...
def _validate_validation_wins(
    ...
    if (set(value) != _VALIDATION_WINS_FIELDS or not _content_valid(value)
        ...
        raise ValueError("Locked v12 selector validation-wins trace differs")
    ...
    if (set(value) != _VALIDATION_HASH_INPUT_FIELDS
            ...
            or any(type(value.get(name)) is not str
                   ...
                   for name in (
                       ...
                       "prior_outer_observation_hashes_sha256",
                       "promoted_v11_closeout_sha256",
                       "promoted_v11_rows_sha256",
                       "promoted_v11_rows_semantic_sha256",
                       "promoted_v11_observation_hashes_sha256",
        ...
        raise ValueError("Locked v12 selector validation hash inputs differ")
...
def _validate_combined_validation_wins(
    ...
    if (set(value) != _COMBINED_VALIDATION_WINS_FIELDS
            ...
            or value.get("precedence")
                != "fresh-v12-validation > promoted-v11 > older-development"
            or not isinstance(value.get("base"), Mapping)
            or not isinstance(value.get("promoted_v11"), Mapping)
            or any(type(value.get(name)) is not int or value[name] <= 0
                   for name in (
                       "prior_unique_observations",
                       "promoted_v11_unique_observations",
                       "fresh_v12_unique_observations",
                       "combined_source_rows", "combined_retained_rows",
        ...
        raise ValueError("Locked v12 combined validation-wins trace differs")
    base = value["base"]
    promoted = value["promoted_v11"]
    ...
    if (base_semantic != base["retained_rows_semantic_sha256"]
            ...
            or promoted["fresh_outer_unique_observations"] < max(
                ...
                value["fresh_v12_unique_observations"])
            or promoted["fresh_outer_unique_observations"] > (
                value["historical_unique_observations"]
                + value["fresh_v12_unique_observations"])):
        raise ValueError("Locked v12 validation-wins source accounting differs")
    return value["retained_rows_semantic_sha256"]
...
def _validate_public_support_audit(value: Any, *, require_passed: bool) -> None:
    ...
    if (not isinstance(value, Mapping)
        ...
        raise ValueError("Locked v12 public replacement support audit differs")
        ...
        if (not isinstance(partition, Mapping)
                ...
                or any(type(partition.get(name)) is not str
                       ...
                       for name in ("scene_registry_sha256",
                                    "episode_registry_sha256"))):
            raise ValueError("Locked v12 public support partition differs")
    ...
    if computed is not require_passed:
        raise ValueError("Locked v12 public support result differs")
...
def _validate_fold_assignment_revision(development: Mapping[str, Any]) -> None:
    ...
    if (development.get("fold_assignment_version") != FOLD_ASSIGNMENT_VERSION
        ...
        raise ValueError("Locked v12 fold-assignment revision differs")
    ...
    if any(legacy_validation.get(name) != 1
           for name in ("rows", "scenes", "episodes")):
        raise ValueError("Locked v12 retired family-only failure differs")
    ...
    for salt, audit in repairs_by_salt.items():
        if (not isinstance(audit, Mapping)
            ...
            raise ValueError("Locked v12 fold-assignment repair audit differs")
        for label in ("support_before", "support_after"):
            ...
            if (not isinstance(support, Mapping)
                ...
                raise ValueError("Locked v12 fold support summary differs")
        ...
        for repair in audit["repairs"]:
            if (not isinstance(repair, Mapping)
                ...
                raise ValueError("Locked v12 same-family fold repair differs")
...
def _validate_selector_report(
    ...
    if (set(report) != _SELECTOR_FIELDS
        ...
        raise ValueError("Locked v12 selector report identity or bindings differ")
    ...
    if (not isinstance(contract_value, Mapping)
            ...
            or contract_value.get(
                ...
                is not True
            or contract_value.get(
                "fresh_v12_outer_labels_or_probabilities_read") is not False
            ...
            or development.get("failed_v11_outer_permanently_closed") is not True
            or development.get("failed_v11_outer_reclassified_as_development")
                is not True
            ...
            or validation_hash_inputs.get("promoted_v11_closeout_sha256")
                != bindings.get("promoted_v11_closeout_sha256")
            or validation_hash_inputs.get("promoted_v11_rows_sha256")
                != bindings.get("promoted_v11_rows_sha256")
            ...
            or validation_wins.get("promoted_v11_unique_observations")
                != validation_hash_inputs.get("promoted_v11_unique_observations")
            or validation_wins.get("fresh_v12_unique_observations")
            ...
            or boundary.get("runtime_action_override") is not False
            or boundary.get("formal_ready") is not False):
        raise ValueError("Locked v12 selector information boundary differs")
    ...
    if (type(development.get("retained_row_count")) is not int
        ...
        raise ValueError("Locked v12 selector development row count differs")
    ...
    if (not isinstance(grid, Mapping)
        ...
        raise ValueError("Locked v12 selector selection differs")
    ...
    if (not isinstance(candidate_reports, list) or not candidate_reports
        ...
        raise ValueError("Locked v12 selector candidate registry differs")
    ...
    for candidate, expected_config_sha in zip(candidate_reports, config_hashes):
        if (not isinstance(candidate, Mapping)
            ...
            raise ValueError("Locked v12 selector candidate report differs")
        ...
        if evaluation.get("status") == "ineligible_replacement_support":
            ...
            if (not isinstance(failure, Mapping)
                    ...
                    or not isinstance(partitions, Mapping)
                    or set(partitions) != {"fit", "validation"}):
                raise ValueError("Locked v12 selector support failure differs")
            for partition in partitions.values():
                if (not isinstance(partition, Mapping)
                        or set(partition) != _SUPPORT_PARTITION_FIELDS
                        or any(type(partition.get(name)) is not int
                        ...
                        or any(type(partition.get(name)) is not str
                               ...
                               for name in ("scene_registry_sha256",
                                            "episode_registry_sha256"))):
                    raise ValueError("Locked v12 selector support partition differs")
            all_supported = all(
            ...
            if ((failure["code"]
                ...
                raise ValueError("Locked v12 selector ineligible summary differs")
            normalized_candidates.append(candidate)
            continue
        if (evaluation.get("status") != "completed"
                or evaluation.get("failure") is not None
                or len(salts) != len(CV_SALTS)):
            raise ValueError("Locked v12 selector completed evaluation differs")
        salt_passes = []
        salt_minima = []
        complexities = []
        assignment_hashes = []
        for salt_report, expected_salt in zip(salts, CV_SALTS):
            ...
            if (not isinstance(salt_report, Mapping)
                ...
                raise ValueError("Locked v12 selector two-salt CV trace differs")
            ...
            for fold in folds:
                ...
                if (set(fold) != _FOLD_REPORT_FIELDS
                    ...
                    raise ValueError("Locked v12 selector fold trace differs")
                if (fold["fit_rows"] + fold["validation_rows"]
                    ...
                    raise ValueError("Locked v12 selector fold partition differs")
                ...
                _gate_matches(
                    ...
                    label="Locked v12 selector informational fold",
            ...
            if (validation_row_total != development.get("retained_row_count")
                    or validation_scene_total
                        != development.get("retained_scene_count")):
                raise ValueError("Locked v12 selector fold coverage differs")
            salt_passes.append(_gate_matches(
                salt_report.get("aggregate_metrics"),
                salt_report.get("aggregate_gate"),
                label="Locked v12 selector aggregate", require_passed=False))
        ...
        robust = _finite_unit(
            ...
            "Locked v12 selector robust minimum")
        expected_capacity = {
        ...
        if (candidate.get("both_salts_pass_all_aggregate_gates") is not both_pass
            ...
            raise ValueError("Locked v12 selector candidate summary differs")
        normalized_candidates.append(candidate)
    ...
    if candidate_grid_file_sha256 != bindings["candidate_grid_sha256"]:
        raise ValueError("Locked v12 selector candidate-grid binding differs")
    ...
    if not eligible:
        raise ValueError("Locked v12 selector has no eligible candidate")
    chosen = eligible[0]
    expected_selection = {
    ...
    if (dict(selection) != expected_selection
        ...
        raise ValueError("Locked v12 selector chosen candidate differs")

    final_fit = report.get("final_fit")
    if (not isinstance(final_fit, Mapping)
            or set(final_fit) != _FINAL_FIT_FIELDS):
        raise ValueError("Locked v12 selector full-development refit differs")
    _gate_matches(final_fit.get("development_metrics"),
                  final_fit.get("development_gate"),
                  label="Locked v12 selector full-development", require_passed=True)
    ...
    expected_final_binding_sha256 = digest({
        ...
        "promoted_v11_rows_sha256": bindings["promoted_v11_rows_sha256"],
        "promoted_v11_closeout_sha256": bindings[
            "promoted_v11_closeout_sha256"],
...
def authenticate_locked_candidate_selector(
    *, lock: Mapping[str, Any], selector_report_path: str | Path,
    ...
    path = _regular(selector_report_path, "locked v12 selector report")
    expected = _sha(expected_selector_report_sha256, "locked selector report")
    raw = path.read_bytes()
    if sha256(raw).hexdigest() != expected:
        raise ValueError("Exact locked v12 selector report bytes required")
    report = _strict_json_bytes(raw, "locked v12 selector report")
    if file_hash(path) != expected:
        raise RuntimeError("Locked v12 selector report changed during authentication")
...
def _validate_candidate_bindings(
    *, lock: Mapping[str, Any], paths: Mapping[str, Path],
    ...
    actual = {
        ...
        "promoted_v11_closeout_sha256": file_hash(
        ...
        "promoted_v11_rows_sha256": file_hash(paths["promoted_v11_rows"]),
...
def _resolve_snapshot(
    *, lock: Mapping[str, Any], actor_path: str | Path,
    ...
    promoted_v11_rows_path: str | Path,
    ...
    promoted_closeout_original = _regular(
        promoted_v11_closeout_path, "consumed-v11 outer closeout")
    if file_hash(promoted_closeout_original) != _sha(
            expected_promoted_v11_closeout_sha256,
            "consumed-v11 outer closeout"):
        raise ValueError("Exact consumed-v11 outer closeout bytes required")
    ...
    promoted_rows_sha = promoted_closeout["consumed_outer"]["rows_sha256"]
    originals = {
        ...
        "candidate_lock": _regular(candidate_lock_path, "v12 candidate lock"),
        "failure_closeout": _regular(
            failure_closeout_path, "failed outer closeout"),
        "promoted_v11_closeout": promoted_closeout_original,
        "development_rows": _regular(
            development_rows_path, "locked development rows", maximum=MAX_NPZ_BYTES),
        "promoted_v11_rows": _regular(
            promoted_v11_rows_path, "promoted failed-v11 outer rows",
            maximum=MAX_NPZ_BYTES),
        "program": _regular(program_path, "locked v12 program"),
    ...
    expected = {
        ...
        "promoted_v11_closeout": bindings["promoted_v11_closeout_sha256"],
        "development_rows": bindings["development_rows_sha256"],
        "promoted_v11_rows": bindings["promoted_v11_rows_sha256"],
    ...
    snapshot = ImmutableInputSnapshot(
        ...
        maximum_bytes={"actor": MAX_NPZ_BYTES, "designation_actor": MAX_NPZ_BYTES,
                       ...
                       "promoted_v11_rows": MAX_NPZ_BYTES},
        prefix="warehouse-r41-v12-outer-collection-inputs-",
    )
    if (bindings["promoted_v11_closeout_sha256"]
            != expected_promoted_v11_closeout_sha256
            or bindings["promoted_v11_rows_sha256"] != promoted_rows_sha):
        raise ValueError("Candidate lock promoted-v11 bindings differ")
    ...
    *, paths: Mapping[str, Path], lock: Mapping[str, Any],
    ...
    value: dict[str, Any] = {
        ...
        "bindings": {
            ...
            "promoted_v11_closeout_sha256": file_hash(
                paths["promoted_v11_closeout"]),
            "promoted_v11_rows_sha256": file_hash(
        ...
        "information_boundary": {
            ...
            "promoted_v11_closeout_authenticated_before_outer_replay": True,
...
def build(
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    candidate_file = _regular(candidate_lock_path, "v12 candidate lock")
    if file_hash(candidate_file) != _sha(
            expected_candidate_lock_sha256, "v12 candidate lock"):
        raise ValueError("Exact v12 candidate lock bytes required")
    candidate_lock = _strict_json(candidate_file, "v12 candidate lock")
    ...
    try:
        ...
        lock_fd = os.open(
            ...
            | getattr(os, "O_NOFOLLOW", 0), 0o600)
        locked = _strict_json(paths["candidate_lock"], "snapshotted v12 candidate lock")
        _validate_candidate_bindings(lock=locked, paths=paths)
        selector_report = _strict_json(
            paths["selector_report"], "locked v12 selector report")
        ...
        promoted_outer = promoted_closeout["consumed_outer"]
        ...
        if (not set(promoted_hashes) <= set(prior_hashes)
                or file_hash(paths["promoted_v11_rows"])
                    != promoted_outer["rows_sha256"]):
            raise ValueError("Promoted-v11 closeout evidence differs")
        ...
        actual_validation_hash_inputs = {
            ...
            "promoted_v11_closeout_sha256": file_hash(
                paths["promoted_v11_closeout"]),
            "promoted_v11_rows_sha256": promoted_outer["rows_sha256"],
            "promoted_v11_rows_semantic_sha256": promoted_outer[
                "rows_semantic_sha256"],
            "promoted_v11_unique_observations": len(promoted_hashes),
            "promoted_v11_observation_hashes_sha256": digest(
                promoted_hashes),
        ...
        arrays, accounting, environment_steps = projection_api._replay_outer(
            ...
            progress_label="fresh_v12_outer_full_collection")
        ...
        temporary = Path(tempfile.mkdtemp(
            prefix=".warehouse-r41-v12-outer-collection-", dir=destination.parent))
        ...
        if (set(reopened) != set(arrays)
            ...
            raise RuntimeError("Staged v12 outer row archive changed")
        ...
        snapshot.verify()
        promoted_closeout_api.read_saved_closeout(
            paths["promoted_v11_closeout"],
            expected_closeout_sha256=expected_promoted_v11_closeout_sha256,
            permanent_attempt_registry=permanent_v11_outer_registry)
...
def authenticate_saved_collection(
    *, report_path: str | Path, rows_path: str | Path,
    ...
    report_file = _regular(report_path, "v12 outer collection report")
    rows_file = _regular(rows_path, "v12 outer rows", maximum=MAX_NPZ_BYTES)
    projection_file = _regular(
        projection_copy_path, "v12 copied outer hash projection")
    expected = {
        "report": _sha(expected_report_sha256, "v12 outer collection report"),
        "rows": _sha(expected_rows_sha256, "v12 outer rows"),
        "projection": _sha(
    ...
    if (file_hash(report_file) != expected["report"]
        ...
        raise ValueError("Exact v12 outer collection artifact set required")
    report = _strict_json(report_file, "v12 outer collection report")
    sources = producer_sources()
    ...
    if (report.get("version") != VERSION
            ...
            or boundary.get(
                ...
                is not True
            or boundary.get(
                "promoted_v11_closeout_authenticated_before_outer_replay")
                is not True
            ...
            or boundary.get("runtime_action_override") is not False
            or report.get("formal_ready") is not False):
        raise ValueError("V12 outer collection receipt differs")
    if file_hash(report_file) != expected["report"]:
        raise RuntimeError("V12 outer collection report changed during authentication")
    return deepcopy(report)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--promoted-v11-closeout", required=True)
    parser.add_argument(
        "--expected-promoted-v11-closeout-sha256", required=True)
    parser.add_argument("--permanent-v11-outer-registry", required=True)
    parser.add_argument("--promoted-v11-rows", required=True)
    ...
    report = build(
        ...
        permanent_v11_outer_registry=args.permanent_v11_outer_registry,
