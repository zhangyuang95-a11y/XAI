...
(() => {
  ...
  function paint(){
    ...
    const stages=["consent","practice","task1","task2","questionnaire","completed"],workflowStage=p==="registration"?"consent":p==="task1_complete"?"task1":p,index=stages.indexOf(workflowStage);
  ...
  function renderExplanations(view,locked){
    ...
    if(signature!==ui.answerSignature){ui.answerSignature=signature;$("answerList").replaceChildren(...answers.map(a=>{const box=document.createElement("article");box.className="answer";const q=document.createElement("strong");q.textContent=a.question;const body=document.createElement("p");body.textContent=["pending","queued","running"].includes(a.status)?tr("questionPending"):a.text || tr("answerUnavailable");const meta=document.createElement("small");meta.textContent=`${tr("ai")} · ${tr("answerFrame")} ${a.frame ?? "—"}`;box.append(q,body,meta);if(a.evidence_detail || a.sources.length){const details=document.createElement("details");details.className="evidence-detail";const summary=document.createElement("summary");summary.textContent=tr("evidenceDetails");const evidence=document.createElement("pre");evidence.textContent=[a.evidence_detail,a.sources.length?`${tr("source")}: ${a.sources.join("; ")}`:""].filter(Boolean).join("\n");details.append(summary,evidence);box.append(details);}return box;}));}
