...
from backend.training import warehouse_r41_diagnostic_rcpd_v13_outer_once as subject
...
def _fixture(tmp_path: Path, monkeypatch, *, fidelity: float = 0.95):
    ...
    promoted_arrays = _arrays([_fp("promoted-scene")], prefix="promoted")
    promoted_rows = tmp_path / "combined_promoted_rows.npz"
    _write_rows(promoted_rows, promoted_arrays)

    promoted_hashes = sorted(set(np.char.decode(
        promoted_arrays["observation_hashes"], "ascii").astype(str).tolist()))
    prior_hashes = promoted_hashes
    ...
    promoted_closeout = _content({
        "combined_promoted_development": {
            "rows_sha256": file_hash(promoted_rows),
            "rows_semantic_sha256": _fp("promoted-rows-semantic"),
            "observation_hash_projection": {
                "outer_observation_hashes": promoted_hashes,
                "outer_observation_hashes_sha256": digest(promoted_hashes),
            },
        },
    })
    promoted_closeout_path = tmp_path / "promotion_closeout.json"
    _write_json(promoted_closeout_path, promoted_closeout)
    files = {}
    ...
    bindings = {
        ...
        "promotion_closeout_sha256": file_hash(promoted_closeout_path),
        ...
        "combined_promoted_rows_sha256": file_hash(promoted_rows),
    ...
    historical = sorted(set(prior_hashes) - set(promoted_hashes))

    def mask_audit(values: list[str], exclusions: list[str]) -> tuple[list[str], dict]:
        keep = ~np.isin(
    ...
    base_retained, base_audit = mask_audit(development_hashes, outer_unique)
    promoted_exclusions = sorted(set(historical) | set(fresh_unique))
    promoted_retained, promoted_audit = mask_audit(
        promoted_ordered, promoted_exclusions)
    validation_wins = {
        "precedence": "fresh-v13-validation > combined-promoted-development > older-development",
        "base": deepcopy(base_audit),
        "combined_promoted": deepcopy(promoted_audit),
        "prior_unique_observations": len(prior_hashes),
        ...
        "combined_source_rows": len(development_hashes) + len(promoted_ordered),
        "combined_retained_rows": len(base_retained) + len(promoted_retained),
        "combined_retained_scene_count": 2,
        "cross_component_observation_overlap": 0,
        "retained_fresh_outer_observation_overlap": 0,
        "retained_rows_semantic_sha256": _fp("combined-retained-semantic"),
    ...
    validation_hash_inputs = {
        ...
        "promotion_closeout_sha256": file_hash(promoted_closeout_path),
        "combined_promoted_rows_sha256": file_hash(promoted_rows),
        "combined_promoted_rows_semantic_sha256": promoted_closeout[
            "combined_promoted_development"]["rows_semantic_sha256"],
        "combined_promoted_unique_observations": len(promoted_hashes),
        "combined_promoted_observation_hashes_sha256": digest(promoted_hashes),
    ...
    args = {
        ...
        "promotion_closeout_path": promoted_closeout_path,
        "expected_promotion_closeout_sha256": file_hash(
        ...
        "combined_promoted_rows_path": promoted_rows,
...
def test_source_closure_has_no_protected_final_or_holdout_module():
    ...
    assert "backend/training/warehouse_r41_diagnostic_rcpd_v13_outer_once.py" in sources
