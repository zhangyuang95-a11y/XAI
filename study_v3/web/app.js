...
function sampleTime(){const now=performance.now(),seconds=Math.min(5,(now-timeSince)/1000);timeSince=now;if(!view?.stage?.startsWith('task')||view.state?.terminal||document.hidden||asking)return;const kind=replay?'replay':(pendingAutomatic()||$('questionInput')===document.activeElement||document.querySelector('details[open]'))?'reading':'active';timeBuckets[kind]+=seconds;}
...
function heading(){return `${domainNavigation()}${progress()}<div class="study-title"><div><div class="eyebrow">${title()}</div><h1>${view.stage.startsWith('task')?'Task '+view.stage.slice(-1):stageLabel(view.stage)}</h1><p>${view.stage==='task1'?tr('baseline'):view.stage==='task2'?(view.can_ask?task2Introduction():tr('control')):view.stage==='task3'?tr('transfer'):''}</p></div><div class="phase-tag"><i class="dot"></i>${view.mode==='pilot'?tr('pilot'):tr('preview')}${view.group?' · '+view.group:''}</div></div>`;}
...
function observeAutomatic(){
 automaticObserver?.disconnect();automaticObserver=null;
...
async function render({animate=false,displayState=null}={}){
 if(pendingAutomatic())replay=null;
...
async function loadFrame(run,turn){
 stopHeld();if(pendingAutomatic()||busy||animationPending||asking||!view?.instance_id)return;
...
document.addEventListener('keydown',e=>{
 if(pendingAutomatic()){if(e.repeat&&[' ','Enter'].includes(e.key))e.preventDefault();return;}
