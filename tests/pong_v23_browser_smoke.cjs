...
(async () => {
  ...
  try {
    ...
    assert.match(await missing.textContent('#setupError'), /nn_model\.json 不存在/);
    assert.equal(await missing.locator('#retryLoad').isVisible(), true);
    assert.equal(await missing.locator('#game').isVisible(), false);
    await missing.unroute('**/nn_model.json');
    await missing.click('#retryLoad');
    await missing.waitForFunction(() => document.querySelector('#setupError').textContent.includes('模型已加载'));
    await missing.click('#start');
    await missing.waitForFunction(() => document.querySelector('#game').hidden === false);
    await missing.close();
    const disconnected = await browser.newPage();
    await disconnected.route('**/nn_model.json', route => route.abort());
    await disconnected.goto(url);
    await disconnected.click('#start');
    await disconnected.waitForFunction(() => document.querySelector('#setupError').textContent.includes('无法连接模型服务'));
    assert.equal(await disconnected.locator('#retryLoad').isVisible(), true);
    ...
    await corrupted.route('**/nn_model.json', route => route.fulfill({ status: 200, contentType: 'application/json', body: '{broken' }));
    await corrupted.goto(url);
    await corrupted.click('#start');
    await corrupted.waitForFunction(() => document.querySelector('#setupError').textContent.includes('不是有效 JSON'));
    assert.equal(await corrupted.locator('#retryLoad').isVisible(), true);
    await corrupted.close();
    const optionalProgram = await browser.newPage();
    await optionalProgram.route('**/program.json', route => route.fulfill({ status: 404, body: 'missing' }));
    await optionalProgram.goto(url);
    await optionalProgram.click('#start');
    await optionalProgram.waitForFunction(() => document.querySelector('#game').hidden === false);
    assert.equal(await optionalProgram.evaluate(() => nnProgram), null);
    await optionalProgram.close();
    const directFile = await browser.newPage();
    await directFile.goto(`file://${require('node:path').resolve('domains/pong/web/index.html')}`);
    assert.match(await directFile.textContent('#setupError'), /直接打开的本地文件/);
    assert.equal(await directFile.locator('#start').isDisabled(), true);
    assert.equal(await directFile.locator('#setupError a').getAttribute('href'), 'https://policylens-warehouse-study.onrender.com/pong/');
