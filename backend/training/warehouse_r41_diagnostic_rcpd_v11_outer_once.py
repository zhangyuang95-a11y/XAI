"""Irrevocably score one locked v11 candidate program on one fresh development outer.
...
VERSION = "warehouse-r41-diagnostic-rcpd-v11-outer-once.v1"
LOCK_SCHEMA = "warehouse_r41_diagnostic_rcpd_v11_candidate_lock_v1"
COLLECTION_SCHEMA = "warehouse_r41_diagnostic_rcpd_v11_outer_collection_v1"
LOCK_STATUS = "locked"
...
def _candidate_lock(
    ...
    lock_path, _, value = _strict_json(
        path, "v11 candidate lock", expected_sha256=expected_sha256)
    ...
    if (value.get("schema_version") != LOCK_SCHEMA
        ...
        raise ValueError("V11 candidate lock semantics differ")
    try:
        strict_bindings = collection_api._validate_candidate_lock_shape(value)
    except ValueError as error:
        raise ValueError("V11 candidate lock semantics differ") from error
...
def _verify_lock_files(bindings: Mapping[str, str], **paths: str | Path) -> dict[str, Path]:
    ...
    if set(paths) != set(expected_keys):
        raise ValueError("Complete v11 candidate-lock file set required")
    result = {}
...
def _validate_candidate_source_closure(
    ...
    if closure is None:
        raise ValueError(
            "V11 candidate source closure mapping is required for runtime authentication")
    if (not closure
    ...
    if missing or changed:
        ...
        raise ValueError(
            "V11 candidate runtime source closure differs: " + "; ".join(details))
...
def _registry_bundle(
    ...
    *, bindings: Mapping[str, str],
    ...
    value = _strict_json_bytes(
        registry_path.read_bytes(), "fresh v11 outer registry")
    report = _strict_json_bytes(
        registry_report_path.read_bytes(), "fresh v11 outer registry report")
    ...
    if (set(value) != _REGISTRY_FIELDS
        ...
        raise ValueError("Fresh v11 outer registry semantics differ")
    public_identities = []
    ...
    for index, (scene, identity) in enumerate(zip(scenes, identities)):
        if (not isinstance(scene, Mapping) or not isinstance(identity, Mapping)
            ...
            raise ValueError("Fresh v11 outer registry identities differ")
    ...
    if (set(report) != _REGISTRY_REPORT_FIELDS
        ...
        raise ValueError("Fresh v11 outer registry/report identity differs")
    if (file_hash(registry_path) != bindings["fresh_outer_registry_sha256"]
        ...
        raise RuntimeError("Fresh v11 outer registry bundle changed during read")
