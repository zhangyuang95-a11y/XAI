...
## 当前 Render 在线试玩（2026-09-10）

公网入口：[https://policylens-warehouse-study.onrender.com](https://policylens-warehouse-study.onrender.com)。该服务从 XAI 主库启动，使用冻结 Actor `309b6e53…`、12 个固定场景、冻结 RCPD 程序和 8 道预测题；线上 NumPy 推理与本地已验收运行时保持一致，不导入训练依赖。AI 的五动作由冻结两层 MLP 的确定性输出决定，提交后不经过协调规则、动作重选、掩码或脚本覆盖；环境只负责解析物理碰撞。

已确认的联合动作采用 380 ms 连续 Canvas 动画：两台机器人同步从真实起点插值到真实终点；等待、阻塞、刷新和回放不会产生假移动。在线流程保留用户 ID、显式同意、A/B × XY/YX 区组、练习、Task 1 三局、Task 2 三局和问卷。

当前免费 Render 实例没有持久磁盘，服务重启或重新部署后 SQLite 记录可能丢失，因此此地址用于在线技术试玩，`formal_ready=false`，不能直接作为持久化正式实验。发布哈希、复现命令和验证边界见 [Render 部署说明](docs/warehouse_alignment_render_deployment_20260910.md)。

## 当前本地 A/B 预实验版本（2026-09-10）

入口：[http://127.0.0.1:8012/](http://127.0.0.1:8012/)。冻结 NN 累计训练 395 万步，五个动作原样提交，无覆盖、掩码或程序接管；独立能力门槛通过。

解释系统已按修订后的证据分工通过技术验收：RCPD 树的普通动作一致率为 92.371%，关键协作情境均高于 89%；反事实由同一冻结 NN 隔离重放，动作及有效方向一致率均为 100%。旧的仅树反事实诊断仍只有 57.209%，作为非门控失败保留。中英文自由问答 480 个案例全部通过独立复核。

A/B、六局、冻结问卷和持久化已接入；r2 在绑定用户 ID 前显示完整参与说明并同时执行前端与服务端同意门控。当前明确是本地预实验，`formal_ready=false`，尚无真人因果效果结论。完整模型、解释边界、清单哈希、启动命令及限制见[最终本地预实验报告](docs/warehouse_alignment_final_local_pilot_20260910.md)。[8011 盲测候选](http://127.0.0.1:8011/)与历史数据保留。Render 使用由本地发布导出的轻量、哈希绑定衍生包；完整验收证据仍只在本机。以下旧模型消融和训练说明属于原版本，不能作为此版本的执行或验收结论。
...
新增同一 v68 权重的四层消融入口：原协调执行、仅取消输出后接管、再取消规则动作掩码、自主神经头消融。最后一项还剥离人工 logit 评分及规则目标输入；这是改变执行方式与输入分布的诊断，未重新训练，不继承原模型验收或解释资格。线上入口和原实验记录保持不变。

```bash
python3 -B -m ui.warehouse_autonomy_server --port 8000
```

打开 <http://127.0.0.1:8000/>。运行边界、原始对照结果、测试和复现命令见 [自主决策审计](docs/warehouse_autonomy.md)。以下历史“直接执行”等产物描述，不能替代上述当前代码执行链路审计。
...
- 机器人冲突时双方本步均等待，计一次碰撞但不终止。旧 v68 执行链路是 Actor 提议后，再由 `runtime_coordination` 选择最终动作；人机模式优先规避玩家各合法动作下的潜在冲突。Actor 输入还含规则动作掩码，forward 内含人工评分项。因此旧版本应描述为神经提议与规则协调的组合，不能称为纯神经动作直接执行。撞墙、货架阻塞、同格争抢、交换位置和进入未离开的队友位置，仍由环境动力学解析。
