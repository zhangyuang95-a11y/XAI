...
_evaluation_summary = evaluation_summary
...
def evaluate_policy(
    ...
    *,
    ...
    unexplained_reversal_steps = 0
    short_cycle_steps = 0
    invalid_goal_switch_steps = 0
    explanation_fact_failures = 0
    simultaneous_semantics_violations = 0
    efficiency = EfficiencyMetrics()
    ...
    for episode in range(episodes):
        ...
        episode_temporal_violations = 0
        episode_explanation_failures = 0
        episode_simultaneous_violations = 0
        episode_unnecessary_waits = 0
        minimum_battery_seen = min(
        ...
        while True:
            ...
            reversal_count = len(info.get("unexplained_reversal_agents", ()))
            cycle_count = len(info.get("short_cycle_agents", ()))
            switch_count = len(info.get("invalid_goal_switch_agents", ()))
            unexplained_reversal_steps += reversal_count
            short_cycle_steps += cycle_count
            invalid_goal_switch_steps += switch_count
            episode_temporal_violations += (
                reversal_count + cycle_count + switch_count
            )
            trace_failures = len(
                info.get("decision_trace", {}).get(
                    "fact_validation_failures",
                    (),
                )
            )
            explanation_fact_failures += trace_failures
            episode_explanation_failures += trace_failures
            audit = info.get("decision_audit", {})
            semantic_failures = int(
                not bool(audit.get("same_pre_move_state", False))
                or int(audit.get("environment_step_calls", 0)) != 1
                or not bool(
                    info.get("decision_trace", {}).get(
                        "same_frozen_state_for_all_agents",
                        False,
                    )
                )
            )
            simultaneous_semantics_violations += semantic_failures
            episode_simultaneous_violations += semantic_failures
            ...
            episode_unnecessary_waits += len(
                set(info.get("avoidable_wait_agents", ()))
                - set(participant_override_ids)
            )
            episode_steps += 1
            minimum_battery_seen = min(
                minimum_battery_seen,
        ...
        if episode_temporal_violations > 0:
            failure_seeds["temporal_consistency"].append(episode_seed)
        if episode_explanation_failures > 0:
            failure_seeds["explanation_fact"].append(episode_seed)
        if episode_simultaneous_violations > 0:
            failure_seeds["simultaneous_semantics"].append(episode_seed)
        if episode_unnecessary_waits > 0:
            failure_seeds["unnecessary_wait"].append(episode_seed)
    return evaluation_summary(
        ...
        unexplained_reversal_steps=unexplained_reversal_steps,
        short_cycle_steps=short_cycle_steps,
        invalid_goal_switch_steps=invalid_goal_switch_steps,
        explanation_fact_failures=explanation_fact_failures,
