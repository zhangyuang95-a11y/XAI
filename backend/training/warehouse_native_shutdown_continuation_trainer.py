"""Finite unchanged-learning continuation of a genuine own-shutdown trainer.

The caller verifies source checkpoint bytes and the registered selection before
supplying an actual loaded source. This module retains beta, alpha, parameters,
Adam states, public-history environments, in-flight episodes and owned RNGs.
It never opens a checkpoint file or grants sampling/qualification. Each finite
cycle has its own explicit protocol, source lineage and zeroed local counters.
"""
...
from .warehouse_native_shutdown_trainer import (OwnShutdownTrainer, VERSION as SHUTDOWN_VERSION,
    SHUTDOWN_REWARD_VERSION, BETAS, execution_sources as parent_sources)
from .warehouse_native_credit_trainer import CREDIT_REWARD_VERSION
...
VERSION = 'warehouse-native-own-shutdown-continuation-trainer.v1'
PROTOCOL_VERSION = 'warehouse-native-own-shutdown-continuation-protocol.v1'
...
def _source(source, fixture):
    if type(source) not in (OwnShutdownTrainer, ShutdownContinuationTrainer):
        raise ValueError('Only genuine OwnShutdownTrainer or ShutdownContinuationTrainer sources are supported')
    if (type(fixture) is not bool or source.test_fixture is not fixture or source.mode != 'observed'
            or source.branch != 'own_credit' or source.alpha != .5 or source.feedback_enabled
            or source.shutdown_arm not in BETAS or source.beta != BETAS[source.shutdown_arm]):
        raise ValueError('Source observation, reward arm or fixture scope differs')
    state = source.state_dict()
    if not fixture and (source.joint_steps <= 0 or source.joint_steps not in source.protocol['evaluation']['checkpoints_ppo_steps']):
        raise ValueError('Production source must be a completed registered validation endpoint')
    return state
...
def _protocol(source, state, *, checkpoint_sha, state_sha, ppo_cap, cycle_id, evaluation_checkpoints, fixture):
    ...
    if (type(endpoints) is not list or not endpoints or any(type(x) is not int or x <= 0
    ...
    identity = {'trainer_version': state['version'], 'branch': source.branch,
        'shutdown_arm': source.shutdown_arm, 'own_shutdown_beta': source.beta,
        'own_shutdown_reward_version': SHUTDOWN_REWARD_VERSION,
    ...
    return {'version': PROTOCOL_VERSION, 'test_fixture': fixture, 'cycle_id': cycle_id,
        'branch': source.branch, 'shutdown_arm': source.shutdown_arm, 'own_shutdown_beta': source.beta,
        'own_shutdown_reward_version': SHUTDOWN_REWARD_VERSION,
        'public_feedback_version': OBSERVER_VERSION, 'public_feedback_mode': 'observed',
        'credit_reward_version': CREDIT_REWARD_VERSION, 'delivery_credit_alpha': source.alpha,
        **{k: copy.deepcopy(source.protocol[k]) for k in ('training', 'reward', 'collision_training_cost', 'seed')},
        ...
        'evaluation': {'checkpoints_ppo_steps': endpoints, 'partners': ['skilled', 'assertive', 'noisy'],
            ...
            'training_credit_in_evaluation': False, 'training_shutdown_in_evaluation': False},
...
class ShutdownContinuationTrainer(OwnShutdownTrainer):
    """Independent continuation; the genuine source class validates the fork."""
    def __init__(self, protocol, source_trainer, *, expected_source_state_sha256,
        ...
        if initialization_sha256(protocol) != initialization_sha256(expected): raise ValueError("Continuation must inherit the exact source learning contract")
