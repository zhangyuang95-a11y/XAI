...
from backend.training import warehouse_r41_diagnostic_final_once_v14 as final_v14
...
from scripts import build_warehouse_r41_diagnostic_admission_v11 as admission_cli
...
from scripts import preflight_warehouse_r41_diagnostic_render_v11 as preflight
...
def test_v11_is_append_only_adapter_for_v13_evidence_and_v9_runtime():
    assert admission.VERSION == "warehouse-r41-diagnostic-admission.v11"
    assert admission.outer_api.VERSION.startswith(
        "warehouse-r41-diagnostic-rcpd-v13-")
    assert admission.final_api.VERSION == "warehouse-r41-diagnostic-final-once.v14"
    ...
    assert release.assemble_from_v11_admitted_components is not None
...
def test_legacy_v9_assembler_stays_callable_and_outside_frozen_final_closure():
    ...
    adapter = inspect.signature(
        release.assemble_from_v11_admitted_components).parameters
    ...
    _path, materializer_sources = final_v14._official_materializer_binding()
    assert "ui/warehouse_alignment_r41_diagnostic_release_v9.py" not in (
        materializer_sources)


def test_v11_admission_binds_v13_closeout_and_projection_parity():
    required = {
...
def test_v11_cli_and_preflight_use_v13_promotion_boundary():
    admission_options = {
...
def test_release_source_closure_contains_v11_boundary():
...
def test_v11_receipt_requires_exact_v11_admission_parent():
    admission_value = {
        "content_sha256": "b" * 64,
    ...
    for field, replacement in (
        ...
        with pytest.raises(ValueError, match="exact v11 admission"):
            receipt._require_exact_admission_parent(
                {"parent": forged}, admission_value, admission_sha)


def test_v11_preflight_rejects_render_start_without_explicit_release_module(
        tmp_path: Path):
    ...
    candidate = tmp_path / "render.v11.yaml"
