"""Select and lock one public-tree candidate for a replacement v13 fresh outer.

The selector treats every permanently promoted outer through the burned v12
final as development data only after the permanent closeouts are authenticated.  The
replacement v13 outer contributes a label-blind ordered observation-hash
projection.  Those hashes win over both development archives before any
action or probability member is opened.  When the two development sources
share a public observation, the newly combined promoted row wins deterministically
and the older row is removed before either source's targets are opened.
...
from backend.training import warehouse_r41_diagnostic_outer_failure_closeout_v9 as base_closeout_api
from backend.training import warehouse_r41_diagnostic_outer_hash_projection_v13 as projection_api
from backend.training import warehouse_r41_diagnostic_final_attempt_closeout_public_v13 as promotion_closeout_api
...
from backend.training import warehouse_r41_diagnostic_rcpd_v12_fit as fit_api
...
VERSION = "warehouse-r41-diagnostic-rcpd-v13-fit-selector.v1"
...
LEGACY_FOLD_ASSIGNMENT_VERSION = (
    "warehouse-r41-diagnostic-rcpd-v12-family-only-folds-retired.v1")
OFFICIAL_FOLD_ASSIGNMENT_SHA256S = {
    "warehouse-r41-v9-blocked-cv-a-20260913": (
        "3f8bec6a6367145c0e8d214f7c1affe7eb77fafedfd0998eccca4d6c05228a2b"),
    "warehouse-r41-v9-blocked-cv-b-20260913": (
        "6f97ce2c62842b8a7ff16dcfce10f9cc4f27d281c88c40aabb8cabe6fe38a458"),
}
GRID_VERSION = "warehouse-r41-diagnostic-rcpd-v12-candidate-grid.v1"
FROZEN_CANDIDATE_GRID_SHA256 = (
    "e30cea308c3b07b69281e15acbee298650a9b585d356c2b7ef279961faba2733"
)
...
_PRIOR_PROJECTION_COMPONENTS = frozenset((
    "consumed_v8", "consumed_v9", "consumed_v10", "consumed_v11",
    "consumed_v12_outer", "burned_v12_final",
))
_PRIOR_PROJECTION_FIELDS = frozenset((
    ...
    "source_v11_closeout_content_sha256",
    "source_v11_projection_content_sha256",
    "source_v12_final_closeout_content_sha256",
    "source_v12_outer_projection_content_sha256",
    "source_v12_final_projection_content_sha256",
...
class NoEligibleCandidateError(RuntimeError):
    ...
    def __init__(self, candidate_reports: Sequence[Mapping[str, Any]]) -> None:
        ...
        super().__init__(
            "No v13 candidate passed both salted aggregate gate suites; "
...
def contract() -> dict[str, Any]:
    return {
        "version": VERSION,
        "development_sources": [
            "permanently closed failed-v8 outer rows",
            "permanently closed v11 and v12 outer/final evaluation rows",
        ],
        "candidate_grid_sha256": FROZEN_CANDIDATE_GRID_SHA256,
        "consumed_v9_outer_input": (
            "identity exclusion is bound by the v13 registry; labels and "
        ...
        "prior_outer_input": (
            "registry-bound union of consumed v8 through v12 one-way "
            "observation hashes only"
        ),
        "prior_outer_projection_bindings": [
            "file_sha256", "content_sha256",
        ],
        "fresh_outer_input": "ordered v13 label-blind observation SHA-256 projection only",
        ...
        "all_retained_closed_outer_rows_reclassified_as_development": True,
        "development_precedence": (
            "fresh-v13-validation > combined-promoted-development > older-development"),
        ...
        "cross_validation": {
            ...
            "legacy_family_only_status_binding": (
                "development.legacy_family_only_split_status"),
        ...
        "shared_pickup_replacement_specialist": {
            "group": "shared_pickup",
        ...
        "shared_charger_endpoint_specialist": {
            "route": "derived.critical.shared_charger > 0.5",
            "fit_population": (
                "unique endpoints of fit-partition effective shared-charger pairs"),
            "sample_weights": (
                "fit-only global weights recomputed with specialist-only 0.8 "
                "WAIT redistribution; base remains at 0.5"),
            "combination": "weighted_average",
            "mix_weight": 1.0,
            "held_fold_labels_used_for_fit": False,
            "runtime_action_override": False,
        },
        "consumed_v9_outer_labels_or_probabilities_read": False,
        "consumed_v10_outer_labels_or_probabilities_read": False,
        "consumed_v11_outer_labels_or_probabilities_read_only_after_closeout_and_masks": True,
        "consumed_v12_outer_labels_or_probabilities_read_only_after_closeout_and_masks": True,
        "burned_v12_final_labels_or_probabilities_read_only_after_closeout_and_masks": True,
        "fresh_v13_outer_labels_or_probabilities_read": False,
...
def read_candidate_grid(path: str | Path, *, expected_sha256: str) -> dict[str, Any]:
    candidate = _regular(path, "v12 candidate grid")
    value = _strict_json(candidate, "v12 candidate grid",
                         expected_sha256=expected_sha256)
    if set(value) != {"version", "configs", "content_sha256"}:
        raise ValueError("V12 candidate grid schema differs")
    ...
    if (value.get("version") != GRID_VERSION or not _content_valid(value)
        ...
        raise ValueError("V12 candidate grid semantics differ")
    normalized = [fit_api.normalize_config(config) for config in configs]
    hashes = [digest(config) for config in normalized]
    if len(hashes) != len(set(hashes)):
        raise ValueError("V12 candidate grid contains duplicate configurations")
...
def read_prior_outer_hash_projection(
    ...
    """Authenticate the registry-bound v8-through-v12 hash-only projection."""
    ...
    if (set(value) != _PRIOR_PROJECTION_FIELDS
            ...
            or value.get("selection_used_this_projection") is not True
...
def prepare_combined_development_rows(
    base_path: str | Path, *, expected_base_sha256: str,
    combined_promoted_path: str | Path, expected_combined_promoted_sha256: str,
    prior_outer_observation_hashes: Sequence[str],
    combined_promoted_observation_hashes: Sequence[str],
    fresh_outer_observation_hashes: Sequence[str],
    actor: NumPyNativeActor,
    early_member_audit: list[str] | None = None,
) -> tuple[dict[str, np.ndarray], dict[str, Any]]:
    """Promote closed v11/v12 rows while freezing masks before target access.

    ``prior_outer_observation_hashes`` is the registry-bound union through the
    burned v12 final.  Older development removes that full union.  The combined
    promoted component removes only older historical hashes plus the new v13 outer,
    so it is not accidentally deleted by its own closeout projection.
    """
    base = _regular(
        base_path, "failed-v8 development rows", maximum=MAX_NPZ_BYTES)
    promoted = _regular(
        combined_promoted_path, "promoted prior evaluation rows",
        maximum=MAX_NPZ_BYTES)
    base_public = read_public_development_projection(
        base, expected_sha256=expected_base_sha256,
        audit=early_member_audit)
    promoted_public = read_public_development_projection(
        promoted, expected_sha256=expected_combined_promoted_sha256,
        audit=early_member_audit)

    prior = validation_hash_union(prior_outer_observation_hashes, ())
    promoted_hashes = validation_hash_union(
        combined_promoted_observation_hashes, ())
    fresh = validation_hash_union(fresh_outer_observation_hashes, ())
    prior_set, promoted_set = set(prior), set(promoted_hashes)
    if not promoted_set or not promoted_set <= prior_set:
        raise ValueError("Combined promoted hashes must be one bound prior component")
    promoted_public_hashes = set(map(str, np.char.decode(
        promoted_public["observation_hashes"], "ascii")))
    if promoted_public_hashes != promoted_set:
        raise ValueError("Combined promoted rows and hash projection differ")
    historical = sorted(prior_set - promoted_set)
    if promoted_set & set(historical):
        raise RuntimeError("Historical and combined promoted projections overlap")

    base_exclusions = validation_hash_union(prior, fresh)
    promoted_exclusions = validation_hash_union(historical, fresh)
    base_keep, base_audit = freeze_validation_wins(
        base_public, base_exclusions)
    promoted_keep, promoted_audit = freeze_validation_wins(
        promoted_public, promoted_exclusions)
    base_retained_hashes = set(map(str, np.char.decode(
        base_public["observation_hashes"][base_keep], "ascii")))
    promoted_retained_hashes = set(map(str, np.char.decode(
        promoted_public["observation_hashes"][promoted_keep], "ascii")))
    if base_retained_hashes & promoted_retained_hashes:
        raise RuntimeError("Development-source precedence left duplicate observations")

    # Both masks and their zero-overlap assertions are now immutable.  This is
    # the first point at which either archive's actions/probabilities are read.
    base_full = _load_full_development_rows(base)
    promoted_full = _load_full_development_rows(promoted)
    if (file_hash(base) != expected_base_sha256
            or file_hash(promoted) != expected_combined_promoted_sha256):
        raise RuntimeError("Development archive changed before private-member read")
    for name in _EARLY_DEVELOPMENT_MEMBERS:
        if (not np.array_equal(base_full[name], base_public[name])
                or not np.array_equal(
                    promoted_full[name], promoted_public[name])):
            raise RuntimeError(
                "Development public member changed between stages")
    rows_api._validate_base_shapes(base_full, actor, "failed-v8 development")
    rows_api._validate_base_shapes(
        promoted_full, actor, "combined promoted development")
    if not bool(np.all(promoted_full["split_validation"])):
        raise ValueError("Combined promoted source must be the consumed outer split")
    base_retained = {
        name: np.asarray(value)[base_keep].copy()
        for name, value in base_full.items()
    }
    promoted_retained = {
        name: np.asarray(value)[promoted_keep].copy()
        for name, value in promoted_full.items()
    }
    if (not len(base_retained["observations"])
            or not len(promoted_retained["observations"])):
        raise ValueError("Validation-wins removed a complete development source")
    if set(base_retained) != set(promoted_retained):
        raise RuntimeError("Development archive field registries differ")
    base_retained["split_validation"] = np.zeros(
        len(base_retained["observations"]), dtype=np.bool_)
    promoted_retained["split_validation"] = np.zeros(
        len(promoted_retained["observations"]), dtype=np.bool_)
    for child_audit, child_rows in (
        (base_audit, base_retained), (promoted_audit, promoted_retained),
    ):
        child_audit.update({
            "retained_rows_semantic_sha256": rows_api.arrays_digest(child_rows),
            "all_retained_rows_marked_development": True,
            "private_members_read_only_after_mask_frozen": True,
            "source_archive_reauthenticated_after_private_read": True,
        })
        child_audit["content_sha256"] = digest({
            key: value for key, value in child_audit.items()
            if key != "content_sha256"
        })
    combined: dict[str, np.ndarray] = {}
    for name in base_retained:
        left, right = base_retained[name], promoted_retained[name]
        if left.dtype != right.dtype or left.shape[1:] != right.shape[1:]:
            raise ValueError("Development archive array contracts differ: " + name)
        combined[name] = np.concatenate((left, right), axis=0)
    combined["split_validation"] = np.zeros(
        len(combined["observations"]), dtype=np.bool_)
    combined_hashes = set(map(str, np.char.decode(
        combined["observation_hashes"], "ascii")))
    if (combined_hashes & set(fresh)
            or set(map(str, np.char.decode(
                base_retained["observation_hashes"], "ascii"))) & promoted_set):
        raise RuntimeError("Combined development reintroduced validation overlap")
    rows_api._validate_base_shapes(combined, actor, "combined v13 development")
    audit: dict[str, Any] = {
        "precedence": (
            "fresh-v13-validation > combined-promoted-development > "
            "older-development"),
        "base": base_audit,
        "combined_promoted": promoted_audit,
        "prior_unique_observations": len(prior),
        "historical_unique_observations": len(historical),
        "combined_promoted_unique_observations": len(promoted_hashes),
        "fresh_v13_unique_observations": len(fresh),
        "combined_promoted_projection_sha256": digest(promoted_hashes),
        "combined_source_rows": (
            len(base_public["observations"])
            + len(promoted_public["observations"])),
        "combined_retained_rows": len(combined["observations"]),
        "combined_retained_scene_count": len(set(map(str, np.char.decode(
            combined["scene_fingerprints"], "ascii")))),
        "cross_component_observation_overlap": 0,
        "retained_fresh_outer_observation_overlap": 0,
        "retained_rows_semantic_sha256": rows_api.arrays_digest(combined),
        "all_retained_rows_marked_development": True,
    ...
    while True:
        ...
        if not candidates:
            raise fit_api.ReplacementSupportError(
                "V13 public same-family swap cannot satisfy frozen support")
...
def _replacement_support_audit(
    ...
    if (route.shape != selected.shape or scenes.shape != route.shape
        ...
        raise ValueError("V13 replacement support registry differs")
...
def legacy_family_only_support_status(
    arrays: Mapping[str, np.ndarray], *, scene_families: Mapping[str, str],
    relations: R41DiagnosticPublicRelationsV9, config: Mapping[str, Any],
) -> dict[str, Any]:
    """Record the old family-only folds on the expanded public development set."""
    scenes = np.char.decode(
        np.asarray(arrays["scene_fingerprints"]), "ascii").astype(str)
    assignments: dict[str, str] = {}
    fold_support: dict[str, dict[str, Any]] = {}
    all_supported = True
    for salt in CV_SALTS:
        assignment = assign_blocked_scene_folds(scene_families, salt=salt)
        assignments[salt] = digest(dict(sorted(assignment.items())))
        ...
        for fold in range(FOLD_COUNT):
            audit = _replacement_support_audit(
                ...
                config=config)
            fold_support[salt][str(fold)] = audit
            all_supported = all_supported and bool(audit["passed"])
    result = {
        "assignment_version": LEGACY_FOLD_ASSIGNMENT_VERSION,
        "status": ("supported_after_combined_promotion" if all_supported
                   else "contains_unsupported_partition"),
        "assignment_sha256s": assignments,
        "fold_support": fold_support,
        "all_partitions_supported": bool(all_supported),
        "action_labels_or_probabilities_used": False,
    }
...
# Compatibility alias for local callers; the returned record is now a status,
# because the expanded promoted-development set no longer reproduces the old
# one-row validation support failure.
legacy_family_only_support_failure = legacy_family_only_support_status
    ...
    for salt in salts:
        ...
        for fold in range(FOLD_COUNT):
            ...
            if (predicted.shape != (int(np.sum(validation)), len(fit_api.ACTIONS))
                ...
                raise ValueError("V13 fold prediction probabilities differ")
    ...
    value: dict[str, Any] = {
        ...
        "selection": {
            "selected_config_sha256": _sha(
                selected_config_sha256, "selected v13 fit configuration"),
        ...
        "information_boundary": {
            ...
            "consumed_v9_outer_labels_or_probabilities_read": False,
            "consumed_v10_outer_labels_or_probabilities_read": False,
            "consumed_v11_outer_targets_read_only_after_closeout_and_masks": True,
            "consumed_v12_outer_targets_read_only_after_closeout_and_masks": True,
            "burned_v12_final_targets_read_only_after_closeout_and_masks": True,
...
def _resolve_snapshot(
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    promoted_bundle_closeout_path: str | Path,
    expected_promoted_bundle_closeout_sha256: str,
    ...
    combined_promoted_rows_path: str | Path, candidate_grid_path: str | Path,
    ...
    if candidate_grid_sha256 != FROZEN_CANDIDATE_GRID_SHA256:
        raise ValueError("Exact preregistered v12 candidate grid SHA-256 required")
    ...
    closeout = base_closeout_api.read_saved_closeout(
        closeout_original,
    ...
    promoted_closeout_original = _regular(
        promoted_bundle_closeout_path, "combined promoted outer closeout")
    promoted_permanent = _directory(
        permanent_promotion_closeout_registry, "permanent v13 closeout registry")
    promoted_closeout = promotion_closeout_api.read_saved_closeout_public(
        promoted_closeout_original,
    ...
    expected_promoted_path = (
        promoted_closeout_original.parent / promotion_closeout_api.COMBINED_ROWS_NAME)
    if Path(combined_promoted_rows_path).expanduser().absolute() != expected_promoted_path:
        raise ValueError("Combined promoted rows must be the closeout companion")
    originals = {
        ...
        "combined_promoted_rows": _regular(
            combined_promoted_rows_path, "promoted prior evaluation rows",
            maximum=MAX_NPZ_BYTES),
        "candidate_grid": _regular(candidate_grid_path, "v12 candidate grid"),
    }
    ...
    expected = {
        ...
        "combined_promoted_rows": promoted_rows_sha,
...
def build(
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    (snapshot, paths, components, designation_original, closeout,
     promoted_closeout) = _resolve_snapshot(
        ...
        promoted_bundle_closeout_path=promoted_bundle_closeout_path,
        expected_promoted_bundle_closeout_sha256=(
            expected_promoted_bundle_closeout_sha256),
    ...
    try:
        ...
        saved_registry, saved_registry_report = registry_api.read_saved_registry(
            paths["registry"], paths["registry_report"],
            expected_registry_sha256=expected_registry_sha256,
            expected_report_sha256=expected_registry_report_sha256,
            burned_v12_final_closeout_path=paths["promoted_bundle_closeout"],
        ...
        promoted_projection = promoted_outer["observation_hash_projection"]
        promoted_rows_sha = _sha(
            promoted_outer["rows_sha256"], "combined promoted rows")
        promoted_rows_semantic_sha = _sha(
            promoted_outer["rows_semantic_sha256"],
            "combined promoted rows semantic digest")
        promoted_hashes = promoted_projection["outer_observation_hashes"]
        if (promoted_outer.get("source_order")
                != promotion_closeout_api.contract()[
                    "combined_promoted_source_order"]
                or promoted_outer.get("all_rows_split_validation_true") is not True
                or type(promoted_outer.get("row_count")) is not int
                or promoted_outer["row_count"] <= 0
                or not _content_valid(promoted_projection)
                or promoted_projection.get("outer_observation_hashes_sha256")
                    != digest(promoted_hashes)):
            raise ValueError("Combined promoted development bundle differs")
        ...
        if (prior_projection_file_sha256
            ...
            raise ValueError(
                "Prior outer projection does not match the v13 registry")
        ...
        if (designation.get("bindings", {}).get("actor_sha256") != actor.artifact_sha256
                ...
                or registry_bindings.get("promotion_closeout_file_sha256")
                    != file_hash(paths["promoted_bundle_closeout"])
                or registry_bindings.get("promotion_closeout_content_sha256")
                    != promoted_closeout["content_sha256"]
                or registry_bindings.get("combined_promoted_rows_sha256")
                    != promoted_rows_sha
                or registry_bindings.get("combined_promoted_rows_semantic_sha256")
                    != promoted_rows_semantic_sha
                or registry_bindings.get(
                    "combined_promoted_observation_hashes_sha256")
                    != promoted_projection[
                        "outer_observation_hashes_sha256"]
                or promoted_closeout.get("bindings", {}).get(
                    "combined_promoted_rows_sha256") != promoted_rows_sha
                or promoted_closeout.get("bindings", {}).get(
                    "combined_promoted_rows_semantic_sha256")
                    != promoted_rows_semantic_sha
                or promoted_closeout.get("bindings", {}).get(
                    "combined_projection_content_sha256")
                    != promoted_projection.get("content_sha256")
            ...
            raise ValueError("V13 selector evidence bindings differ")
        ...
        validation_hash_inputs = {
            ...
            "promotion_closeout_sha256": file_hash(
                paths["promoted_bundle_closeout"]),
            "combined_promoted_rows_sha256": promoted_rows_sha,
            "combined_promoted_rows_semantic_sha256": promoted_rows_semantic_sha,
            "combined_promoted_unique_observations": len(promoted_hashes),
            "combined_promoted_observation_hashes_sha256": digest(
                promoted_hashes),
        ...
        validation_hash_inputs["content_sha256"] = digest(
            validation_hash_inputs)
        arrays, overlap_audit = prepare_combined_development_rows(
            paths["development_rows"], expected_base_sha256=closed_rows_sha,
            combined_promoted_path=paths["combined_promoted_rows"],
            expected_combined_promoted_sha256=promoted_rows_sha,
            prior_outer_observation_hashes=prior_hashes,
            combined_promoted_observation_hashes=promoted_hashes,
            fresh_outer_observation_hashes=fresh_hashes, actor=actor)
        ...
        if relations.base_feature_names != actor_feature_names:
            raise ValueError("V13 relation base features differ from the frozen Actor")
        ...
        for salt in CV_SALTS:
            ...
            if (assignment_audit["repaired_assignment_sha256"]
                ...
                raise RuntimeError("Official v13 fold-assignment regression differs")
            fold_assignment_repairs[salt] = assignment_audit
        legacy_support_status = legacy_family_only_support_status(
        ...
        if (program.base_feature_names != actor_feature_names
            ...
            raise RuntimeError("Locked v13 program relation/Actor binding differs")
        ...
        if (not np.array_equal(
            ...
            raise RuntimeError("Locked v13 program round-trip action parity differs")
        if final_gate.get("passed") is not True:
            raise RuntimeError("Full-development v13 refit failed the nine hard gates")

        temporary = Path(tempfile.mkdtemp(
            prefix=".warehouse-r41-v13-fit-selector-", dir=destination.parent))
        ...
        report: dict[str, Any] = {
            ...
            "development": {
                ...
                "consumed_v11_outer_permanently_closed": True,
                "consumed_v11_outer_reclassified_as_development": True,
                "consumed_v12_outer_permanently_closed": True,
                "consumed_v12_outer_reclassified_as_development": True,
                "burned_v12_final_permanently_closed": True,
                "burned_v12_final_reclassified_as_development": True,
                ...
                "legacy_family_only_split_status": legacy_support_status,
        ...
        base_closeout_api.read_saved_closeout(
            failure_closeout_path,
            expected_closeout_sha256=expected_failure_closeout_sha256,
            permanent_registry=permanent_closeout_registry)
        promotion_closeout_api.read_saved_closeout_public(
            promoted_bundle_closeout_path,
            expected_closeout_sha256=expected_promoted_bundle_closeout_sha256,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    lock = build(
        ...
        promoted_bundle_closeout_path=args.promotion_closeout,
        expected_promoted_bundle_closeout_sha256=(
...
__all__ = [
    ...
    "prepare_development_rows", "prepare_combined_development_rows",
    "scene_families_from_public_geometry",
