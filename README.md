...
本次公开试玩仍是 9 月 2 日版本的内部预实验。Warehouse 新参与者默认使用中文，可点右上角 `English` 切换；A 组在 Task 1 每步点击机器人 2 旁的“为什么？”才展开该步理由，也可在右侧用快捷问题或自由提问。B 组无解释，Task 2 两组都不能读取新旧回答。它使用冻结的现有 Actor 加上上述 Robot 2 规则辅助控制器，因此不再是“纯 NN 原样动作”实验，也不宣称解释已经改善人类表现。
