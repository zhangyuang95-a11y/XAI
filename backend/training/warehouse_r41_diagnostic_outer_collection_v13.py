"""Collect the full fresh v13 outer rows only after candidate lock.
...
VERSION = "warehouse-r41-diagnostic-outer-collection.v13"
SCHEMA_VERSION = "warehouse_r41_diagnostic_rcpd_v13_outer_collection_v1"
STATUS = "collected_unscored"
CANDIDATE_LOCK_VERSION = "warehouse_r41_diagnostic_rcpd_v13_candidate_lock_v1"
SELECTOR_VERSION = "warehouse-r41-diagnostic-rcpd-v13-fit-selector.v1"
...
_LOCK_BOUNDARY_FIELDS = frozenset((
    ...
    "consumed_v11_outer_targets_read_only_after_closeout_and_masks",
    "consumed_v12_outer_targets_read_only_after_closeout_and_masks",
    "burned_v12_final_targets_read_only_after_closeout_and_masks",
...
_LEGACY_SUPPORT_STATUS_FIELDS = frozenset((
    "assignment_version", "status", "assignment_sha256s", "fold_support",
    "all_partitions_supported", "action_labels_or_probabilities_used",
    "content_sha256",
))
...
_DEVELOPMENT_FIELDS = frozenset((
    ...
    "failed_v8_outer_reclassified_as_development",
    "consumed_v11_outer_permanently_closed",
    "consumed_v11_outer_reclassified_as_development",
    "consumed_v12_outer_permanently_closed",
    "consumed_v12_outer_reclassified_as_development",
    "burned_v12_final_permanently_closed",
    "burned_v12_final_reclassified_as_development", "validation_wins",
    "validation_hash_inputs", "scene_families", "fold_assignment_version",
    "replacement_support_registry_sha256",
    "fold_assignment_repairs",
    "legacy_family_only_split_status", "retained_scene_count",
    "retained_row_count",
))
...
_COMBINED_VALIDATION_WINS_FIELDS = frozenset((
    "precedence", "base", "combined_promoted", "prior_unique_observations",
    "historical_unique_observations", "combined_promoted_unique_observations",
    "fresh_v13_unique_observations", "combined_promoted_projection_sha256",
    "combined_source_rows", "combined_retained_rows",
    "combined_retained_scene_count", "cross_component_observation_overlap",
    "retained_fresh_outer_observation_overlap",
    "retained_rows_semantic_sha256",
    "all_retained_rows_marked_development",
    "private_members_read_only_after_both_masks_frozen",
    "both_source_archives_reauthenticated_after_private_read",
    "content_sha256",
))
_VALIDATION_HASH_INPUT_FIELDS = frozenset((
    ...
    "promotion_closeout_sha256", "combined_promoted_rows_sha256",
    "combined_promoted_rows_semantic_sha256",
    "combined_promoted_unique_observations",
...
def _validate_static_rows(arrays: Mapping[str, np.ndarray]) -> None:
    if set(arrays) != rows_v7._FIELDS:
        raise ValueError("V13 outer row array schema differs")
    ...
    if (count <= 0
        ...
        raise ValueError("V13 outer row shapes or dtypes differ")
    ...
    if (not np.isfinite(observations).all()
        ...
        raise ValueError("V13 outer numeric or action-authority rows differ")
    ...
    if not np.array_equal(
        ...
        raise ValueError("V13 outer public-observation hashes differ")
    for name in ("observation_hashes", "scene_fingerprints",
                 "source_state_hashes"):
        try:
            values = np.char.decode(arrays[name], "ascii").astype(str)
        except UnicodeDecodeError as error:
            raise ValueError("V13 outer row text is not ASCII") from error
        if any(_HEX.fullmatch(str(value)) is None for value in values):
            raise ValueError("V13 outer row SHA-256 field differs")
...
def load_authenticated_rows(
    ...
    path = _regular(rows_path, "v13 outer rows", maximum=MAX_NPZ_BYTES)
    if file_hash(path) != _sha(expected_rows_sha256, "v13 outer rows"):
        raise ValueError("Exact v13 outer row bytes required")
    ...
    try:
        with zipfile.ZipFile(path) as archive:
            ...
            if (len(names) != len(set(names)) or set(names) != expected_members
                ...
                raise ValueError("V13 outer row archive contents differ")
        with np.load(path, allow_pickle=False) as archive:
            if set(archive.files) != rows_v7._FIELDS:
                raise ValueError("V13 outer row array schema differs")
            arrays = {name: archive[name].copy() for name in archive.files}
    except (OSError, ValueError, zipfile.BadZipFile, KeyError) as error:
        raise ValueError("V13 outer rows are not a safe strict NPZ") from error
    if file_hash(path) != expected_rows_sha256:
        raise RuntimeError("V13 outer rows changed during strict load")
...
def _validate_candidate_lock_shape(value: Mapping[str, Any]) -> dict[str, str]:
    ...
    if (set(value) != _LOCK_FIELDS
        ...
        raise ValueError("Exact locked v13 candidate contract required")
...
def _validate_validation_wins(
    ...
    if (set(value) != _VALIDATION_WINS_FIELDS or not _content_valid(value)
        ...
        raise ValueError("Locked v13 selector validation-wins trace differs")
...
def _validate_validation_hash_inputs(value: Mapping[str, Any]) -> None:
    if (set(value) != _VALIDATION_HASH_INPUT_FIELDS
        ...
        raise ValueError("Locked v13 selector validation hash inputs differ")


def _validate_combined_validation_wins(
    value: Mapping[str, Any], *, retained_row_count: int,
    retained_scene_count: int, validation_hash_inputs: Mapping[str, Any],
) -> str:
    """Authenticate both frozen source masks and their combined row digest."""
    if (set(value) != _COMBINED_VALIDATION_WINS_FIELDS
            or not _content_valid(value)
            or value.get("precedence")
                != ("fresh-v13-validation > combined-promoted-development > "
                    "older-development")
            or not isinstance(value.get("base"), Mapping)
            or not isinstance(value.get("combined_promoted"), Mapping)
            or any(type(value.get(name)) is not int or value[name] <= 0
                   for name in (
                       "prior_unique_observations",
                       "combined_promoted_unique_observations",
                       "fresh_v13_unique_observations",
                       "combined_source_rows", "combined_retained_rows",
                       "combined_retained_scene_count"))
            or type(value.get("historical_unique_observations")) is not int
            or value["historical_unique_observations"] < 0
            or value["combined_retained_rows"] != retained_row_count
            or value["combined_retained_scene_count"] != retained_scene_count
            or value["prior_unique_observations"]
                != validation_hash_inputs["prior_outer_unique_observations"]
            or value["combined_promoted_unique_observations"]
                != validation_hash_inputs["combined_promoted_unique_observations"]
            or value["fresh_v13_unique_observations"]
                != validation_hash_inputs["fresh_outer_unique_observations"]
            or value["combined_promoted_projection_sha256"]
                != validation_hash_inputs[
                    "combined_promoted_observation_hashes_sha256"]
            or value["historical_unique_observations"] != (
                value["prior_unique_observations"]
                - value["combined_promoted_unique_observations"])
            or value["cross_component_observation_overlap"] != 0
            or value["retained_fresh_outer_observation_overlap"] != 0
            or any(type(value.get(name)) is not str
                   or _HEX.fullmatch(value[name]) is None
                   for name in (
                       "combined_promoted_projection_sha256",
                       "retained_rows_semantic_sha256"))
            or value.get("all_retained_rows_marked_development") is not True
            or value.get(
                "private_members_read_only_after_both_masks_frozen") is not True
            or value.get(
                "both_source_archives_reauthenticated_after_private_read") is not True):
        raise ValueError("Locked v13 combined validation-wins trace differs")
    base = value["base"]
    promoted = value["combined_promoted"]
    base_semantic = _validate_validation_wins(
        base, retained_row_count=base.get("retained_rows", -1))
    promoted_semantic = _validate_validation_wins(
        promoted, retained_row_count=promoted.get("retained_rows", -1))
    if (base_semantic != base["retained_rows_semantic_sha256"]
            or promoted_semantic != promoted["retained_rows_semantic_sha256"]
            or value["combined_source_rows"]
                != base["source_rows"] + promoted["source_rows"]
            or value["combined_retained_rows"]
                != base["retained_rows"] + promoted["retained_rows"]
            or base["fresh_outer_unique_observations"]
                != validation_hash_inputs["combined_unique_observations"]
            or promoted["fresh_outer_unique_observations"] < max(
                value["historical_unique_observations"],
                value["fresh_v13_unique_observations"])
            or promoted["fresh_outer_unique_observations"] > (
                value["historical_unique_observations"]
                + value["fresh_v13_unique_observations"])):
        raise ValueError("Locked v13 validation-wins source accounting differs")
    return value["retained_rows_semantic_sha256"]
...
def _validate_public_support_audit(value: Any, *, require_passed: bool) -> None:
    ...
    if (not isinstance(value, Mapping)
        ...
        raise ValueError("Locked v13 public replacement support audit differs")
        ...
        if (not isinstance(partition, Mapping)
                ...
                or any(type(partition.get(name)) is not str
                       ...
                       for name in ("scene_registry_sha256",
                                    "episode_registry_sha256"))):
            raise ValueError("Locked v13 public support partition differs")
    ...
    if computed is not require_passed:
        raise ValueError("Locked v13 public support result differs")
...
def _validate_fold_assignment_revision(development: Mapping[str, Any]) -> None:
    ...
    legacy = development.get("legacy_family_only_split_status")
    if (development.get("fold_assignment_version") != FOLD_ASSIGNMENT_VERSION
            ...
            or set(legacy) != _LEGACY_SUPPORT_STATUS_FIELDS
            or legacy.get("assignment_version") != LEGACY_FOLD_ASSIGNMENT_VERSION
            or legacy.get("status") != "supported_after_combined_promotion"
            or legacy.get("all_partitions_supported") is not True
            or not isinstance(legacy.get("assignment_sha256s"), Mapping)
            or list(legacy["assignment_sha256s"]) != list(CV_SALTS)
            or not isinstance(legacy.get("fold_support"), Mapping)
            or list(legacy["fold_support"]) != list(CV_SALTS)
            or legacy.get("action_labels_or_probabilities_used") is not False
            or not _content_valid(legacy)):
        raise ValueError("Locked v13 fold-assignment revision differs")
    for salt in CV_SALTS:
        folds = legacy["fold_support"].get(salt)
        if (legacy["assignment_sha256s"].get(salt)
                != OFFICIAL_FOLD_ASSIGNMENT_SHA256S[salt]
                or not isinstance(folds, Mapping)
                or set(folds) != {str(index) for index in range(CV_FOLDS)}):
            raise ValueError("Locked v13 legacy fold status differs")
        for audit in folds.values():
            _validate_public_support_audit(audit, require_passed=True)
    ...
    for salt, audit in repairs_by_salt.items():
        if (not isinstance(audit, Mapping)
            ...
            raise ValueError("Locked v13 fold-assignment repair audit differs")
        for label in ("support_before", "support_after"):
            ...
            if (not isinstance(support, Mapping)
                ...
                raise ValueError("Locked v13 fold support summary differs")
        if any(audit["support_after"][str(fold)][name]
               < audit["minimum_support"][name]
               for fold in range(CV_FOLDS)
               for name in audit["minimum_support"]):
            raise ValueError("Locked v13 repaired fold remains unsupported")
        if len(audit["repairs"]) != 0:
            raise ValueError("Locked v13 minimal fold repair count differs")
        for repair in audit["repairs"]:
            if (not isinstance(repair, Mapping)
                ...
                raise ValueError("Locked v13 same-family fold repair differs")
        if ((not audit["repairs"]
             and audit["legacy_assignment_sha256"]
                ...
                or (audit["repairs"]
                    and audit["legacy_assignment_sha256"]
                        == audit["repaired_assignment_sha256"])):
            raise ValueError("Locked v13 fold repair assignment digest differs")
        if (audit["repaired_assignment_sha256"]
                != OFFICIAL_FOLD_ASSIGNMENT_SHA256S[salt]):
            raise ValueError("Locked v13 official fold assignment differs")
    if any(legacy["assignment_sha256s"][salt]
           != repairs_by_salt[salt]["legacy_assignment_sha256"]
           for salt in CV_SALTS):
        raise ValueError("Locked v13 legacy fold assignment differs")
...
def _validate_selector_report(
    ...
    if (set(report) != _SELECTOR_FIELDS
        ...
        raise ValueError("Locked v13 selector report identity or bindings differ")
    ...
    if (not isinstance(contract_value, Mapping)
            ...
            or contract_value.get(
                "consumed_v10_outer_labels_or_probabilities_read") is not False
            or contract_value.get(
                "consumed_v11_outer_labels_or_probabilities_read_only_after_closeout_and_masks")
                is not True
            or contract_value.get(
                "consumed_v12_outer_labels_or_probabilities_read_only_after_closeout_and_masks")
                is not True
            or contract_value.get(
                "burned_v12_final_labels_or_probabilities_read_only_after_closeout_and_masks")
                is not True
            or contract_value.get(
                "fresh_v13_outer_labels_or_probabilities_read") is not False
            ...
            or development.get("consumed_v11_outer_permanently_closed") is not True
            or development.get("consumed_v11_outer_reclassified_as_development")
                is not True
            or development.get("consumed_v12_outer_permanently_closed") is not True
            or development.get("consumed_v12_outer_reclassified_as_development")
                is not True
            or development.get("burned_v12_final_permanently_closed") is not True
            or development.get("burned_v12_final_reclassified_as_development")
                is not True
            ...
            or validation_hash_inputs.get("promotion_closeout_sha256")
                != bindings.get("promotion_closeout_sha256")
            or validation_hash_inputs.get("combined_promoted_rows_sha256")
                != bindings.get("combined_promoted_rows_sha256")
            ...
            or validation_hash_inputs.get("labels_or_probabilities_included")
                is not False
            or validation_wins.get("prior_unique_observations")
                != validation_hash_inputs.get("prior_outer_unique_observations")
            or validation_wins.get("combined_promoted_unique_observations")
                != validation_hash_inputs.get("combined_promoted_unique_observations")
            or validation_wins.get("fresh_v13_unique_observations")
                != validation_hash_inputs.get("fresh_outer_unique_observations")
            or validation_wins.get("retained_fresh_outer_observation_overlap") != 0
            or validation_wins.get(
                "private_members_read_only_after_both_masks_frozen") is not True
            ...
            or boundary.get("runtime_action_override") is not False
            or boundary.get("formal_ready") is not False):
        raise ValueError("Locked v13 selector information boundary differs")
    ...
    if (type(development.get("retained_row_count")) is not int
        ...
        raise ValueError("Locked v13 selector development row count differs")
    retained_rows_semantic_sha256 = _validate_combined_validation_wins(
        validation_wins,
        retained_row_count=development["retained_row_count"],
        retained_scene_count=development.get("retained_scene_count", -1),
    ...
    if (not isinstance(grid, Mapping)
        ...
        raise ValueError("Locked v13 selector selection differs")
    ...
    if (not isinstance(candidate_reports, list) or not candidate_reports
        ...
        raise ValueError("Locked v13 selector candidate registry differs")
    ...
    for candidate, expected_config_sha in zip(candidate_reports, config_hashes):
        if (not isinstance(candidate, Mapping)
            ...
            raise ValueError("Locked v13 selector candidate report differs")
        ...
        if evaluation.get("status") == "ineligible_replacement_support":
            ...
            if (not isinstance(failure, Mapping)
                    ...
                    or not isinstance(partitions, Mapping)
                    or set(partitions) != {"fit", "validation"}):
                raise ValueError("Locked v13 selector support failure differs")
            for partition in partitions.values():
                if (not isinstance(partition, Mapping)
                        or set(partition) != _SUPPORT_PARTITION_FIELDS
                        or any(type(partition.get(name)) is not int
                        ...
                        or any(type(partition.get(name)) is not str
                               ...
                               for name in ("scene_registry_sha256",
                                            "episode_registry_sha256"))):
                    raise ValueError("Locked v13 selector support partition differs")
            all_supported = all(
            ...
            if ((failure["code"]
                ...
                raise ValueError("Locked v13 selector ineligible summary differs")
            normalized_candidates.append(candidate)
            continue
        if (evaluation.get("status") != "completed"
                or evaluation.get("failure") is not None
                or len(salts) != len(CV_SALTS)):
            raise ValueError("Locked v13 selector completed evaluation differs")
        salt_passes = []
        salt_minima = []
        complexities = []
        assignment_hashes = []
        for salt_report, expected_salt in zip(salts, CV_SALTS):
            ...
            if (not isinstance(salt_report, Mapping)
                ...
                raise ValueError("Locked v13 selector two-salt CV trace differs")
            ...
            for fold in folds:
                ...
                if (set(fold) != _FOLD_REPORT_FIELDS
                    ...
                    raise ValueError("Locked v13 selector fold trace differs")
                if (fold["fit_rows"] + fold["validation_rows"]
                    ...
                    raise ValueError("Locked v13 selector fold partition differs")
                ...
                _gate_matches(
                    ...
                    label="Locked v13 selector informational fold",
        ...
        if (candidate.get("both_salts_pass_all_aggregate_gates") is not both_pass
            ...
            raise ValueError("Locked v13 selector candidate summary differs")
        normalized_candidates.append(candidate)
    ...
    if candidate_grid_file_sha256 != bindings["candidate_grid_sha256"]:
        raise ValueError("Locked v13 selector candidate-grid binding differs")
    ...
    if not eligible:
        raise ValueError("Locked v13 selector has no eligible candidate")
    chosen = eligible[0]
    expected_selection = {
    ...
    if (dict(selection) != expected_selection
        ...
        raise ValueError("Locked v13 selector chosen candidate differs")

    final_fit = report.get("final_fit")
    if (not isinstance(final_fit, Mapping)
            or set(final_fit) != _FINAL_FIT_FIELDS):
        raise ValueError("Locked v13 selector full-development refit differs")
    _gate_matches(final_fit.get("development_metrics"),
                  final_fit.get("development_gate"),
                  label="Locked v13 selector full-development", require_passed=True)
...
def authenticate_locked_candidate_selector(
    *, lock: Mapping[str, Any], selector_report_path: str | Path,
    ...
    path = _regular(selector_report_path, "locked v13 selector report")
    expected = _sha(expected_selector_report_sha256, "locked selector report")
    raw = path.read_bytes()
    if sha256(raw).hexdigest() != expected:
        raise ValueError("Exact locked v13 selector report bytes required")
    report = _strict_json_bytes(raw, "locked v13 selector report")
    if file_hash(path) != expected:
        raise RuntimeError("Locked v13 selector report changed during authentication")
...
def _validate_candidate_bindings(
    *, lock: Mapping[str, Any], paths: Mapping[str, Path],
    ...
    actual = {
        ...
        "combined_promoted_rows_sha256": file_hash(paths["combined_promoted_rows"]),
    ...
    if any(bindings[name] != value for name, value in actual.items()):
        raise ValueError("V12 candidate lock artifact binding differs")
    ...
    if closure is not None:
        ...
        if (not normalized
            ...
            raise ValueError("V12 candidate source closure binding differs")
    else:
        report_binding = selector_report.get("bindings", {}).get(
            "source_closure_sha256")
        if report_binding != bindings["source_closure_sha256"]:
            raise ValueError("V12 candidate source closure is not authenticated")
    return bindings
...
def _resolve_snapshot(
    *, lock: Mapping[str, Any], actor_path: str | Path,
    ...
    ImmutableInputSnapshot, dict[str, Path], dict[str, Path], Path,
    dict[str, Any],
]:
    bindings = _validate_candidate_lock_shape(lock)
    promoted_closeout_original = _regular(
        promotion_closeout_path, "v13 promotion closeout")
    if file_hash(promoted_closeout_original) != _sha(
            expected_promotion_closeout_sha256,
            "v13 promotion closeout"):
        raise ValueError("Exact v13 promotion closeout bytes required")
    ...
    originals = {
        ...
        "candidate_lock": _regular(candidate_lock_path, "v13 candidate lock"),
        ...
        "combined_promoted_rows": _regular(
            combined_promoted_rows_path, "combined promoted development rows",
            maximum=MAX_NPZ_BYTES),
        "program": _regular(program_path, "locked v13 program"),
    ...
    if (bindings["promotion_closeout_sha256"]
            != expected_promotion_closeout_sha256
            or bindings["combined_promoted_rows_sha256"] != promoted_rows_sha):
        raise ValueError("Candidate lock combined-promoted bindings differ")
    ...
    *, paths: Mapping[str, Path], lock: Mapping[str, Any],
    ...
    value: dict[str, Any] = {
        ...
        "information_boundary": {
            ...
            "promotion_closeout_authenticated_before_outer_replay": True,
...
def build(
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    candidate_file = _regular(candidate_lock_path, "v13 candidate lock")
    if file_hash(candidate_file) != _sha(
            expected_candidate_lock_sha256, "v13 candidate lock"):
        raise ValueError("Exact v13 candidate lock bytes required")
    candidate_lock = _strict_json(candidate_file, "v13 candidate lock")
    _validate_candidate_lock_shape(candidate_lock)

    sources = producer_sources()
    (snapshot, paths, components, designation_original,
    ...
    try:
        ...
        lock_fd = os.open(
            ...
            | getattr(os, "O_NOFOLLOW", 0), 0o600)
        locked = _strict_json(paths["candidate_lock"], "snapshotted v13 candidate lock")
        _validate_candidate_bindings(lock=locked, paths=paths)
        selector_report = _strict_json(
            paths["selector_report"], "locked v13 selector report")
        ...
        if (not set(promoted_hashes) <= set(prior_hashes)
                or file_hash(paths["combined_promoted_rows"])
                    != promoted_outer["rows_sha256"]):
            raise ValueError("Combined-promoted closeout evidence differs")
        ...
        actual_validation_hash_inputs = {
            ...
            "promotion_closeout_sha256": file_hash(
                paths["promotion_closeout"]),
            "combined_promoted_rows_sha256": promoted_outer["rows_sha256"],
        ...
        arrays, accounting, environment_steps = projection_api._replay_outer(
            ...
            progress_label="fresh_v13_outer_full_collection")
        ...
        temporary = Path(tempfile.mkdtemp(
            prefix=".warehouse-r41-v13-outer-collection-", dir=destination.parent))
        ...
        if (set(reopened) != set(arrays)
            ...
            raise RuntimeError("Staged v13 outer row archive changed")
...
def authenticate_saved_collection(
    *, report_path: str | Path, rows_path: str | Path,
    ...
    report_file = _regular(report_path, "v13 outer collection report")
    rows_file = _regular(rows_path, "v13 outer rows", maximum=MAX_NPZ_BYTES)
    projection_file = _regular(
        projection_copy_path, "v12 copied outer hash projection")
    expected = {
        "report": _sha(expected_report_sha256, "v13 outer collection report"),
        "rows": _sha(expected_rows_sha256, "v13 outer rows"),
        "projection": _sha(
            expected_projection_copy_sha256, "v12 copied hash projection"),
    }
    if (file_hash(report_file) != expected["report"]
        ...
        raise ValueError("Exact v13 outer collection artifact set required")
    report = _strict_json(report_file, "v13 outer collection report")
    sources = producer_sources()
    ...
    if (report.get("version") != VERSION
            ...
            or boundary.get("runtime_action_override") is not False
            or report.get("formal_ready") is not False):
        raise ValueError("V13 outer collection receipt differs")
    if file_hash(report_file) != expected["report"]:
        raise RuntimeError("V13 outer collection report changed during authentication")
    return deepcopy(report)
