...
function task2Introduction(){if(release?.share_preview)return lang==='zh'?'体验版：先完成一次引导提问，之后可在关键节点选择是否查看解释。你是蓝色角色，AI 是橙色角色。':'Demo: ask one guided question first. Later, choose whether to see an explanation. You are blue; the AI is orange.';if(view.automatic_explanation_protocol==='guided-question-choice-v3')return lang==='zh'?'Task 2 开始时请完成一次引导提问。后续关键节点可选择直接继续或查看解释；你也可以在提问框追问。提问和阅读不消耗游戏步数。':'Start Task 2 with one guided question. At later decision points, choose to continue or see an explanation. You can also ask follow-up questions. Asking and reading do not use game turns.';return view.automatic_explanations_enabled?(lang==='zh'?'Task 2 中，AI 会在预设节点自动提供解释。你也可以主动追问；提问不消耗游戏回合。':'In Task 2, your AI teammate explains at preset points. You can also ask follow-up questions without using game turns.'):tr('task2Intro');}
...
function pongExplanationDock(){
 return domain==='pong'&&pendingAutomatic()?'<div id="pongExplanationDock" class="pong-explanation-dock" aria-hidden="true"></div>':'';
}
...
function observeAutomatic(){
 ...
 const card=pendingAutomatic()||latestAutomatic(),element=$('automaticDialog')||$('automaticExplanation');
