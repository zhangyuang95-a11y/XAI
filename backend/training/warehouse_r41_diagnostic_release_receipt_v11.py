"""Append-only receipt for one admitted r4.1 diagnostic v11 package."""
...
VERSION = "warehouse-r41-diagnostic-release-receipt.v11"
STATUS = "authenticated_internal_diagnostic_release_v11"
...
def _require_exact_admission_parent(
    ...
    """Reject packages whose parent is anything but this exact v11 admission."""
    ...
    if manifest.get("parent") != expected:
        raise ValueError("V11 package parent is not the exact v11 admission")
...
def validate_inputs(*, admission_path: str | Path,
    ...
    if file_hash(admission_file) != admission_sha:
        raise ValueError("V11 admission bytes differ")
    ...
    try:
        ...
        raise ValueError("V11 Base64 Secret File is invalid") from error
    package_raw = package.read_bytes()
    if decoded != package_raw:
        raise ValueError("V11 Base64 Secret File does not encode the package")
    ...
    if (manifest.get("play_scenes") != admission["play_scenes"]
        ...
        raise ValueError("V11 package does not derive from the admission")
    identities = manifest["identities"]
    bindings = admission["bindings"]
    for name in identities:
        if name in bindings and identities[name] != bindings[name]:
            raise ValueError("V11 package identity differs from admission: " + name)
    return {
...
def read_saved_receipt(path: str | Path, *, expected_sha256: str,
    ...
    if file_hash(receipt) != _sha(expected_sha256, "v9 release receipt"):
        raise ValueError("V11 release receipt bytes differ")
    ...
    if (set(value) != _FIELDS or value.get("version") != VERSION
        ...
        raise ValueError("Exact v11 release receipt required")
    current = validate_inputs(**inputs)
    for name, child in current.items():
        if value.get(name) != child:
            raise ValueError("V11 receipt differs from live input: " + name)
    return deepcopy(value)
