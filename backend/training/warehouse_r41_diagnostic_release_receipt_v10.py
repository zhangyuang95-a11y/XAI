"""Append-only receipt for one admitted r4.1 diagnostic v10 package."""
...
VERSION = "warehouse-r41-diagnostic-release-receipt.v10"
STATUS = "authenticated_internal_diagnostic_release_v10"
...
def _manifest_sha(package: Path) -> str:
    try:
        with zipfile.ZipFile(package, "r") as archive:
            ...
            if len(matches) != 1:
                raise ValueError("V10 package must contain one manifest")
            raw = archive.read(matches[0])
    except zipfile.BadZipFile as error:
        raise ValueError("V10 package is not a ZIP") from error
...
def validate_inputs(*, admission_path: str | Path,
    ...
    if file_hash(admission_file) != admission_sha:
        raise ValueError("V10 admission bytes differ")
    ...
    if len(encoded_raw) > release.MAX_BASE64_BYTES:
        raise ValueError("V10 Base64 Secret File exceeds 960,000 bytes")
    try:
        decoded = base64.b64decode(b"".join(encoded_raw.split()), validate=True)
    except (ValueError, base64.binascii.Error) as error:
        raise ValueError("V10 Base64 Secret File is invalid") from error
    package_raw = package.read_bytes()
    if decoded != package_raw:
        raise ValueError("V10 Base64 Secret File does not encode the package")
    ...
    for name in identities:
        if name in bindings and identities[name] != bindings[name]:
            raise ValueError("V10 package identity differs from admission: " + name)
    return {
...
def read_saved_receipt(path: str | Path, *, expected_sha256: str,
    ...
    if file_hash(receipt) != _sha(expected_sha256, "v9 release receipt"):
        raise ValueError("V10 release receipt bytes differ")
    ...
    if (set(value) != _FIELDS or value.get("version") != VERSION
        ...
        raise ValueError("Exact v10 release receipt required")
    current = validate_inputs(**inputs)
    for name, child in current.items():
        if value.get(name) != child:
            raise ValueError("V10 receipt differs from live input: " + name)
    return deepcopy(value)
