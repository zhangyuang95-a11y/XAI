...
let timeSince=performance.now(),timeBuckets={active:0,reading:0,replay:0,understanding_rating:0};
function sampleTime(){const now=performance.now(),seconds=Math.min(5,(now-timeSince)/1000);timeSince=now;if(!view?.stage?.startsWith('task')||(view.state?.terminal&&!pendingUnderstanding())||document.hidden||asking)return;const kind=pendingUnderstanding()?'understanding_rating':replay?'replay':(pendingAutomatic()||$('questionInput')===document.activeElement||document.querySelector('details[open]'))?'reading':'active';timeBuckets[kind]+=seconds;}
function takeTimings(){sampleTime();const items=Object.entries(timeBuckets).filter(([,seconds])=>seconds>0).map(([kind,seconds])=>({kind,seconds}));timeBuckets={active:0,reading:0,replay:0,understanding_rating:0};return items;}
...
const prolificEntry=location.pathname==='/prolific/'||!!reviewDomain;
let domain=reviewDomain||(prolificEntry?null:location.pathname.split('/').filter(Boolean)[0]||null);
const preview=!!reviewDomain||new URLSearchParams(location.search).get('preview')==='1';
const channel='BroadcastChannel' in window?new BroadcastChannel('policylens.study.v3'+(reviewDomain?'.review.'+reviewDomain:'')):null;
...
function languageUI(){if(prolificEntry||view?.prolific){const brand=document.querySelector('.brand');if(brand)brand.href=reviewDomain?'/review/'+reviewDomain+'/':'/prolific/';}if($('headerContext'))$('headerContext').textContent=view?.instance_id?title()+' · '+(view.stage.startsWith('task')?'Task '+view.stage.slice(-1):stageLabel(view.stage)):'HUMAN + AI';document.documentElement.lang=lang==='zh'?'zh-CN':'en';$('englishButton').classList.toggle('active',lang==='en');$('chineseButton').classList.toggle('active',lang==='zh');$('studyBadge').textContent=tr(view?.mode==='preview'?'preview':'pilot');$('footerText').textContent=tr('footer');}
...
function clearChat(){stopHeld();chatEpoch++;if(view){view.questions=[];view.automatic_explanations=[];view.can_ask=false;}draftQuestion='';asking=false;const box=$('chatLog');if(box)box.replaceChildren();const input=$('questionInput');if(input)input.value='';if($('chatPanel'))$('chatPanel').hidden=true;automaticObserver?.disconnect();if($('automaticExplanation'))$('automaticExplanation').remove();if($('automaticDialog'))$('automaticDialog').remove();}
async function refresh(){
 ...
 try{const next=await api(prolificEntry&&!reviewDomain?'/api/prolific/session'+location.search:'/api/study/view?domain='+encodeURIComponent(domain));if(epoch!==chatEpoch||sequence!==refreshSequence||document.hidden)return;
...
async function command(kind,extra={}){
 ...
 if(old.can_ask&&!next.can_ask)clearChat();if(epoch!==chatEpoch||document.hidden){next.questions=[];next.automatic_explanations=[];next.can_ask=false;pendingRefresh=true;}
...
function welcome(){if(release?.share_preview)return `<section class="panel"><h1>Task 2 demo / 体验版</h1><p>Choose a game to start or resume. / 选择游戏开始或继续。</p><a class="button" href="/try/">Choose a game / 选择游戏</a></section>`;return `<a class="back-link" href="/">${tr('back')}</a><div class="welcome-layout"><div><div class="eyebrow">${tr('eyebrow')}</div><h1>${title()}</h1><p class="muted">${lang==='zh'?meta[domain].desc:meta[domain].en}</p><div class="card-art">${cardArt(domain)}</div><p class="quiet-note">${domain==='kitchen'?(lang==='zh'?'先完成厨房练习，再进行三个任务，最后填写问卷。':'Try the kitchen controls, complete three tasks, then answer a short questionnaire.'):(lang==='zh'?'先观看演示，再完成三个任务，最后填写问卷。':'Watch the demonstration, complete three tasks, then answer a short questionnaire.')}</p></div><section class="panel"><h2>${lang==='zh'?'准备开始':'Before you begin'}</h2>${view?.previous_version_saved?`<p class="notice">${lang==='zh'?'玩法已更新。旧版记录已保留，请从新版演示开始。':'The tasks have been updated. Your previous records are saved; begin the new demonstration below.'}</p>`:''}<p class="muted">${tr('data')}</p><form id="startForm"><label class="field"><span>${tr('participant')}</span><input id="participantInput" autocomplete="off" minlength="3" maxlength="64" required value="${esc(participantId)}" placeholder="study-001"><small>${tr('participantHint')}</small></label>${preview?`<div class="notice">${tr('preview')} · ${lang==='zh'?'数据不计入真人研究':'Excluded from human-study analysis'}</div><label class="field"><span>${tr('previewKey')}</span><input id="adminInput" type="password" autocomplete="off" required></label>`:''}<label class="field"><span>${tr('group')}</span><select id="groupInput"><option value="A">A</option><option value="B">B</option></select><small>${lang==='zh'?'A 组仅 Task 2 可提问；B 组不提供问答。':'Group A can ask questions in Task 2 only; Group B has no Q&A.'}</small></label><label class="check-field"><input id="consentInput" type="checkbox" required><span>${tr('consent')}</span></label><div id="entryStatus" class="notice" role="status" aria-live="polite" ${preview||release?.study_ready?'hidden':''}><p id="entryStatusMessage">${entryStatusText()}</p><button id="entryRetryButton" type="button" class="button secondary">${tr('entryRetry')}</button></div><button id="startButton" class="button full" aria-describedby="entryStatusMessage" ${!release?.study_ready&&!preview?'disabled':''}>${domain==='kitchen'?tr('tutorialStart'):tr('start')} →</button></form></section></div>`;}
function progress(){if(release?.share_preview)return '';const stages=['demo','task1','task2','task3','questionnaire','completed'];const i=stages.indexOf(view.stage);return `<ol class="progress">${stages.slice(0,5).map((s,n)=>`<li class="${n===i?'current':n<i?'done':''}">${n===0?stageLabel('demo'):n===4?tr('questionnaire'):'Task '+n}</li>`).join('')}</ol>`;}
function task2Introduction(){return lang==='zh'?'首次提问按四步完成：① 点击 AI 头上的“？”；② 选择问题；③ 确认发送并阅读解释；④ 点击 Confirm 后才能继续游戏。此引导只出现一次；后续解释可跳过，定期 1–5 分评分仍必须提交。提问和阅读不消耗游戏步数。':'Complete the first question in four steps: ① click “?” above the AI; ② choose a question; ③ confirm the question and read the answer; ④ click Confirm to resume gameplay. This walkthrough happens once. Later explanations are optional; periodic 1–5 ratings remain required. Asking and reading do not use game turns.';}
function understandingNotice(){return view?.understanding_rating_enabled?`<p class="hint">${lang==='zh'?'Task 2 每完成约 20% 的进度时，在右侧用 1–5 分评价对 AI 队友的理解；任务结束时也需评分。必须选择并提交后才能继续。':'Every 20% of Task 2, and at the end, rate your understanding of the AI teammate from 1 to 5 on the right. Select and submit a rating before continuing.'}</p>`:'';}
function rewardNotice(){
 const r=view?.rewards;if(!r)return '';
 const money=pence=>'£'+(pence/100).toFixed(2),zh=lang==='zh';
 const preview=view.mode!=='pilot';
 const rows=r.completed_tasks.map(t=>`Task ${t.task}: ${money(t.bonus_pence)}`).join(' · ');
 return `<section class="reward-notice" aria-label="${zh?'报酬与奖金':'Payment and bonuses'}"><strong>${preview?(zh?'报酬规则预览（此体验不支付报酬）':'Reward preview — this demo is unpaid'):(zh?'报酬与奖金':'Payment and bonuses')}</strong><p>${zh?`完成实验的固定报酬 ${money(r.base_pence)}，Task 1、2、3 各最高 ${money(r.task_max_pence)}；总报酬最高 ${money(r.maximum_total_pence)}。`:`Complete the study for ${money(r.base_pence)} base pay, plus up to ${money(r.task_max_pence)} for each of Tasks 1, 2 and 3; up to ${money(r.maximum_total_pence)} total.`}</p><p>${zh?`每个 Task 的最终分数从 ${r.score_lower} 到 ${r.score_upper} 分，奖金线性增加至 ${money(r.task_max_pence)}。低于下限为 £0.00，高于上限封顶；每项四舍五入至便士。教学和演示不计分；提问次数及理解度评分不影响奖金。`:`Each task’s final score from ${r.score_lower} to ${r.score_upper} earns a linearly increasing bonus up to ${money(r.task_max_pence)}. At or below the lower bound: £0.00; at or above the upper bound: the maximum. Each bonus is rounded to the nearest penny (half up). Tutorials and demos do not count; questions and understanding ratings do not affect bonuses.`}</p>${rows?`<p class="reward-earned">${rows}<br><strong>${zh?'已完成任务奖金合计':'Bonus from completed tasks'}: ${money(r.earned_bonus_pence)}</strong>${view.stage==='completed'?` · ${zh?'含固定报酬合计':'Total including base pay'}: ${money(r.total_pence)}`:''}</p>`:''}${!preview?`<p>${zh?'奖金由研究团队核对后通过 Prolific 单独发放；此处金额不是到账确认。':'Bonuses are paid separately through Prolific after review; these amounts are not a payment confirmation.'}</p>`:''}</section>`;
}
function heading(){return `${domainNavigation()}${progress()}<div class="study-title"><div><div class="eyebrow">${title()}</div><h1>${view.stage.startsWith('task')?'Task '+view.stage.slice(-1):stageLabel(view.stage)}</h1><p>${view.stage==='task1'?tr('baseline'):view.stage==='task2'?(view.can_ask?task2Introduction():tr('control')):view.stage==='task3'?tr('transfer'):''}</p>${understandingNotice()}</div><div class="phase-tag"><i class="dot"></i>${view.mode==='pilot'?tr('pilot'):tr('preview')}${view.group?' · '+view.group:''}</div></div>${rewardNotice()}`;}
...
function board(s,isDemo=false){
 ...
 <div id="boardDrawing" class="board ${s.domain==='pong'?'pong-board':''}"><canvas id="studyCanvas" role="img" aria-label="${esc(title())}" tabindex="0"></canvas>${!isDemo&&view?.can_ask?`<button id="aiQuestionButton" class="ai-question-button ${questionGuide()&&!questionGuide().requested&&!ratingBlocksPlay()?'question-guide-target':''}" type="button" aria-controls="chatPanel" aria-label="${lang==='zh'?'向 AI 队友提问':'Ask your AI teammate a question'}" title="${lang==='zh'?'点击向 AI 队友提问':'Click to ask your AI teammate'}"> ? ${questionGuide()&&!questionGuide().requested&&!ratingBlocksPlay()?`<span class="question-guide-label">${lang==='zh'?'点击 ? 提问':'Click ? to ask'}</span>`:''}</button>`:''}</div></section>`;
}
...
function domainNavigation(){if(reviewDomain)return '';if(release?.share_preview)return `<nav class="domain-navigation"><a href="/try/">${lang==='zh'?'← 选择游戏':'← Choose a game'}</a><a href="/try/${domain}?restart=1">${lang==='zh'?'重新体验':'Restart demo'}</a></nav>`;if(view?.prolific)return ''; return `<nav class="domain-navigation" aria-label="${lang==='zh'?'切换领域':'Switch domain'}">${Object.entries(meta).map(([key,m])=>`<a href="/${key}/${preview?'?preview=1':''}" ${key===domain?'aria-current="page"':''}>${esc(lang==='zh'?m.zh:m.name)}</a>`).join('')}</nav>`;}
...
let automaticObserver=null,lastAutomaticId=null;
const automaticAcknowledged=new Set(),automaticPending=new Set();
function promptsBlockPlay(){return !!questionGuide()?.four_step||ratingBlocksPlay()||(!view?.optional_prompts&&!!pendingAutomatic());}
function ratingBlocksPlay(){return !!pendingUnderstanding();}
function optionalPromptNote(){return view?.optional_prompts&&!pendingUnderstanding()&&!questionGuide()?.four_step?`<p class="hint">${lang==='zh'?'可选择回答，也可以直接继续操作。未回答会记为跳过。':'You may respond or keep playing. Unanswered prompts are recorded as skipped.'}</p><button type="button" class="button secondary full" data-skip-prompts>${lang==='zh'?'暂不回答，继续':'Continue without answering'}</button>`:'';}
function pendingUnderstanding(){return view?.understanding_rating||null;}
function understandingDialog(){
 const r=pendingUnderstanding();if(!r)return '';const zh=lang==='zh';
 return `<section id="understandingDialog" class="panel understanding-dialog side-check" aria-labelledby="understandingTitle"><form id="understandingForm"><p class="eyebrow">Task ${r.task} · ${r.checkpoint}% ${zh?'进度':'progress'}</p><h2 id="understandingTitle">${esc(r.question)}</h2><p class="muted">${zh?'请根据当前感受选择，没有对错之分。':'Choose the rating that best matches how you feel. There is no right or wrong answer.'}</p><fieldset class="understanding-scale"><legend class="sr-only">${zh?'理解程度，1 至 5 分':'Understanding, from 1 to 5'}</legend>${r.labels.map((label,i)=>`<label><input type="radio" name="understanding" value="${i+1}" required><span class="rating-number">${i+1}</span><span>${esc(label)}</span></label>`).join('')}</fieldset><button id="submitUnderstanding" type="submit" class="button full" disabled>${zh?'提交并继续':'Submit and continue'}</button></form><p class="hint">${zh?'请先选择并提交评分，再继续游戏。':'Please select and submit a rating before continuing.'}</p></section>`;
}
let focusedUnderstanding=null;
function syncUnderstandingDialog(){
 const panel=$('understandingDialog'),rating=pendingUnderstanding();if(!panel||!rating)return false;
 stopHeld();if(!document.hidden&&focusedUnderstanding!==rating.id){focusedUnderstanding=rating.id;$('understandingTitle').tabIndex=-1;$('understandingTitle').focus({preventScroll:true});}
 updateControlLocks();return true;
}
function pendingAutomatic(){return view?.can_ask?(view.automatic_explanations||[]).find(c=>c.requires_confirmation&&!c.confirmed&&!c.skipped):null;}
function questionGuide(){const c=pendingAutomatic();return c?.in_question_panel&&c.onboarding?c:null;}
function latestAutomatic(){return view?.can_ask&&!replay?(view.automatic_explanations||[]).at(-1):null;}
function automaticPanel(){
 const card=latestAutomatic();if(!card||card.requires_confirmation)return '';
 return `<section id="automaticExplanation" class="panel automatic-explanation" data-explanation-id="${esc(card.id)}" aria-live="polite" aria-atomic="true"><div class="eyebrow">${lang==='zh'?'AI 队友解释':'AI teammate explanation'} · Task 2 · ${tr('turn')} ${card.turn}</div><p>${esc(card.body)}</p><div class="hint">${lang==='zh'?'解释对应标注回合。你可以继续操作，也可以在提问框追问。':'This explanation refers to the turn shown above. Continue playing or use the question box to follow up.'}</div></section>`;
}
function automaticDialog(){
 if(ratingBlocksPlay())return '';
 const card=pendingAutomatic();if(!card||questionGuide())return '';
 const zh=lang==='zh',choosing=card.guided&&!card.requested;
 const title=choosing?(card.onboarding?(zh?'先体验一次提问':'Try asking a question'):(zh?'你理解这个决定吗？':'Do you understand this decision?')):(zh?'AI 队友的回答':'Your AI teammate answers');
 const prompt=card.onboarding?(zh?'Task 2 中可以询问 AI 为什么这样做。请先点击下面的问题，查看回答后继续。提问和阅读不会消耗游戏步数。':'In Task 2, you can ask why the AI makes a decision. First, select the question below, read the answer, then continue. Asking and reading do not use game turns.'):(zh?'你理解 AI 队友为什么做这个决定吗？可以直接继续，也可以查看原因。':'Do you understand why your AI teammate is making this decision? You can continue or ask to see why.');
 const buttons=choosing?`${card.onboarding?'':`<button id="understoodExplanationButton" class="button secondary full">${zh?'我理解，继续':'I understand — continue'}</button>`}<button id="requestExplanationButton" class="button full">${card.onboarding?esc(card.question):(zh?'还不理解，查看原因':'Not yet — show me why')}</button>`:`<button id="confirmExplanationButton" class="button full">${zh?'确认并继续':'Confirm and continue'}</button>`;
 return `<section id="automaticDialog" class="panel automatic-dialog side-check" data-domain="${esc(domain)}" aria-labelledby="automaticTitle" aria-describedby="automaticReason" data-explanation-id="${esc(card.id)}"><div class="automatic-speaker"><span class="automatic-avatar" aria-hidden="true">2</span><div><h2 id="automaticTitle">${title}</h2><div class="hint">Task 2 · ${tr('turn')} ${card.turn}</div></div></div>${card.guided&&card.requested?`<p class="guided-question">${zh?'你的问题：':'Your question: '}${esc(card.question)}</p>`:''}<div id="automaticReason" class="automatic-body">${esc(choosing?prompt:card.body)}</div>${choosing?'':`<p class="hint">${view.optional_prompts?(zh?'你可以继续操作，也可以确认已读。':'You can keep playing or acknowledge this answer.'):(zh?'游戏已暂停。请阅读回答，然后确认并继续。':'The game is paused. Read the answer, then confirm to continue.')}</p>`}<div class="automatic-choices">${buttons}</div>${optionalPromptNote()}</section>`;
}
let focusedQuestionGuide=null,focusedAutomatic=null;
function syncAutomaticDialog(){
 const guide=questionGuide();if(guide&&!document.hidden){
  if(guide.four_step||!view.optional_prompts)stopHeld();if((guide.four_step||!view.optional_prompts)&&focusedQuestionGuide!==guide.id){focusedQuestionGuide=guide.id;($('aiQuestionButton')||$('guidedQuestionExample'))?.focus({preventScroll:true});}
  updateControlLocks();return;
 }
 const panel=$('automaticDialog'),card=pendingAutomatic();if(!panel||!card||document.hidden)return;
 if(!view.optional_prompts)stopHeld();const key=card.id+':'+card.requested;
 if(!view.optional_prompts&&focusedAutomatic!==key){focusedAutomatic=key;($('understoodExplanationButton')||$('confirmExplanationButton'))?.focus({preventScroll:true});}
 updateControlLocks();
}
function observeAutomatic(){
 automaticObserver?.disconnect();automaticObserver=null;if(ratingBlocksPlay())return;
 const card=pendingAutomatic()||latestAutomatic(),element=$('guidedAnswer')||$('automaticDialog')||$('automaticExplanation');
 if(!card||!element||document.hidden||(card.guided&&!card.requested))return;
 if(card.id!==lastAutomaticId){if(!view.optional_prompts)stopHeld();lastAutomaticId=card.id;}
 if(card.displayed||automaticAcknowledged.has(card.id)||automaticPending.has(card.id))return;
 const instance=view.instance_id,run=view.run_id;
 automaticObserver=new IntersectionObserver(entries=>{
  if(!entries.some(e=>e.isIntersecting&&e.intersectionRatio>=.5)||document.hidden||!view?.can_ask||view.instance_id!==instance||view.run_id!==run||latestAutomatic()?.id!==card.id||automaticPending.has(card.id))return;
  automaticObserver?.disconnect();automaticPending.add(card.id);
  api('/api/study/automatic-explanation-displayed',{instance_id:instance,explanation_id:card.id})
   .then(()=>automaticAcknowledged.add(card.id)).catch(()=>{}).finally(()=>automaticPending.delete(card.id));
 },{threshold:[.5]});
 automaticObserver.observe(element);
}
let selectedGuideQuestion=null;
function fourStepGuide(g){
 const zh=lang==='zh',options=g.question_options||[],selected=selectedGuideQuestion?.card===g.id?selectedGuideQuestion.id:null;
 const step=g.requested?4:!g.guide_opened?1:selected?3:2;
 const steps=zh?['点击 AI 头上的“？”打开提问区。','选择一个你想问的问题。','点击“确认发送”，查看 AI 的解释。','读完后点击“Confirm — 确认并继续”，才可进行下一步。']:['Click the “?” above the AI to open the question choices.','Select one question you want to ask.','Click “Confirm question” to send it and see the explanation.','Read the answer, then click “Confirm and continue” to unlock the next move.'];
 return `<section id="chatPanel" class="panel chat-panel guided-chat-panel"><div class="question-guide"><h3>${zh?'首次提问指引':'First question walkthrough'} · ${step}/4</h3><ol class="guide-steps">${steps.map((text,i)=>`<li class="${i+1===step?'current':i+1<step?'done':''}" ${i+1===step?'aria-current="step"':''}>${text}</li>`).join('')}</ol><p>${zh?'仅首次需要完成这四步。游戏已暂停；最后点击 Confirm 后才会恢复。后续解释可跳过，1–5 分评分仍须提交。':'Complete these four steps once. Gameplay is paused until the final Confirm. Later explanations are optional; 1–5 ratings remain required.'}</p></div>${!g.requested?`<fieldset class="guide-options" ${!g.guide_opened?'disabled':''}><legend>${zh?'② 选择一个问题':'② Choose a question'}</legend>${options.map(q=>`<label><input type="radio" name="guideQuestion" value="${esc(q.id)}" ${selected===q.id?'checked':''}> ${esc(q.text)}</label>`).join('')}</fieldset><button id="sendGuideQuestion" class="button full" ${!g.guide_opened||!selected?'disabled':''}>${zh?'③ 确认发送':'③ Confirm question'}</button>`:`<div id="guidedAnswer" class="chat-message"><div class="chat-question">${esc(g.question)}</div><div class="chat-answer">${esc(g.body)}</div></div><button id="confirmExplanationButton" class="button full">${zh?'④ Confirm — 确认并继续':'④ Confirm and continue'}</button>`}</section>`;
}
function questionPanel(){
 if(!view.can_ask)return `<section class="panel locked-panel"><h3>${tr('closed')}</h3>${view.stage==='task3'?tr('transfer'):view.stage==='task1'?tr('baseline'):tr('control')}</section>`;
 const zh=lang==='zh',g=ratingBlocksPlay()?null:questionGuide(),available=!release?.share_preview||release.share_qa_available;
 if(g?.four_step)return fourStepGuide(g);
 const previous=(view.automatic_explanations||[]).find(c=>c.in_question_panel&&c.onboarding&&c.requested);
 const answer=previous?`<div ${g?'id="guidedAnswer"':''} class="chat-message"><div class="chat-question">${esc(previous.question)}</div><div class="chat-answer">${esc(previous.body)}</div><div class="chat-meta">${tr('turn')} ${previous.turn}</div></div>`:'';
 const intro=g?`<div class="question-guide" role="status"><h4>${zh?'试着向 AI 提问':'Try asking your AI teammate'}</h4><p>${g.requested?(zh?'回答已显示。你可以阅读后确认，也可以直接继续操作。':'Read the answer below. You may acknowledge it or keep playing.'):(zh?'先练习一次提问：点击机器人 2 头上的“？”，询问它为什么这样行动。也可以选择下方示例并发送。回答会显示在这里。提问不消耗步数。':'Try asking once: click the “?” above robot 2 to ask why it is taking this action. Or select and send the example below. The answer appears here. Questions do not use game turns.')}</p></div>`:'';
 return `<section id="chatPanel" class="panel chat-panel ${g?'guided-chat-panel':''}"><h3>${tr('ask')}</h3>${intro}<p class="hint">${g?'':task2Introduction()}</p><div id="chatLog" class="chat-log">${answer}${(view.questions||[]).slice(-1).map(q=>`<div class="chat-message"><div class="chat-question">${esc(q.question)}</div><div class="chat-answer">${esc(q.result?.answer||'')}</div><div class="chat-meta">${tr('turn')} ${q.target_turn}</div></div>`).join('')}</div>${g?.requested?`<button id="confirmExplanationButton" class="button full">${zh?'确认并继续':'Confirm and continue'}</button>`:`${g?`<button id="guidedQuestionExample" class="example guided-example">${esc(g.question)}</button>`:''}<textarea id="questionInput" maxlength="2000" placeholder="${g?(zh?'点击上方示例问题，然后发送':'Select the example above, then send it'):tr('questionPlaceholder')}" ${g?'readonly':''} ${asking||(!g&&!available)?'disabled':''}>${esc(draftQuestion)}</textarea>${g?'':`<div class="examples">${questionExamples().map(q=>`<button class="example" data-example="${q.id}">${esc(q.label)}</button>`).join('')}</div>`}${domain==='pong'&&!g?`<label class="hypothetical-lane"><span>${zh?'假设我的位置':'Imagine my position'}</span><select id="hypotheticalLane" aria-label="${zh?'假设球道':'Hypothetical lane'}">${Array.from({length:9},(_,i)=>i+1).map(n=>`<option value="${n}" ${n===hypotheticalLane?'selected':''}>${zh?'第 '+n+' 道':'Lane '+n}</option>`).join('')}</select><button class="example" data-example="position">${zh?'你会怎么移动？':'How would you move?'}</button></label>`:''}<button id="askButton" class="button teal full">${asking?tr('asking'):tr('send')} ↗</button>`}${g?optionalPromptNote():''}${!available?`<p class="hint">${zh?'体验站的自由问答正在连接中；示例提问和节点解释可正常体验。':'Free-text Q&A is not connected on this demo yet. Guided questions and node explanations are available.'}</p>`:''}</section>`;
}
...
function taskPage(){const s=replay?.state||view.state;if(release?.share_preview&&view.run_status==='completed'&&!replay&&!pendingUnderstanding())return `${heading()}${understandingDialog()}<section class="panel result-panel"><h2>${lang==='zh'?'Task 2 体验完成':'Task 2 demo complete'}</h2><div class="result-score">${Number(s.score.task_score).toFixed(0)}${scoreSuffix(s.score)}</div><a class="button" href="/try/">${lang==='zh'?'体验另一个游戏':'Try another game'}</a> <a class="button secondary" href="/try/${domain}?restart=1">${lang==='zh'?'重新体验':'Restart demo'}</a></section>${replayPanel(s)}`;if(view.run_status==='completed'&&!replay&&!pendingUnderstanding())return `${heading()}${understandingDialog()}<section class="panel result-panel"><div class="eyebrow">${tr('finishTitle')}</div><div class="result-score">${Number(s.score.task_score).toFixed(0)}${scoreSuffix(s.score)}</div><p class="muted">${tr('finishSub')}</p>${view.stage==='task2'?`<p class="hint">${view.task2_explanation_notice?tr('closed')+'. '+tr('transfer'):tr('control')}</p>`:''}<button id="nextButton" class="button">${view.stage==='task3'?tr('toSurvey'):tr('nextTask')+' '+(Number(view.stage.slice(-1))+1)} →</button></section>${replayPanel(s)}`;return `${heading()}<div class="workspace ${domain==='pong'?'pong-workspace':''}"><div>${board(s)}${replayPanel(s)}</div><aside class="side-stack">${understandingDialog()}${automaticDialog()}${automaticPanel()}${questionPanel()}<section class="panel"><h3>${tr('actions')}</h3>${replay?`<div class="notice">${tr('readOnly')}</div>`:''}<div class="action-grid">${view.actions.map(a=>`<button class="action-button ${selectedAction===a?'selected':''}" data-action="${a}" ${replay?'disabled':''}>${esc(actionLabel(a))}</button>`).join('')}</div><p class="control-keys">${controlsText()}</p>${domain==='kitchen'?`<p class="interaction-hint">${(typeof s.interaction==='object'&&s.interaction?.available)?'E: ':''}${esc(typeof s.interaction==='string'?s.interaction:(lang==='zh'?s.interaction?.label_zh:s.interaction?.label_en)||'')}</p>`:''}<p class="hint">${tr('controls')}</p></section>${domain==='kitchen'?kitchenMenu(s):''}<section class="panel"><details><summary>${domain==='kitchen'?tr('operationHelp'):tr('rules')}</summary>${conciseRules(s)}</details></section></aside></div>`;}
...
function reconcileChildren(parent,desired){
 ...
 for(let i=0;i<wanted.length;i++){
  ...
  for(const attr of [...current.attributes])if(!fresh.hasAttribute(attr.name)&&!(['DETAILS','DIALOG'].includes(current.tagName)&&attr.name==='open'))current.removeAttribute(attr.name);
...
async function render({animate=false,displayState=null}={}){
 if(pendingAutomatic()||pendingUnderstanding())replay=null;
 ...
 // Checks live in the task sidebar, never in a modal overlay.
 ...
 const canvas=$('studyCanvas');if(!canvas){boardRenderer?.destroy();boardRenderer=null;syncUnderstandingDialog();return;}
 ...
 if(!syncUnderstandingDialog())syncAutomaticDialog();observeAutomatic();
}
function bind(){if(prolificEntry&&!view?.instance_id)window.ProlificEntry.bind(async next=>{view=next;domain=next.domain;participantId=next.participant_id;await render();},render);if($('startForm'))$('startForm').onsubmit=async e=>{e.preventDefault();if(busy||(!preview&&!release?.study_ready))return;busy=true;updateEntryStatus();adminKey=$('adminInput')?.value||'';try{view=await api('/api/study/session',{domain,participant_id:$('participantInput').value,language:lang,consent:$('consentInput').checked,mode:preview?'preview':'pilot',group:$('groupInput').value},adminKey?{Authorization:'Bearer '+adminKey}:{});participantId=view.participant_id;adminKey='';render();}catch(err){if(err.code==='study_not_ready'){release={...release,study_ready:false};entryFailed=false;entryCheckedAt=-Infinity;}report(err);}finally{busy=false;updateEntryStatus();scheduleEntryCheck();}};
 if($('understandingDialog'))$('understandingDialog').oncancel=e=>e.preventDefault();
 document.querySelectorAll('[data-skip-prompts]').forEach(b=>{b.disabled=busy||animationPending;b.onclick=()=>{if(!busy&&!animationPending)command('skip_prompts');};});
 if($('understandingForm')){
  $('understandingForm').onchange=updateControlLocks;
  $('understandingForm').onsubmit=e=>{e.preventDefault();const r=pendingUnderstanding(),choice=new FormData(e.target).get('understanding');if(r&&choice&&!busy&&!animationPending)command('understanding_rating',{rating_id:r.id,rating:Number(choice)});};
 }
 if($('aiQuestionButton')){
 ...
 if($('aiQuestionButton'))$('aiQuestionButton').onclick=()=>{
  stopHeld();if(!view?.can_ask||ratingBlocksPlay()||busy||animationPending)return;
  const panel=$('chatPanel');if(!panel)return;
  const guide=pendingAutomatic();
  if(guide?.four_step&&!guide.requested){if(!guide.guide_opened)return command('open_question_guide',{explanation_id:guide.id});$('chatPanel')?.scrollIntoView({block:'nearest',behavior:'smooth'});return;}
  if(guide?.guided&&!guide.requested){draftQuestion='';return command('request_explanation',{explanation_id:guide.id,question_id:'why',source:'ai_question_button'});}
  const target=guide?$('confirmExplanationButton'):$('questionInput');
  if(innerWidth<900)panel.scrollIntoView({block:'nearest',behavior:'smooth'});
 ...
 if($('automaticDialog'))$('automaticDialog').oncancel=e=>e.preventDefault();
 if($('confirmExplanationButton'))$('confirmExplanationButton').onclick=()=>{const card=pendingAutomatic();if(card&&!busy&&!animationPending)command('confirm_explanation',{explanation_id:card.id,...(card.guided?{choice:'explanation'}:{})});};
 document.querySelectorAll('[name=guideQuestion]').forEach(input=>input.onchange=()=>{selectedGuideQuestion={card:questionGuide().id,id:input.value};render();});
 if($('sendGuideQuestion'))$('sendGuideQuestion').onclick=()=>{const g=questionGuide();if(g?.guide_opened&&selectedGuideQuestion?.card===g.id&&!busy&&!animationPending)command('request_explanation',{explanation_id:g.id,question_id:selectedGuideQuestion.id,source:'ai_question_button'});};
 if($('guidedQuestionExample'))$('guidedQuestionExample').onclick=()=>{const g=questionGuide();if(g&&!g.requested&&!busy){draftQuestion=g.question;$('questionInput').value=draftQuestion;updateControlLocks();$('askButton')?.focus();}};
 if($('requestExplanationButton'))$('requestExplanationButton').onclick=()=>{const card=pendingAutomatic();if(card&&!busy&&!animationPending)command('request_explanation',{explanation_id:card.id,question_id:'why'});};
 if($('understoodExplanationButton'))$('understoodExplanationButton').onclick=()=>{const card=pendingAutomatic();if(card&&!busy&&!animationPending)command('confirm_explanation',{explanation_id:card.id,choice:'understood'});};
...
async function loadFrame(run,turn){
 stopHeld();if(promptsBlockPlay()||busy||animationPending||asking||!view?.instance_id)return;
...
function canMove(){return !promptsBlockPlay()&&!busy&&!animationPending&&!replayLoading&&!asking&&!replay&&!document.hidden&&!editing()&&inputStageActive()&&!!actionState();}
function updateControlLocks(){updateDemoControls();if($('sendGuideQuestion'))$('sendGuideQuestion').disabled=busy||animationPending||!questionGuide()?.guide_opened||selectedGuideQuestion?.card!==questionGuide()?.id;for(const input of document.querySelectorAll('[name=guideQuestion]'))input.disabled=busy||animationPending||!questionGuide()?.guide_opened;if($('submitUnderstanding'))$('submitUnderstanding').disabled=busy||animationPending||!$('understandingForm')?.querySelector('input:checked');for(const input of document.querySelectorAll('#understandingForm input'))input.disabled=busy||animationPending;for(const id of ['confirmExplanationButton','requestExplanationButton','understoodExplanationButton','guidedQuestionExample','aiQuestionButton'])if($(id))$(id).disabled=ratingBlocksPlay()||busy||animationPending;for(const id of ['englishButton','chineseButton'])if($(id))$(id).disabled=busy||animationPending;for(const button of document.querySelectorAll('[data-action]'))button.disabled=promptsBlockPlay()||busy||animationPending||replayLoading||asking||!!replay||!inputStageActive()||!availableActions().includes(button.dataset.action);for(const id of ['tutorialStartButton','tutorialRetryButton','tutorialSkipButton'])if($(id))$(id).disabled=busy||animationPending||(id==='tutorialStartButton'&&view?.tutorial?.playing&&!view?.tutorial?.completed);if($('tutorialPauseButton'))$('tutorialPauseButton').disabled=!view?.tutorial?.playing||view.tutorial.completed||tutorialPauseRequested;if($('hypotheticalLane'))$('hypotheticalLane').disabled=busy||animationPending||replayLoading||asking||!view?.can_ask;if($('nextButton'))$('nextButton').disabled=ratingBlocksPlay()||busy||animationPending;if($('askButton'))$('askButton').disabled=ratingBlocksPlay()||busy||animationPending||replayLoading||asking||!view?.can_ask||(questionGuide()?$('questionInput')?.value.trim()!==questionGuide().question:(!!pendingAutomatic()||(release?.share_preview&&!release.share_qa_available)));for(const button of document.querySelectorAll('[data-example]'))button.disabled=busy||animationPending||replayLoading||asking||!view?.can_ask||(release?.share_preview&&!release.share_qa_available);}
...
document.addEventListener('keydown',e=>{
 if([' ','Enter'].includes(e.key)&&document.activeElement?.closest('button,a,summary'))return;
 if(promptsBlockPlay()){if(e.repeat&&[' ','Enter'].includes(e.key))e.preventDefault();return;}
