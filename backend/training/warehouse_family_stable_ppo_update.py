...
VERSION = "warehouse-family-stable-ppo-update.v1"
MAX_LAMBDA = .2
...
def stable_ppo_update(native, batch, program=None, *, lambda_value=0,
    ...
    names = tuple(native.envs[0].feature_names)
    ...
    ordinary_targets[selected] = prediction.predict(program, obs[selected], names)
    aux_targets = None
    ...
    audit.update(path="mixed_feedback" if len(aux) else "ordinary_feedback",
        auxiliary_pool_pairs=len(aux), ordinary_tree_rows=int(selected.sum()),
    ...
    with native._rng_context():
        for epoch in range(native.cfg["epochs"]):
            ...
            for start in range(0, len(obs), native.cfg["minibatch"]):
                ...
                if count:
                    ordinary_kl = _kl(logits[mask], data["targets"][index][mask])
                    ordinary_loss = lam*(1.-fraction)*ordinary_kl
                    audit["ordinary_kl_row_visits"] += count
