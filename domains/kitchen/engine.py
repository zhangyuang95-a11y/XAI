...
DOMAIN = "kitchen"
VERSION = "kitchen-v6.5.0"
SCENARIO_VERSION = "kitchen-scenarios-v6.5.0"
MENU_VERSION = "kitchen-menu-v4-three-dishes"
WIDTH, HEIGHT = 9, 7
...
def _scenario(seed, task):
    ...
    # Three dishes permit only two recipe-count splits. Repeated splits use a
    # different permutation so Task 3 cannot repeat the Task 1 menu.
    ...
    menu_rng = random.Random(seed * 71 + egg_count * 10007)
    menu_rng.shuffle(legal_menus)
    recipes = legal_menus[((task - 1) // (count - 1)) % len(legal_menus)]
    arrivals = [0] * count
...
def rules(language="en"):
    pairs = [
        ...
        (f"All three menu dishes are visible from the start. Their quantities and sequence are fixed before play, with at most two consecutive dishes of the same recipe. Matching bound orders may be served in any order. Every on-time correct dish adds {SERVE_POINTS} points; every game step costs {STEP_COST}. The serving action itself therefore adds {SERVE_POINTS - STEP_COST} net points; waiting once then serving adds {SERVE_POINTS - 2 * STEP_COST} across the two turns. Trash disposal costs {INGREDIENT_DISCARD_COST} for a single ingredient/component or {DISH_DISCARD_COST} for a combined dish. Scores can be negative. Serving on the deadline turn is accepted; expiry alone never removes food. Questions and replay cost no turns or points.", f"三道菜单从开始就全部公开，数量和顺序在游戏前固定；同菜最多连续两道。可按任意顺序完成对应绑定订单。每正确按时上菜加 {SERVE_POINTS} 分，每推进一步扣 {STEP_COST} 分；上菜动作本身净增 {SERVE_POINTS - STEP_COST} 分，先等待一步再上菜共净增 {SERVE_POINTS - 2 * STEP_COST} 分。垃圾桶丢弃单份原料或配料扣 {INGREDIENT_DISCARD_COST} 分，合成菜品扣 {DISH_DISCARD_COST} 分。得分可为负；截止回合仍可上菜，过期本身不会移除食物。提问和回看不消耗步数或分数。"),
