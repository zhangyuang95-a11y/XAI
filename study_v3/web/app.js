...
Object.assign(EN,{tutorial:'Kitchen practice',tutorialStart:'Start practice',tutorialResume:'Resume practice',tutorialPause:'Pause practice',tutorialRetry:'Retry this section',tutorialSkip:'Skip tutorial and start Task 1',tutorialComplete:'Practice complete',tutorialPaused:'Practice is paused. Start when you are ready.',tutorialNote:'Practice only. These actions and scores do not count toward your tasks.',operationHelp:'Controls and public rules',practiceTurn:'Practice turn',tutorialGoal:'Your current goal'});
Object.assign(ZH,{tutorial:'厨房练习',tutorialStart:'开始练习',tutorialResume:'继续练习',tutorialPause:'暂停练习',tutorialRetry:'重练当前段',tutorialSkip:'跳过教程，进入 Task 1',tutorialComplete:'练习完成',tutorialPaused:'练习已暂停，准备好后开始。',tutorialNote:'独立练习。操作与分数均不计入正式任务。',operationHelp:'操作帮助与公共规则',practiceTurn:'练习回合',tutorialGoal:'当前目标'});
...
function languageUI(){if($('headerContext'))$('headerContext').textContent=view?.instance_id?title()+' · '+(view.stage.startsWith('task')?'Task '+view.stage.slice(-1):stageLabel(view.stage)):'HUMAN + AI';document.documentElement.lang=lang==='zh'?'zh-CN':'en';$('englishButton').classList.toggle('active',lang==='en');$('chineseButton').classList.toggle('active',lang==='zh');$('studyBadge').textContent=tr('pilot');$('footerText').textContent=tr('footer');}
...
function tutorialPage(){
 const t=view.tutorial,s=actionState(),total=t.total_segments||7,index=Math.min(total-1,Math.max(0,t.index)),message=localized(t.feedback||t.message),interaction=typeof s.interaction==='string'?s.interaction:localized({en:s.interaction?.label_en,zh:s.interaction?.label_zh});
 const completed=!!t.completed,playing=t.playing&&!tutorialPauseRequested;
 const hintIds=[['controls'],[],['prep'],['capacity'],['containers'],['freshness'],['menu','score']][index]||[];
 const hints=(view.public_help||[]).filter(rule=>hintIds.includes(rule.id));
 return `${heading()}<div class="workspace kitchen-tutorial"><div>${board(s,true)}</div><aside class="side-stack"><section class="panel tutorial-goal" aria-labelledby="tutorialGoalTitle"><span class="number-label">${tr('step')} ${index+1} / ${total}</span><div class="tutorial-segments" aria-hidden="true">${Array.from({length:total},(_,i)=>`<span class="${i<index||completed?'done':i===index?'current':''}"></span>`).join('')}</div><h2 id="tutorialGoalTitle">${completed?tr('tutorialComplete'):tr('tutorialGoal')}</h2><p id="tutorialGoal" class="tutorial-goal-text" aria-live="polite">${esc(localized(t.goal))}</p>${tutorialProgress(t)}${index===2?tutorialPreparation(s):''}${hints.length?`<div class="tutorial-context">${hints.map(rule=>`<p>${esc(localized(rule))}</p>`).join('')}</div>`:''}${message?`<p id="tutorialFeedback" class="tutorial-feedback" role="status">${esc(message)}</p>`:''}${!playing&&!completed?`<p class="tutorial-paused" role="status">${tr('tutorialPaused')}</p>`:''}<div class="button-row"><button id="tutorialStartButton" class="button" ${playing?'disabled':''}>${completed?tr('startTask'):(s.turn>0||index>0?tr('tutorialResume'):tr('tutorialStart'))}</button><button id="tutorialPauseButton" class="button secondary" ${!playing||completed?'disabled':''}>${tr('tutorialPause')}</button><button id="tutorialRetryButton" class="button secondary">${tr('tutorialRetry')}</button></div>${completed?'':`<button id="tutorialSkipButton" class="tutorial-skip">${tr('tutorialSkip')} →</button>`}</section><section class="panel"><h3>${tr('actions')}</h3><div class="action-grid">${availableActions().map(a=>`<button class="action-button" data-action="${esc(a)}" ${!playing||completed?'disabled':''}>${esc(actionLabel(a))}</button>`).join('')}</div><p class="control-keys">${controlsText()}</p><p class="interaction-hint">${s.interaction?.available?'E: ':''}${esc(interaction)}</p><p class="hint">${tr('controls')}</p></section>${index===total-1?kitchenMenu(s):''}<section class="panel"><details><summary>${tr('operationHelp')}</summary>${conciseRules(s)}</details></section></aside></div>`;
}
