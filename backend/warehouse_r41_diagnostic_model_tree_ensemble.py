...
VERSION = "warehouse-r41-diagnostic-model-tree-ensemble.v1"
ENVELOPE_VERSION = "warehouse-r41-diagnostic-model-tree-ensemble-envelope.v1"
...
class R41DiagnosticModelTreeEnsemble:
    """Mean-probability ensemble of four explicit, quantized model trees."""
    ...
    def from_unquantized_members(
        ...
        if len(members) != MEMBER_COUNT:
            raise ValueError("Exactly four unquantized members are required")
        first = members[0][1]
        compact = []
    ...
    def trace(self, features: Mapping[str, float]) -> tuple[ProgramTraceStep, ...]:
        # Concatenating all four paths makes every predicate that contributed to
...
__all__ = [
    "VERSION", "ENVELOPE_VERSION", "AGGREGATION", "QMAX", "MEMBER_COUNT",
