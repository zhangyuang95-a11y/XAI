...
from backend.training import warehouse_r41_diagnostic_outer_hash_projection_v13 as subject
...
def _validation_only_rows() -> tuple[list[dict], list[dict]]:
    ...
    scene = {"id": "diagnostic_v13_fresh_outer_0000",
...
def test_registry_validation_accepts_v13_identity_schema(
    ...
    fingerprint = digest({"fresh-v13-scene": 1})
    identity = {
        "batch_index": 0,
    ...
    scene = {
        "id": "diagnostic_v13_fresh_outer_0000",
        "seed": identity["seed"],
        "family_id": identity["family_id"],
        "fingerprint": fingerprint,
    }
    sources = {
        "backend/training/warehouse_r41_diagnostic_rcpd_v13_outer_split.py": (
            "b" * 64),
    }
    bindings = {name: "f" * 64 for name in subject._RECOVERY_BINDINGS}
    bindings.update({
        ...
        "consumed_v10_selected_identity_sha256": "1" * 64,
        "consumed_v10_outer_observation_hashes_sha256": "2" * 64,
        "consumed_v11_selected_identity_sha256": "3" * 64,
        "consumed_v11_outer_observation_hashes_sha256": "4" * 64,
        "consumed_v12_outer_selected_identity_sha256": "5" * 64,
        "consumed_v12_outer_observation_hashes_sha256": "6" * 64,
        "burned_v12_final_selected_identity_sha256": "7" * 64,
        "burned_v12_final_observation_hashes_sha256": "8" * 64,
        "contract_sha256": digest(subject.registry_api.contract()),
    })
    # The production contract fixes 64 identities; repeat the construction
    # with distinct seeds/fingerprints and the exact v13 scene-id prefix.
    identities = []
    scenes = []
    for index in range(subject.SCENE_COUNT):
        ...
        row["seed"] += index
        row["fingerprint"] = digest({"fresh-v13-scene": index})
    ...
    exclusion_counts = {
        ...
        "consumed_v10_outer_identities": 64,
        "consumed_v11_outer_identities": 64,
        "consumed_v12_outer_identities": 64,
        "burned_v12_final_identities": 64,
        "union_candidate_identities_excluded": 785,
    }
    exclusion_digests = {
        "consumed_v9_outer_identities_sha256": "e" * 64,
        "consumed_v10_outer_identities_sha256": "1" * 64,
        "consumed_v10_outer_observation_hashes_sha256": "2" * 64,
        "consumed_v11_outer_identities_sha256": "3" * 64,
        "consumed_v11_outer_observation_hashes_sha256": "4" * 64,
        "consumed_v12_outer_identities_sha256": "5" * 64,
        "consumed_v12_outer_observation_hashes_sha256": "6" * 64,
        "burned_v12_final_identities_sha256": "7" * 64,
        "burned_v12_final_observation_hashes_sha256": "8" * 64,
    }
    statistics = {
        **exclusion_counts,
        ...
        "selected_outer_family_counts": subject.registry_api.FAMILY_QUOTAS,
            "combined_promoted_unique_observation_count": 123,
            "selected_exposed_seed_overlap": 0,
            "selected_exposed_fingerprint_overlap": 0,
            "hash_screen": {
                "selected_scene_count": subject.SCENE_COUNT,
    ...
    registry = {
        ...
        "contract": subject.registry_api.contract(),
        "bindings": bindings,
...
def test_registry_hash_screen_must_match_independent_replay(
    monkeypatch: pytest.MonkeyPatch,
):
    rows, scenes = _validation_only_rows()
    arrays, _accounting = subject._projection_rows_to_arrays(rows)
    hashes = list(map(str, np.char.decode(
        arrays["observation_hashes"], "ascii")))
    screen = {
        "selected": [{
            "family_id": "conflict_family_01",
            "fingerprint": scenes[0]["fingerprint"],
            "seed": 48100000,
    ...
    screen["content_sha256"] = digest(screen)
    monkeypatch.setattr(subject, "SCENE_COUNT", 1)
    registry = {
        "development_outer": [{
            **scenes[0], "family_id": "conflict_family_01", "seed": 48100000,
        }],
        "statistics": {"hash_screen": screen},
    }
    subject._validate_registry_hash_screen_replay(arrays, registry=registry)
    changed = deepcopy(registry)
    changed["statistics"]["hash_screen"]["selected"][0][
        "observation_hashes_sha256"] = "f" * 64
    changed["statistics"]["hash_screen"]["content_sha256"] = digest({
        key: value for key, value in
        changed["statistics"]["hash_screen"].items()
        if key != "content_sha256"
    })
    with pytest.raises(ValueError, match="hash-screen replay"):
        subject._validate_registry_hash_screen_replay(arrays, registry=changed)


def test_source_closure_has_both_v13_identity_and_replay_producers_only():
    ...
    assert "release_receipt" not in source
...
def test_build_rechecks_sources_and_inputs_at_publish_boundary():
    ...
    assert source.index("_replay_outer") < source.index(
        "_validate_registry_hash_screen_replay")
