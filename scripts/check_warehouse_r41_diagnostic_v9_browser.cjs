...
/*
 * Real Playwright acceptance for the r4.1-diagnostic v9 participant UI.
...
const VERSION = 'warehouse-r41-diagnostic-v9-playwright-acceptance.v1';
...
function parseArgs(argv) {
  ...
  requireThat(fs.existsSync(result.database) && fs.statSync(result.database).isFile()
    && path.basename(result.database).endsWith('_diagnostic_v9_browser_qa.sqlite3'),
    'Use a separate *_diagnostic_v9_browser_qa.sqlite3 database');
    ...
    const request=https.get(endpoint,{ca,rejectUnauthorized:true,minVersion:'TLSv1.2'},response=>{
      ...
      response.on('end',()=>{
        try {
          ...
          'HTTPS health identity differs from diagnostic v9 expectations');
...
assert ctx.get('version')=='warehouse-r41-diagnostic-online-release.v9'
...
async function execute(args) {
  ...
  const command=async(t,kind,gesture)=>{await settled(t);const responsePromise=t.page.waitForResponse(response=>{
    ...
    t.lastRequestBody=response.request().postDataJSON();
  ...
  const ask=async(t,question,focus)=>{const replay=await t.page.locator('body').getAttribute('data-replay')==='true';const frame=Number(replay?await t.page.locator('#historySlider').inputValue():await t.page.locator('body').getAttribute('data-frame')),run=t.view.run_id,before=physical(await apiView(t));
    ...
    if(!t.oldQuestionPayload)t.oldQuestionPayload=JSON.parse(JSON.stringify(t.lastRequestBody));
  ...
  const forbiddenQuestion=async t=>{t.view=await apiView(t);const before=physical(t.view);
    const payload={operation_id:crypto.randomUUID(),expected_version:t.view.version,kind:'question',run_id:t.view.run_id,
      frame:t.view.state.frame,question:'Why did Robot 2 choose that action?',language:'en',focus:'executed'};
    const response=await t.context.request.post(args.base+COMMAND_PATH,{data:payload,headers:{Origin:args.base}});const body=await response.json();
    requireThat(response.status()===403 && (body.error==='explanation_forbidden'||body.error?.code==='explanation_forbidden'),
      `Task 2 accepted a new explanation request: HTTP ${response.status()} ${JSON.stringify(body)}`);
    if(t.oldQuestionPayload){const replayed=await t.context.request.post(args.base+COMMAND_PATH,{data:t.oldQuestionPayload,headers:{Origin:args.base}});
      const replayedBody=await replayed.json();requireThat(replayed.status()===200,'Task 2 rejected an idempotent old request without a safe projection');
      validateView(replayedBody);requireThat(replayedBody.answers.length===0&&!replayedBody.explain_allowed,'Task 2 replay exposed an old answer');}
    if(t.firstTask1Run){const history=await t.context.request.get(args.base+'/api/history?run_id='+encodeURIComponent(t.firstTask1Run));
      const historyBody=await history.json();requireThat(history.status()===403 && (historyBody.error==='history_current_run_only'||historyBody.error?.code==='history_current_run_only'),
        'Task 2 exposed history from an earlier run');}
    requireThat(physical(await apiView(t))===before,'Forbidden Task 2 explanation request changed physical state');};
  ...
  try {
    ...
    for(let index=0;index<MATRIX.length;index++) {
      ...
      const id=`qa_ui_v9_${token}_${index+1}`;requireThat(ID.test(id),'Generated participant ID is invalid');
    ...
    const primary=aParticipants[0];if(primary) {
      ...
      primary.oldQuestionPayload=JSON.parse(JSON.stringify(primary.lastRequestBody));
    ...
    for(const t of participants) {
      ...
      if(t.condition==='A') {
        ...
        requireThat(!(await t.page.locator('#explanationPanel').isVisible()),'B group sees explanation panel');
        await t.page.locator('#previousFrame').click();await t.page.waitForFunction(()=>document.body.dataset.replay==='true');
        requireThat(await t.page.locator('[data-action="WAIT"]').isDisabled(),'B replay left actions enabled');
        requireThat(!(await t.page.locator('#explanationPanel').isVisible()),'B replay exposed explanations');
        await t.page.locator('#liveButton').click();await t.page.waitForFunction(()=>document.body.dataset.replay==='false');
      }
      ...
      t.firstTask1Run=t.view.run_id;await nextRound(t);await endRound(t);await nextRound(t);await endRound(t);await nextRound(t);
    ...
    const report={version:VERSION,status:args.allowSyntheticFixture?'passed_synthetic_browser_harness_validation':'passed_r41_diagnostic_v9_browser_acceptance',
      ...
      authenticated_tls_probe:tls,frontend_timing:frontendTiming,motion_reports:motionReports,screenshots,counters,errors,checks:['authenticated_loopback_TLS','declared_380ms_and_observed_intermediate_frames','four_isolated_contexts','A_XY_A_YX_B_XY_B_YX','zh_en','tutorial_play_pause_restart_skip_refresh',
        'task1_live_historical_post_round_questions','B_replay_only','task2_old_and_new_explanations_hidden','380ms_intermediate_tutorial_and_task_motion',
