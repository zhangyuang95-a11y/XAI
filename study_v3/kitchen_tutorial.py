...
def public_help():
    ...
    return [
        ...
        {'id':'menu','en':f"{r['orders_per_task']} menu dishes; at most two identical dishes consecutively. Budget: {r['max_turns']} turns. Deadlines: {', '.join(map(str,r['order_deadlines']))}, counted from task start. Serving on the deadline is accepted; an unfinished order expires at that turn's end. Dishes remain bound to their orders; serving need not follow menu order.", 'zh':f"每轮 {r['orders_per_task']} 道菜，相同菜最多连续两道，上限 {r['max_turns']} 回合。截止回合为 {'、'.join(map(str,r['order_deadlines']))}，从本局开始累计。截止当回合仍可上菜，回合结束未完成才过期。菜与订单绑定，但不要求按菜单顺序上菜。"},
