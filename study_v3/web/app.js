...
function captureEntryDraft(){
 ...
 return {participant:$('participantInput').value,recovery:$('recoveryInput').value,group:$('groupInput').value,consent:$('consentInput').checked,key:$('adminInput')?.value};
}
function restoreEntryDraft(draft){
 if(!draft||!$('startForm'))return;
 $('participantInput').value=draft.participant;$('recoveryInput').value=draft.recovery;$('groupInput').value=draft.group;$('consentInput').checked=draft.consent;
...
async function setLanguage(next){stopHeld();if(busy||animationPending)return;draftQuestion=$('questionInput')?.value||draftQuestion;lang=next;localStorage.setItem('policylens.language',lang);languageUI();if(view?.instance_id)await command('language',{language:lang});else render();}
...
async function refresh(){
 if(!domain||document.hidden)return;if(busy||animationPending){pendingRefresh=true;return;}
 const epoch=chatEpoch,sequence=++refreshSequence;
 try{const next=await api('/api/study/view?domain='+encodeURIComponent(domain));if(epoch!==chatEpoch||sequence!==refreshSequence||document.hidden)return;
 participantId=next.participant_id||participantId;
 if(next.instance_id){if(view?.instance_id===next.instance_id&&next.revision<view.revision)return;if(view?.can_ask&&!next.can_ask)clearChat();view=next;lang=view.language;languageUI();await render();}
...
async function command(kind,extra={}){
 if(busy||!view?.instance_id)return;if(kind!=='action')stopHeld();busy=true;updateControlLocks();const epoch=chatEpoch;
 ...
 if(next.stage!==old.stage){replay=null;draftQuestion='';}if(next.state?.terminal)stopHeld();
 view=next;channel?.postMessage({id:view.instance_id,revision:view.revision});await render({animate:kind==='action'});
 }catch(e){stopHeld();report(e);if(e.code==='stale_state')pendingRefresh=true;}
 finally{busy=false;updateControlLocks();if(pendingRefresh){pendingRefresh=false;await refresh();}scheduleHeld();}
}
...
function heading(){return `${domainNavigation()}${progress()}<div class="study-title"><div><div class="eyebrow">${title()}</div><h1>${view.stage.startsWith('task')?'Task '+view.stage.slice(-1):tr(view.stage)}</h1><p>${view.stage==='task1'?tr('baseline'):view.stage==='task2'?(view.can_ask?tr('task2Intro'):tr('control')):view.stage==='task3'?tr('transfer'):''}</p></div><div class="phase-tag"><i class="dot"></i>${view.mode==='pilot'?tr('pilot'):tr('preview')}${view.group?' · '+view.group:''}</div></div>`;}
...
function board(s,isDemo=false){
 ...
 return `<section class="panel board-panel"><div class="board-toolbar"><span>${isDemo?tr('demoNote'):tr('publicInfo')}</span><div class="legend"><span><i class="swatch"></i>${tr('you')}</span><span><i class="swatch ai"></i>${tr('ai')}</span></div></div>
 <div class="scorebar ${isDemo?'demo-scorebar':''}">${isDemo?'':`<div class="metric"><span>${tr('score')}</span><b>${Number(score.task_score||0).toFixed(0)}</b>${scoreSuffix(score)}${scoreFeedback(s)}</div>`}<div class="metric"><span>${tr('turn')}</span><b>${s.turn}</b><small> / ${s.max_turns}</small></div><div class="metric"><span>${tr('remaining')}</span><b>${Math.max(0,s.max_turns-s.turn)}</b></div></div>
...
function kitchenMenu(s){if(!s?.orders)return '';return `<section class="panel kitchen-menu"><h3>${lang==='zh'?'本轮菜单 · 5 道菜':'Menu · 5 dishes'}</h3><ol>${s.orders.map((order,i)=>`<li class="menu-${esc(order.status)}"><span>${i+1}. ${esc(tr(order.recipe))}</span><b>${order.status==='completed'?'✓':order.status==='expired'?(lang==='zh'?'已截止':'Closed'):(lang==='zh'?'待上菜':'To serve')}</b></li>`).join('')}</ol><p class="hint">${lang==='zh'?'每道上菜 +100 · 每步 −1 · 丢原料 −3 · 丢成品 −10':'Serve +100 · Each move −1 · Trash ingredient −3 · Trash dish −10'}</p></section>`;}

function domainNavigation(){return `<nav class="domain-navigation" aria-label="${lang==='zh'?'切换领域':'Switch domain'}">${Object.entries(meta).map(([key,m])=>`<a href="/${key}/${preview?'?preview=1':''}" ${key===domain?'aria-current="page"':''}>${esc(lang==='zh'?m.zh:m.name)}</a>`).join('')}</nav>`;}
function conciseRules(s){
 const rules={warehouse:{en:['Pick up a package at A and deliver it to B. Keep an eye on your battery.','WASD: move. Space: wait.'],zh:['从 A 点拿货送到 B 点，同时关注自己的电量。','WASD 移动，空格等待。']},pong:{en:['Choose which balls to catch together: small balls score 1; large balls need both players and score 3. Some arrive together, so you cannot catch everything.','A / D: move left / right. Space: wait.'],zh:['一起取舍接球：小球 1 分；大球需两人配合，得 3 分。有些球同时落下，无法全部接住。','A / D 左右移动，空格等待。']},kitchen:{en:['Prepare and hand over ingredients; collect, plate and serve all 5 dishes. Tomato/pepper: 3 prep turns; egg: 4; meat: 5.','WASD: move and turn. E: use the station in front. Space: wait.'],zh:['备料后交给 AI，取回成品并装盘，完成菜单上的 5 道菜。番茄／辣椒备料 3 回合，鸡蛋 4 回合，肉 5 回合。','WASD 移动和转向，E 操作面前工位，空格等待。']}};
 let html=`<ul class="rules-list compact-rules">${rules[domain][lang].map(rule=>`<li>${esc(rule)}</li>`).join('')}</ul>`;
 if(domain==='kitchen'&&s?.orders){const counts={};for(const order of s.orders)if(order.status==='pending')counts[order.recipe]=(counts[order.recipe]||0)+1;const dishes=Object.entries(counts).map(([recipe,count])=>`${esc(tr(recipe))} × ${count}`).join(' · ');if(dishes)html+=`<p class="recipe-goal">${lang==='zh'?'待制作':'To serve'}: ${dishes}</p>`;}
 return html;
}
...
function completedPage(){return `${heading()}<div class="task-intro"><div class="eyebrow">${tr('completed')}</div><h1>${tr('thanks')}</h1><p>${tr('saved')}</p></div><div class="results-grid">${view.task_runs.map(r=>`<div class="panel"><div class="eyebrow">Task ${r.task}</div><b>${Number(r.score.task_score).toFixed(0)}</b>${scoreSuffix(r.score)}</div>`).join('')}</div><div class="task-intro"><a class="button secondary" href="/">${tr('homeButton')} →</a></div>`;}
...
async function render({animate=false,displayState=null}={}){
 ...
 languageUI();let html;
 if(!meta[domain])html=home();else if(!view?.instance_id)html=welcome();else if(view.stage==='demo')html=demoPage();else if(view.stage==='questionnaire')html=surveyPage();else if(view.stage==='completed')html=completedPage();else html=taskPage();
 ...
 const state=displayState||replay?.state||(view?.stage==='demo'?view.demo.frames[demoFrame()]:view.state);
 if(state){animationPending=animate;updateControlLocks();await boardRenderer.setState(state,{animate,language:lang});animationPending=false;updateControlLocks();}
}
function bind(){if($('startForm'))$('startForm').onsubmit=async e=>{e.preventDefault();if(busy||(!preview&&!release?.study_ready))return;busy=true;updateEntryStatus();adminKey=$('adminInput')?.value||'';try{view=await api('/api/study/session',{domain,participant_id:$('participantInput').value,recovery_code:$('recoveryInput').value,language:lang,consent:$('consentInput').checked,mode:preview?'preview':'pilot',group:$('groupInput').value},adminKey?{Authorization:'Bearer '+adminKey}:{});participantId=view.participant_id;adminKey='';render();}catch(err){if(err.code==='study_not_ready'){release={...release,study_ready:false};entryFailed=false;entryCheckedAt=-Infinity;}report(err);}finally{busy=false;updateEntryStatus();scheduleEntryCheck();}};
...
function canMove(){return !busy&&!animationPending&&!replayLoading&&!asking&&!replay&&!document.hidden&&!editing()&&view?.stage?.startsWith('task')&&view?.state&&!view.state.terminal;}
function updateControlLocks(){updateDemoControls();for(const id of ['englishButton','chineseButton'])if($(id))$(id).disabled=busy||animationPending;for(const button of document.querySelectorAll('[data-action]'))button.disabled=busy||animationPending||replayLoading||asking||!!replay||!view?.actions?.includes(button.dataset.action);if($('hypotheticalLane'))$('hypotheticalLane').disabled=busy||animationPending||replayLoading||asking||!view?.can_ask;if($('askButton'))$('askButton').disabled=busy||animationPending||replayLoading||asking||!view?.can_ask;for(const button of document.querySelectorAll('[data-example]'))button.disabled=busy||animationPending||replayLoading||asking||!view?.can_ask;}
function submitAction(action){if(!canMove())return;if(!view.actions.includes(action)){if(domain==='kitchen'&&action==='interact')toast(lang==='zh'?view.state.interaction?.label_zh:view.state.interaction?.label_en);return;}selectedAction=action;return command('action',{run_id:view.run_id,turn:view.state.turn,action});}
function scheduleHeld(){if(heldTimer||!heldDirection||!canMove())return;if(!view.actions.includes(heldDirection)){stopHeld();return;}heldTimer=setTimeout(()=>{heldTimer=null;if(heldDirection&&canMove())submitAction(heldDirection);},0);}
...
document.addEventListener('keydown',e=>{
 if(e.ctrlKey||e.metaKey||e.altKey||editing()||!view?.stage?.startsWith('task')||view.state?.terminal||replay||asking||document.hidden)return;
...
(async()=>{languageUI();try{release=await loadEntryRelease();entryFailed=false;}catch{entryFailed=true;}try{if(domain)await refresh();render();}catch(e){$('app').textContent=tr('retry');report(e);}})();
