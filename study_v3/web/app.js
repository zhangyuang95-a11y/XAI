...
function promptsBlockPlay(){return ratingBlocksPlay()||(!view?.optional_prompts&&!!pendingAutomatic());}
function ratingBlocksPlay(){return !!pendingUnderstanding();}
function optionalPromptNote(){return view?.optional_prompts&&!pendingUnderstanding()?`<p class="hint">${lang==='zh'?'可选择回答，也可以直接继续操作。未回答会记为跳过。':'You may respond or keep playing. Unanswered prompts are recorded as skipped.'}</p><button type="button" class="button secondary full" data-skip-prompts>${lang==='zh'?'暂不回答，继续':'Continue without answering'}</button>`:'';}
