...
def _selector_report(lock: dict) -> dict:
    ...
    validation_wins = {
        ...
        "promoted_v11": source_mask("promoted", 62),
        "prior_unique_observations": 40,
        "historical_unique_observations": 30,
        "promoted_v11_unique_observations": 10,
        "fresh_v12_unique_observations": 32,
    ...
    validation_hash_inputs = {
        ...
        "promoted_v11_closeout_sha256": lock["bindings"][
            "promoted_v11_closeout_sha256"],
        "promoted_v11_rows_sha256": lock["bindings"][
            "promoted_v11_rows_sha256"],
        "promoted_v11_rows_semantic_sha256": digest({"rows": "semantic"}),
        "promoted_v11_unique_observations": 10,
        "promoted_v11_observation_hashes_sha256": digest(["promoted"]),
...
def test_collection_contract_and_reader_api_are_unscored():
    ...
    assert subject.SCHEMA_VERSION == (
        "warehouse_r41_diagnostic_rcpd_v12_outer_collection_v1")
...
def test_candidate_lock_is_validated_before_any_outer_replay(tmp_path, monkeypatch):
    ...
    kwargs = dict(
        ...
        expected_projection_receipt_sha256="4" * 64,
            ...
            promoted_v11_closeout_path="promoted-closeout",
            expected_promoted_v11_closeout_sha256="6" * 64,
            permanent_v11_outer_registry="permanent-v11",
            promoted_v11_rows_path="promoted-rows",
        program_path="program", selector_report_path="selector", output=tmp_path / "out")
    with pytest.raises(ValueError, match="locked v12 candidate"):
...
def test_candidate_bindings_include_registry_report_and_projection_receipt(
    ...
    names = (
        ...
        "projection_receipt",
        "development_rows", "promoted_v11_rows", "program", "selector_report",
    )
    paths = {}
    for name in names:
        path = tmp_path / name
...
def test_source_closure_binds_projection_registry_and_has_no_protected_reader():
    ...
    assert "backend/training/warehouse_r41_diagnostic_outer_collection_v12.py" in sources
    assert "backend/training/warehouse_r41_diagnostic_outer_hash_projection_v12.py" in sources
    assert "backend/training/warehouse_r41_diagnostic_rcpd_v12_outer_split.py" in sources
