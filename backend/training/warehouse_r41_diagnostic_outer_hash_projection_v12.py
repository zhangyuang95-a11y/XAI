"""Publish the label-blind ordered observation-hash projection for v12 outer.

The fresh outer scene identities must already be frozen by the v12 registry.
...
VERSION = "warehouse-r41-diagnostic-outer-hash-projection.v12"
...
def _resolve_snapshot(
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    registry = _regular(registry_path, "fresh v12 outer registry")
    registry_report = _regular(registry_report_path, "fresh v12 outer registry report")
    ...
    expected = {
        ...
        "registry": _sha(expected_registry_sha256, "fresh v12 registry"),
        "registry_report": _sha(
            expected_registry_report_sha256, "fresh v12 registry report"),
    }
    ...
    snapshot = ImmutableInputSnapshot(
        ...
        prefix="warehouse-r41-v12-outer-projection-inputs-",
    )
...
def _validate_registry(
    *, paths: Mapping[str, Path], component_originals: Mapping[str, Path],
    ...
    registry = _strict_json(paths["registry"], "fresh v12 outer registry")
    report = _strict_json(paths["registry_report"], "fresh v12 outer registry report")
    ...
    if (registry.get("version") != registry_api.VERSION
        ...
        raise ValueError("Exact identity-frozen fresh v12 outer registry required")
    ...
    for index, (scene, identity) in enumerate(zip(scenes, identities)):
        if not isinstance(scene, Mapping) or not isinstance(identity, Mapping):
            raise ValueError("Fresh v12 outer scene identity differs")
        ...
        if (any(type(public[key]) is not int for key in ("batch_index", "seed"))
            ...
            raise ValueError("Fresh v12 outer materialised identity differs")
    ...
    recovery_statistics_valid = (
        ...
        and exclusion_counts.get("consumed_v11_outer_identities")
            == registry_api.FRESH_OUTER_SCENE_COUNT
        and exclusion_counts.get("union_candidate_identities_excluded") == 657
        ...
        and bindings.get("consumed_v11_outer_rows_sha256")
            == registry_api.consumed_v11_api.EXPECTED_COLLECTION_ROWS_SHA256
        and bindings.get("consumed_v11_outer_rows_semantic_sha256")
            == registry_api.consumed_v11_api.EXPECTED_COLLECTION_ROWS_SEMANTIC_SHA256
    ...
    if (not isinstance(bindings, Mapping)
        ...
        raise ValueError("Fresh v12 outer registry/report binding differs")
...
def _validate_registry_hash_screen_replay(
    ...
    """Reproduce the per-scene commitments used by the v12 hash screen."""
    statistics = registry.get("statistics")
    ...
    if (not isinstance(screen, Mapping) or not _content_valid(screen)
            ...
            or not isinstance(scenes, list) or len(scenes) != SCENE_COUNT):
        raise ValueError("Fresh v12 registry hash-screen audit differs")
    ...
    if len(observation_hashes) != len(scene_fingerprints):
        raise ValueError("Fresh v12 registry hash-screen replay differs")
    ...
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    try:
        ...
        arrays, _accounting, environment_steps = _replay_outer(
            ...
            progress_label="fresh_v12_outer_hash_projection")
        ...
        temporary = Path(tempfile.mkdtemp(
            prefix=".warehouse-r41-v12-outer-projection-", dir=destination.parent))
...
def read_saved_projection(
    *, projection_path: str | Path, receipt_path: str | Path,
    ...
    projection_file = _regular(projection_path, "v12 outer hash projection")
    receipt_file = _regular(receipt_path, "v12 outer projection receipt")
    if (file_hash(projection_file) != _sha(
            expected_projection_sha256, "v12 outer hash projection")
            or file_hash(receipt_file) != _sha(
                expected_receipt_sha256, "v12 outer projection receipt")):
        raise ValueError("Exact v12 outer projection artifact pair required")
    projection = validate_projection(_strict_json(
        projection_file, "v12 outer hash projection"))
    receipt = _strict_json(receipt_file, "v12 outer projection receipt")
