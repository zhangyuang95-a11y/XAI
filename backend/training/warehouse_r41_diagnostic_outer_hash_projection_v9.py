"""Publish the label-blind ordered observation-hash projection for v9 outer.

The fresh outer scene identities must already be frozen by the v9 registry.
...
VERSION = "warehouse-r41-diagnostic-outer-hash-projection.v9"
...
def _resolve_snapshot(
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    registry = _regular(registry_path, "fresh v9 outer registry")
    registry_report = _regular(registry_report_path, "fresh v9 outer registry report")
    ...
    expected = {
        ...
        "registry": _sha(expected_registry_sha256, "fresh v9 registry"),
        "registry_report": _sha(
            expected_registry_report_sha256, "fresh v9 registry report"),
    }
    ...
    snapshot = ImmutableInputSnapshot(
        ...
        prefix="warehouse-r41-v9-outer-projection-inputs-",
    )
    ...
    *, paths: Mapping[str, Path], component_originals: Mapping[str, Path],
    ...
    for index, (scene, identity) in enumerate(zip(scenes, identities)):
        if not isinstance(scene, Mapping) or not isinstance(identity, Mapping):
            raise ValueError("Fresh v9 outer scene identity differs")
        ...
        if (any(type(public[key]) is not int for key in ("batch_index", "seed"))
            ...
            raise ValueError("Fresh v9 outer materialised identity differs")
    ...
    if (not isinstance(bindings, Mapping)
        ...
        raise ValueError("Fresh v9 outer registry/report binding differs")
    ...
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    try:
        ...
        arrays, _accounting, environment_steps = _replay_outer(
            ...
            progress_label="fresh_v9_outer_hash_projection")
        ...
        temporary = Path(tempfile.mkdtemp(
            prefix=".warehouse-r41-v9-outer-projection-", dir=destination.parent))
...
def read_saved_projection(
    *, projection_path: str | Path, receipt_path: str | Path,
    ...
    projection_file = _regular(projection_path, "v9 outer hash projection")
    receipt_file = _regular(receipt_path, "v9 outer projection receipt")
    if (file_hash(projection_file) != _sha(
            expected_projection_sha256, "v9 outer hash projection")
            or file_hash(receipt_file) != _sha(
                expected_receipt_sha256, "v9 outer projection receipt")):
        raise ValueError("Exact v9 outer projection artifact pair required")
    ...
    if (receipt.get("version") != RECEIPT_VERSION
        ...
        raise ValueError("V9 outer projection receipt differs")
