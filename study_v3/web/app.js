...
function task2Introduction(){if(release?.share_preview)return lang==='zh'?'体验版：到第一个解释节点时，在右侧提问区完成一次引导提问，之后可选择是否查看解释。你是蓝色角色，AI 是橙色角色。':'Demo: play until the first explanation point, then try asking in the question panel. Later explanations are optional. You are blue; the AI is orange.';if(['guided-question-choice-v3','first-node-question-guide-v4'].includes(view.automatic_explanation_protocol))return lang==='zh'?'Task 2 的第一个解释节点会引导你在右侧提问。后续关键节点可选择直接继续或查看解释；你也可以在提问框追问。提问和阅读不消耗游戏步数。':'At the first explanation point in Task 2, try asking in the question panel. At later decision points, choose to continue or see an explanation. You can also ask follow-up questions. Asking and reading do not use game turns.';return view.automatic_explanations_enabled?(lang==='zh'?'Task 2 中，AI 会在预设节点自动提供解释。你也可以主动追问；提问不消耗游戏回合。':'In Task 2, your AI teammate explains at preset points. You can also ask follow-up questions without using game turns.'):tr('task2Intro');}
...
function pongExplanationDock(){
 return domain==='pong'&&pendingAutomatic()&&!questionGuide()?'<div id="pongExplanationDock" class="pong-explanation-dock" aria-hidden="true"></div>':'';
}
function automaticDialog(){
 ...
 const buttons=choosing?`${card.onboarding?'':`<button id="understoodExplanationButton" class="button secondary full">${zh?'我理解，继续':'I understand — continue'}</button>`}<button id="requestExplanationButton" class="button full">${card.onboarding?esc(card.question):(zh?'查看解释':'Show me why')}</button>`:`<button id="confirmExplanationButton" class="button full">${zh?'确认并继续':'Confirm and continue'}</button>`;
 return `<dialog id="automaticDialog" class="automatic-dialog" data-domain="${esc(domain)}" aria-labelledby="automaticTitle" aria-describedby="automaticReason" data-explanation-id="${esc(card.id)}">${domain==='pong'?'<svg class="automatic-connector" aria-hidden="true"><path/><circle r="4"/></svg>':''}<div class="automatic-speaker"><span class="automatic-avatar" aria-hidden="true">2</span><div><h2 id="automaticTitle">${title}</h2><div class="hint">Task 2 · ${tr('turn')} ${card.turn}</div></div></div>${card.guided&&card.requested?`<p class="guided-question">${zh?'你的问题：':'Your question: '}${esc(card.question)}</p>`:''}<div id="automaticReason" class="automatic-body">${esc(choosing?prompt:card.body)}</div>${choosing?'':`<p class="hint">${zh?'游戏已暂停。请阅读回答，然后确认并继续。':'The game is paused. Read the answer, then confirm to continue.'}</p>`}<div class="automatic-choices">${buttons}</div></dialog>`;
}
function automaticAnchor(){
 const canvas=$('studyCanvas');if(!canvas)return null;const rect=canvas.getBoundingClientRect();
 return {x:rect.left+Number(canvas.dataset.aiAnchorX||.5)*rect.width,y:rect.top+Number(canvas.dataset.aiAnchorY||.5)*rect.height};
}
function positionAutomaticDialog(){
 ...
 const w=dialog.offsetWidth,h=dialog.offsetHeight,margin=16;
 const left=Math.max(margin,Math.min(innerWidth-w-margin,anchor.x-w/2));
 let top=anchor.y-h-36,below=false;
 if(top<margin&&anchor.y+36+h<innerHeight-margin){top=anchor.y+36;below=true;}
 top=Math.max(margin,Math.min(innerHeight-h-margin,top));
 dialog.style.left=left+'px';dialog.style.top=top+'px';
 dialog.style.setProperty('--tail-x',Math.max(24,Math.min(w-32,anchor.x-left))+'px');
 dialog.classList.toggle('below-ai',below);
}
let focusedQuestionGuide=null;
function syncAutomaticDialog(){
 const guide=questionGuide();if(guide&&!document.hidden){stopHeld();if(focusedQuestionGuide!==guide.id){focusedQuestionGuide=guide.id;$('chatPanel')?.scrollIntoView({block:'center',behavior:'instant'});$('guidedQuestionExample')?.focus({preventScroll:true});}updateControlLocks();return;}
 const dialog=$('automaticDialog');if(!dialog||!pendingAutomatic()||document.hidden)return;
 stopHeld();
 if(!dialog.open){
  const anchor=automaticAnchor();
  if(domain!=='pong'&&anchor&&(anchor.y>innerHeight-70||anchor.y<130))window.scrollBy({top:anchor.y-innerHeight*.72,behavior:'instant'});
  dialog.showModal();
  if(domain==='pong'){
  ...
  ( $('requestExplanationButton')||$('confirmExplanationButton'))?.focus({preventScroll:true});
 }
 positionAutomaticDialog();updateControlLocks();
}
window.addEventListener('resize',positionAutomaticDialog);
window.addEventListener('scroll',positionAutomaticDialog,{passive:true});
...
function questionPanel(){
 ...
 const zh=lang==='zh',g=questionGuide(),available=!release?.share_preview||release.share_qa_available;
 ...
 const intro=g?`<div class="question-guide" role="status"><h4>${zh?'试着向 AI 提问':'Try asking your AI teammate'}</h4><p>${g.requested?(zh?'回答已显示。阅读后点击确认，继续游戏。':'Read the answer below, then confirm to continue.'):(zh?'到了第一个解释节点。请先点击下面的示例问题，再点击“发送问题”。之后你可以在这里继续提问。提问和阅读不消耗步数。':'You have reached the first explanation point. Select the example below, then click “Send question”. You can use this panel for follow-up questions. Asking and reading do not use game turns.')}</p></div>`:'';
