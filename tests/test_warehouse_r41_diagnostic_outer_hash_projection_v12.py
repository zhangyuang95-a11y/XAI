...
from backend.training import warehouse_r41_diagnostic_outer_hash_projection_v12 as subject
...
def _validation_only_rows() -> tuple[list[dict], list[dict]]:
    ...
    scene = {"id": "diagnostic_v12_fresh_outer_0000",
...
def test_registry_validation_accepts_v12_identity_schema(
    ...
    fingerprint = digest({"fresh-v12-scene": 1})
    identity = {
        "batch_index": 0,
    ...
    scene = {
        "id": "diagnostic_v12_fresh_outer_0000",
        "seed": identity["seed"],
    ...
    bindings.update({
        ...
        "consumed_v11_outer_rows_sha256": (
            subject.registry_api.consumed_v11_api.EXPECTED_COLLECTION_ROWS_SHA256),
        "consumed_v11_outer_rows_semantic_sha256": (
            subject.registry_api.consumed_v11_api.EXPECTED_COLLECTION_ROWS_SEMANTIC_SHA256),
    })
    # The production contract fixes 64 identities; repeat the construction
    # with distinct seeds/fingerprints and the exact v12 scene-id prefix.
    identities = []
    scenes = []
    for index in range(subject.SCENE_COUNT):
        ...
        row["seed"] += index
        row["fingerprint"] = digest({"fresh-v12-scene": index})
        materialized = dict(scene)
        materialized.update({
            "id": f"diagnostic_v12_fresh_outer_{index:04d}",
            "seed": row["seed"],
    ...
    exclusion_counts = {
        ...
        "union_candidate_identities_excluded": 657,
    }
    exclusion_digests = {
...
def test_source_closure_has_both_v12_identity_and_replay_producers_only():
    sources = subject.producer_sources()
    assert "backend/training/warehouse_r41_diagnostic_outer_hash_projection_v12.py" in sources
    assert "backend/training/warehouse_r41_diagnostic_rcpd_v11_outer_split.py" in sources
