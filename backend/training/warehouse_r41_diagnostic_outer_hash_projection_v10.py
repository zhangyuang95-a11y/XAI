"""Publish the label-blind ordered observation-hash projection for v10 outer.

The fresh outer scene identities must already be frozen by the v10 registry.
...
VERSION = "warehouse-r41-diagnostic-outer-hash-projection.v10"
...
def _resolve_snapshot(
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    registry = _regular(registry_path, "fresh v10 outer registry")
    registry_report = _regular(registry_report_path, "fresh v10 outer registry report")
    ...
    expected = {
        ...
        "registry": _sha(expected_registry_sha256, "fresh v10 registry"),
        "registry_report": _sha(
            expected_registry_report_sha256, "fresh v10 registry report"),
    }
    ...
    snapshot = ImmutableInputSnapshot(
        ...
        prefix="warehouse-r41-v10-outer-projection-inputs-",
    )
...
def _validate_registry(
    *, paths: Mapping[str, Path], component_originals: Mapping[str, Path],
    ...
    registry = _strict_json(paths["registry"], "fresh v10 outer registry")
    report = _strict_json(paths["registry_report"], "fresh v10 outer registry report")
    ...
    if (registry.get("version") != registry_api.VERSION
        ...
        raise ValueError("Exact identity-frozen fresh v10 outer registry required")
    ...
    for index, (scene, identity) in enumerate(zip(scenes, identities)):
        if not isinstance(scene, Mapping) or not isinstance(identity, Mapping):
            raise ValueError("Fresh v10 outer scene identity differs")
        ...
        if (any(type(public[key]) is not int for key in ("batch_index", "seed"))
            ...
            raise ValueError("Fresh v10 outer materialised identity differs")
    ...
    recovery_statistics_valid = (
        ...
        and exclusion_counts.get("union_candidate_identities_excluded") == 529
    ...
    if (not isinstance(bindings, Mapping)
        ...
        raise ValueError("Fresh v10 outer registry/report binding differs")
    ...
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    try:
        ...
        arrays, _accounting, environment_steps = _replay_outer(
            ...
            progress_label="fresh_v10_outer_hash_projection")
        ...
        temporary = Path(tempfile.mkdtemp(
            prefix=".warehouse-r41-v10-outer-projection-", dir=destination.parent))
...
def read_saved_projection(
    *, projection_path: str | Path, receipt_path: str | Path,
    ...
    projection_file = _regular(projection_path, "v10 outer hash projection")
    receipt_file = _regular(receipt_path, "v10 outer projection receipt")
    if (file_hash(projection_file) != _sha(
            expected_projection_sha256, "v10 outer hash projection")
            or file_hash(receipt_file) != _sha(
                expected_receipt_sha256, "v10 outer projection receipt")):
        raise ValueError("Exact v10 outer projection artifact pair required")
    projection = validate_projection(_strict_json(
        projection_file, "v10 outer hash projection"))
    receipt = _strict_json(receipt_file, "v10 outer projection receipt")
    ...
    if (receipt.get("version") != RECEIPT_VERSION
        ...
        raise ValueError("V10 outer projection receipt differs")
