#!/usr/bin/env python3
"""Fail-closed local Render preflight for r4.1 diagnostic v9.
...
from backend.training import warehouse_r41_diagnostic_release_receipt_v9 as receipt_api
...
VERSION = "warehouse-r41-diagnostic-render-preflight.v9"
STATUS = "ready_for_manual_render_diagnostic_v9_deploy"
...
def check_render_yaml(path: str | Path, *, package_sha256: str,
    ...
    if any(env.get(name) != child for name, child in required.items()):
        raise ValueError("render.yaml has stale or missing v9 release variables")
...
def preflight(*, admission_path: str | Path,
    ...
    if _HEX.fullmatch(str(expected_receipt_sha256)) is None:
        raise ValueError("Exact external v9 receipt SHA-256 required")
    ...
    if file_hash(receipt_file) != expected_receipt_sha256:
        raise ValueError("V9 receipt bytes differ")
    inputs = dict(
    ...
    if (manifest_sha != receipt["manifest_sha256"]
        ...
        raise ValueError("V9 release identity differs at preflight")
    ...
    if check_clean_checkout:
        ...
        if (clean.get("version") != release.VERSION
            ...
            raise ValueError("Clean-checkout v9 release load differs")
