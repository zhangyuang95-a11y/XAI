"""Select and lock one v9 public-tree candidate before fresh-outer labels exist.

The selector treats the failed v8 outer as development data only after its
permanent closeout is authenticated.  The fresh v9 outer contributes a
label-blind ordered observation-hash projection.  Those hashes win over the
development archive before any action or probability member is opened.
...
VERSION = "warehouse-r41-diagnostic-rcpd-v9-fit-selector.v1"
...
def contract() -> dict[str, Any]:
    return {
        ...
        "fresh_outer_input": "ordered label-blind observation SHA-256 projection only",
...
def evaluate_candidate(
    ...
    if tuple(salts) != CV_SALTS:
        raise ValueError("Exactly the two frozen v9 CV salts are required")
    ...
    value: dict[str, Any] = {
        ...
        "selection": {
            "selected_config_sha256": _sha(
                selected_config_sha256, "selected v9 fit configuration"),
...
def _resolve_snapshot(
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    expected = {
        ...
        "candidate_grid": _sha(expected_candidate_grid_sha256, "v9 candidate grid"),
    }
    ...
    snapshot = ImmutableInputSnapshot(
        ...
        prefix="warehouse-r41-v9-fit-selector-inputs-",
    )
    ...
    *, actor_path: str | Path, protocol_path: str | Path,
    ...
    try:
        ...
        if (designation.get("bindings", {}).get("actor_sha256") != actor.artifact_sha256
            ...
            raise ValueError("V9 selector evidence bindings differ")
        ...
        if relations.base_feature_names != actor_feature_names:
            raise ValueError("V9 relation base features differ from the frozen Actor")
        ...
        if (program.base_feature_names != actor_feature_names
            ...
            raise RuntimeError("Locked v9 program relation/Actor binding differs")
        ...
        if (not np.array_equal(
            ...
            raise RuntimeError("Locked v9 program round-trip action parity differs")
        if final_gate.get("passed") is not True:
            raise RuntimeError("Full-development v9 refit failed the nine hard gates")

        temporary = Path(tempfile.mkdtemp(
            prefix=".warehouse-r41-v9-fit-selector-", dir=destination.parent))
...
__all__ = [
    "VERSION", "STATUS", "LOCK_SCHEMA", "GRID_VERSION", "CV_SALTS",
