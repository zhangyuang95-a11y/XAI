...
def _feature_family(name: str) -> str:
    """Adapter-supplied namespace, with no domain interpretation."""
    return name.split(".", 1)[0] if "." in name else "features"
...
def _program_feature_allowed(name: str) -> bool:
    # Feature availability is an explicit adapter responsibility.
    return bool(name)


def _is_relational_predicate(name: str) -> bool:
    # Adapters declare relational features through this documented namespace.
