...
from backend.training import warehouse_r41_diagnostic_outer_hash_projection_v11 as subject
...
def _validation_only_rows() -> tuple[list[dict], list[dict]]:
    ...
    scene = {"id": "diagnostic_v11_fresh_outer_0000",
...
def test_registry_validation_accepts_v11_identity_schema(
    ...
    fingerprint = digest({"fresh-v11-scene": 1})
    identity = {
        "batch_index": 0,
    ...
    scene = {
        "id": "diagnostic_v11_fresh_outer_0000",
        "seed": identity["seed"],
    ...
    # with distinct seeds/fingerprints and the exact v11 scene-id prefix.
    identities = []
    scenes = []
    for index in range(subject.SCENE_COUNT):
        ...
        row["seed"] += index
        row["fingerprint"] = digest({"fresh-v11-scene": index})
    ...
    exclusion_counts = {
        ...
        "union_candidate_identities_excluded": 593,
    }
    exclusion_digests = {
...
def test_source_closure_has_both_v11_identity_and_replay_producers_only():
    sources = subject.producer_sources()
    assert "backend/training/warehouse_r41_diagnostic_outer_hash_projection_v11.py" in sources
