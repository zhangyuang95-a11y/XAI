"""Irrevocable v14 final-test controller for the warehouse explanation program.
...
VERSION = "warehouse-r41-diagnostic-final-once.v14"
...
MIN_MATERIALIZER_TIMEOUT_SECONDS = 1_800
MAX_MATERIALIZER_TIMEOUT_SECONDS = 14_400
TIMING_SAFETY_MULTIPLIER = 3.0
OFFICIAL_FINAL_MATERIALIZER_RELATIVE_PATH = (
    "backend/training/warehouse_r41_diagnostic_final_materializer_v14.py"
)
...
def contract() -> dict[str, Any]:
    return {
        ...
        "v13_timeout_closeout_authenticated_before_claim": True,
        "fresh_v14_candidate_universe_authenticated_before_claim": True,
        "public_timing_calibration_authenticated_before_claim": True,
        "materializer_timeout_at_least_three_x_public_upper_bound": True,
...
def _materializer_timeout_seconds(calibration: Mapping[str, Any]) -> int:
    """Validate a public, development-only timing bound before final claim."""
    timing = calibration.get("timing")
    if not isinstance(timing, Mapping):
        raise ValueError("Public v14 timing calibration is missing")
    upper = timing.get("measured_upper_bound_seconds")
    timeout = timing.get("required_timeout_seconds")
    if (type(upper) not in (int, float) or isinstance(upper, bool)
            or not np.isfinite(float(upper))
            or float(upper) <= LEGACY_TIMEOUT_SECONDS
            or type(timeout) is not int or isinstance(timeout, bool)):
        raise ValueError("Public v14 timing calibration differs")
    minimum = max(
        MIN_MATERIALIZER_TIMEOUT_SECONDS,
        int(np.ceil(TIMING_SAFETY_MULTIPLIER * float(upper))),
    )
    if (timeout < minimum or timeout > MAX_MATERIALIZER_TIMEOUT_SECONDS
            or timeout <= LEGACY_TIMEOUT_SECONDS):
        raise ValueError("V14 materializer timeout is not safely calibrated")
    return timeout
...
def _run_materializer(
    ...
    *, timeout_seconds: int,
    ...
    if (type(timeout_seconds) is not int or isinstance(timeout_seconds, bool)
            or timeout_seconds < MIN_MATERIALIZER_TIMEOUT_SECONDS
            or timeout_seconds > MAX_MATERIALIZER_TIMEOUT_SECONDS):
        raise ValueError("V14 materializer timeout differs")
    subprocess.run(
...
def _authenticate_preclaim(
    *, candidate_lock_path: str | Path, expected_candidate_lock_sha256: str,
    ...
    expected_candidate_universe_sha256: str,
    timing_calibration_path: str | Path,
    expected_timing_calibration_sha256: str,
) -> dict[str, Any]:
    ...
    timeout_path = outer_api._authenticate_file(
    ...
    universe_path = outer_api._authenticate_file(
        candidate_universe_path, "v14 candidate universe",
        expected_sha256=_sha(
            expected_candidate_universe_sha256, "v14 candidate universe"))
    candidate_universe = universe_api.read_saved_universe(
        universe_path,
        expected_universe_sha256=expected_candidate_universe_sha256,
        timeout_closeout_path=timeout_path,
        expected_timeout_closeout_sha256=expected_timeout_closeout_sha256,
        permanent_timeout_closeout_registry=permanent_timeout)
    timing_path = outer_api._authenticate_file(
        timing_calibration_path, "v14 timing calibration",
        expected_sha256=_sha(
            expected_timing_calibration_sha256, "v14 timing calibration"))
    timing_calibration = timing_api.read_saved_calibration(
        timing_path,
        expected_calibration_sha256=expected_timing_calibration_sha256,
        timeout_closeout_path=timeout_path,
        expected_timeout_closeout_sha256=expected_timeout_closeout_sha256,
        permanent_timeout_closeout_registry=permanent_timeout)
    timeout_bindings = timeout_closeout.get("bindings")
    universe_bindings = candidate_universe.get("bindings")
    timing_bindings = timing_calibration.get("bindings")
    if (not isinstance(timeout_bindings, Mapping)
            or not isinstance(universe_bindings, Mapping)
            or not isinstance(timing_bindings, Mapping)
            or timeout_bindings.get("candidate_lock_sha256")
                != file_hash(lock_path)
            or timeout_bindings.get("actor_sha256") != bindings["actor_sha256"]
            or timeout_bindings.get("protocol_sha256")
                != bindings["protocol_sha256"]
            or timeout_bindings.get("runtime_manifest_sha256")
                != bindings["runtime_manifest_sha256"]
            or timeout_bindings.get("program_sha256")
                != bindings["program_sha256"]
            or timeout_bindings.get("outer_result_sha256")
                != expected_outer_result_sha256
            or universe_bindings.get("timeout_closeout_file_sha256")
                != expected_timeout_closeout_sha256
            or universe_bindings.get("timeout_closeout_content_sha256")
            ...
            or universe_bindings.get("runtime_manifest_sha256")
                != bindings["runtime_manifest_sha256"]
            or timing_bindings.get("timeout_closeout_file_sha256")
                != expected_timeout_closeout_sha256
            or timing_bindings.get("timeout_closeout_content_sha256")
                != timeout_closeout.get("content_sha256")):
        raise ValueError("V14 timeout closeout, universe, or timing binding differs")
    materializer_timeout_seconds = _materializer_timeout_seconds(
        timing_calibration)
    ...
    return {
        ...
        "timing_calibration_path": timing_path,
...
def _validate_material(
    ...
    for index, scene in enumerate(scenes):
        if (not isinstance(scene, Mapping)
                ...
                or scene.get("id")
                    != f"diagnostic_v14_fresh_final_{index:04d}"
...
def run_final_once(
    *, candidate_lock_path: str | Path, expected_candidate_lock_sha256: str,
    ...
    """Run one v14 final attempt; neither salt nor final identity is an input."""
    ...
    preclaim = _authenticate_preclaim(
        ...
        timing_calibration_path=timing_calibration_path,
        expected_timing_calibration_sha256=expected_timing_calibration_sha256)
    ...
    attempt_inputs = {
        ...
        "private_salt_commitment": PRIVATE_SALT_COMMITMENT,
        "private_salt_domain_sha256": sha256(PRIVATE_SALT_DOMAIN).hexdigest(),
        "timeout_closeout_sha256": file_hash(
            preclaim["timeout_closeout_path"]),
        "timeout_closeout_content_sha256": preclaim[
            "timeout_closeout"]["content_sha256"],
        "candidate_universe_sha256": file_hash(
            preclaim["candidate_universe_path"]),
        "candidate_universe_content_sha256": preclaim[
            "candidate_universe"]["content_sha256"],
        "candidate_universe_identity_sha256": preclaim[
            "candidate_universe"]["ordered_identity_sha256"],
        "timing_calibration_sha256": file_hash(
            preclaim["timing_calibration_path"]),
        "timing_calibration_content_sha256": preclaim[
    ...
    try:
        material = dict(_run_materializer(
            ...
            timeout_seconds=preclaim["materializer_timeout_seconds"]))
...
def read_completion(
    ...
    if (not result_path.is_file() or result_path.is_symlink()
            ...
            or file_hash(result_path) != _sha(
                expected_completion_sha256, "v14 final completion")):
        raise ValueError("Exact v14 final completion bytes required")
    value = outer_api._strict_json_bytes(
        result_path.read_bytes(), "v14 final completion")
    if (not isinstance(value, Mapping) or not _content_valid(value)
        ...
        raise ValueError("V14 final completion semantics differ")
    if value["status"] == STATUS_PASSED:
        ...
        if (set(value) != _SUCCESS_COMPLETION_FIELDS
            ...
            raise ValueError("Successful v14 final completion differs")
        ...
        raise ValueError("Failed v14 final completion differs")
    permanent = _directory(permanent_final_registry, "permanent final registry")
    campaign = _directory(
    ...
    if (not anchor_path.is_file() or anchor_path.is_symlink()
        ...
        raise ValueError("Permanent v14 final claim/completion differs")
