...
from backend.training import warehouse_r41_diagnostic_rcpd_v13_fit_selector as subject
...
def test_v13_selector_uses_replacement_registry_without_prior_targets():
    contract = subject.contract()
    assert subject.VERSION == "warehouse-r41-diagnostic-rcpd-v13-fit-selector.v1"
    assert subject.registry_api.VERSION == \
        "warehouse-r41-diagnostic-rcpd-v13-fresh-outer-registry.v1"
    assert subject.projection_api.VERSION == \
        "warehouse-r41-diagnostic-outer-hash-projection.v13"
    # V13 reuses the single frozen development-only v12 configuration.
    ...
    assert subject.OFFICIAL_FOLD_ASSIGNMENT_SHA256S == {
    ...
    assert contract["fresh_v13_outer_labels_or_probabilities_read"] is False
    assert "v13 registry" in contract["consumed_v9_outer_input"]
...
def test_v13_selector_rejects_a_replacement_candidate_grid_before_file_access():
    wrong = "0" * 64
    with pytest.raises(ValueError, match="preregistered v12 candidate grid"):
        subject._resolve_snapshot(
            ...
            promoted_bundle_closeout_path="missing",
            expected_promoted_bundle_closeout_sha256=wrong,
            permanent_promotion_closeout_registry="missing",
            ...
            development_rows_path="missing", combined_promoted_rows_path="missing",
            candidate_grid_path="missing",
            expected_candidate_grid_sha256=wrong,
        )


def test_v13_validation_wins_mask_precedes_private_development_read(
...
def test_v13_promotes_closed_rows_with_public_precedence_before_target_read(
    tmp_path: Path, monkeypatch,
):
    base = _arrays(6)
    promoted = _arrays(4)
    promoted["observations"][:, 0] += 20
    # One public observation is shared across sources.  The promoted bundle wins.
    promoted["observations"][0] = base["observations"][0]
    promoted["observation_hashes"] = np.asarray([
        subject.rows_v7.legacy._obs_hash(row)
        for row in promoted["observations"]
    ], dtype="S64")
    promoted["scene_fingerprints"] = np.asarray([
        _fingerprint(f"promoted-scene-{index}") for index in range(4)
    ], dtype="S64")
    promoted["split_validation"] = np.ones(4, dtype=np.bool_)
    base_path, promoted_path = tmp_path / "base.npz", tmp_path / "promoted.npz"
    _write_rows(base_path, base)
    _write_rows(promoted_path, promoted)

    promoted_hashes = sorted(set(map(
        str, np.char.decode(promoted["observation_hashes"], "ascii"))))
    historical = base["observation_hashes"][3].decode()
    prior = sorted(set(promoted_hashes) | {historical})
    fresh = sorted({base["observation_hashes"][2].decode(),
                    promoted["observation_hashes"][1].decode()})
    events: list[str] = []
    real_freeze = subject.freeze_validation_wins
    ...
    def full(path: Path):
        assert events[:2] == ["mask", "mask"]
        assert all(event == "load" for event in events[2:])
        events.append("load")
        source = base if path == base_path else promoted
        return {name: value.copy() for name, value in source.items()}

    monkeypatch.setattr(subject, "freeze_validation_wins", freeze)
    monkeypatch.setattr(subject, "_load_full_development_rows", full)
    monkeypatch.setattr(subject.rows_api, "_validate_base_shapes", lambda *args: None)
    combined, audit = subject.prepare_combined_development_rows(
        base_path, expected_base_sha256=file_hash(base_path),
        combined_promoted_path=promoted_path,
        expected_combined_promoted_sha256=file_hash(promoted_path),
        prior_outer_observation_hashes=prior,
        combined_promoted_observation_hashes=promoted_hashes,
        fresh_outer_observation_hashes=fresh, actor=object())

    assert events == ["mask", "mask", "load", "load"]
    kept = list(map(str, np.char.decode(
        combined["observation_hashes"], "ascii")))
    assert promoted["observation_hashes"][0].decode() in kept
    assert base["observation_hashes"][2].decode() not in kept
    assert promoted["observation_hashes"][1].decode() not in kept
    assert historical not in kept
    assert kept.count(promoted["observation_hashes"][0].decode()) == 1
    assert not combined["split_validation"].any()
    assert audit["precedence"] == (
        "fresh-v13-validation > combined-promoted-development > older-development")
    assert audit["cross_component_observation_overlap"] == 0
    assert audit["retained_fresh_outer_observation_overlap"] == 0
    assert audit["private_members_read_only_after_both_masks_frozen"] is True
...
def _prior_projection() -> dict:
    ...
    value = {
        ...
        "source_v11_closeout_content_sha256": _fingerprint("v11-closeout"),
        "source_v11_projection_content_sha256": _fingerprint("v11-projection"),
        "source_v12_final_closeout_content_sha256": _fingerprint("v12-closeout"),
        "source_v12_outer_projection_content_sha256": _fingerprint("v12-outer"),
        "source_v12_final_projection_content_sha256": _fingerprint("v12-final"),
        ...
        "component_unique_counts": {
            ...
            "consumed_v11": 1, "consumed_v12_outer": 1,
            "burned_v12_final": 1},
        "component_hashes_sha256": {
            ...
            "consumed_v11": _fingerprint("v11-hashes"),
            "consumed_v12_outer": _fingerprint("v12-outer-hashes"),
            "burned_v12_final": _fingerprint("v12-final-hashes"),
        },
        "selector_rule": (
...
def test_v13_prior_outer_projection_is_strict_and_label_free(tmp_path: Path):
...
def test_v13_validation_union_includes_prior_and_fresh_hashes():
...
def test_v13_validation_union_removes_consumed_observations():
...
def _config() -> dict:
    return {
        ...
        "shared_charger_endpoint_specialist": {
            "version": subject.fit_api.CHARGER_SPECIALIST_VERSION,
            "enabled": True,
            "group": "shared_charger",
            "combination": "weighted_average",
            "mix_weight": 1.0,
            "route": {
                "feature_name": subject.fit_api.CHARGER_ROUTE_FEATURE,
                "operator": ">", "threshold": 0.5,
            },
            "fit_source": (
...
def test_v13_support_repair_is_deterministic_and_supported():
...
def test_v13_failed_public_support_is_ineligible_without_calling_fit(monkeypatch):
    arrays = _arrays(6)
    families = {
...
def test_v13_fit_support_disagreement_fails_closed(monkeypatch):
    arrays = _arrays(6)
    families = {
        value.decode(): "family-a"
        for value in arrays["scene_fingerprints"]
...
def test_v13_grid_continues_after_an_ineligible_candidate(monkeypatch):
    calls: list[str] = []
...
def test_v13_all_ineligible_has_deterministic_complete_summary_digest():
    report = {
