"""Fail-closed admission for the warehouse r4.1 diagnostic v10 release.
...
VERSION = "warehouse-r41-diagnostic-admission.v10"
STATUS = "admitted_internal_diagnostic_v10"
ARTIFACT_NAMES = (
...
def source_closure() -> dict[str, str]:
    """Bind the complete transitive v10 admission implementation."""
...
def _validate_components_snapshot(
    ...
    if (completion.get("status") != final_api.STATUS_PASSED
        ...
        raise ValueError("Passing immutable v13 protected-final completion required")
    ...
    if (current_hashes != initial_hashes or source_closure() != sources
        ...
        raise RuntimeError("V10 admission input or source changed during validation")
...
def validate_components(
    ...
    with snapshot_api.ImmutableInputSnapshot(
            ...
            prefix="warehouse-r41-diagnostic-v10-admission-") as frozen:
...
def read_saved_admission(
    ...
    for name in ("artifacts", "bindings", "play_scenes", "sources"):
        if value.get(name) != checked[name]:
            raise ValueError("V10 admission " + name + " differs from evidence")
    _reject_sensitive(value, "saved v10 admission")
    return deepcopy(value)
