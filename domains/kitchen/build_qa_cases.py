...
def occupied_output_trace(frames):
    # A reproducible participant mistake, executed from the original trajectory:
    # at turn 42 both pans await vegetables and the human has an empty hand.
    # Bringing an unnecessary raw egg blocks an upcoming completed dish.
    state=deepcopy(frames[42]); result=[state]
    assert state['human']['holding'] is None
    assert all(p['phase']=='await_vegetable' for p in state['pots'])
    for target in ('egg','handoff'):
...
def build():
    ...
    specs.extend([
        ...
        ('full_menu','start','How many dishes are in the menu, and can I see the full list now?','这次菜单一共几道菜，现在能看到全部吗？',['public_rule8','order1','order2','order3','order4','order5'],[{'path':'total_orders','equals':5}],'shared','rule'),
