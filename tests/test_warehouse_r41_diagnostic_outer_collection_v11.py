...
from backend.training import warehouse_r41_diagnostic_outer_collection_v11 as subject
...
def test_collection_contract_and_reader_api_are_unscored():
    ...
    assert subject.SCHEMA_VERSION == (
        "warehouse_r41_diagnostic_rcpd_v11_outer_collection_v1")
...
def test_candidate_lock_is_validated_before_any_outer_replay(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError, match="locked v11 candidate"):
...
def test_source_closure_binds_projection_registry_and_has_no_protected_reader():
    ...
    assert "backend/training/warehouse_r41_diagnostic_outer_collection_v11.py" in sources
