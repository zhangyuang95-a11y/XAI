"""Durable complete current-Actor branch collection through its genuine alignment runtime.
...
VERSION = 'warehouse-family-current-alignment-teacher-collection.v1'
...
def _context(actor, protocol, actor_sha, protocol_sha, scenarios):
    ...
    runtime=runtime_api.AlignmentRuntime(paths['actor'],protocol=declared,expected_actor_sha256=actor_sha,
...
def read_data(output):
    ...
    if (plan.get('version')!=VERSION or plan.get('producer')!=VERSION
        ...
        raise ValueError('Only the genuine current alignment-runtime collection is supported')
    for relative,expected in plan['source_files'].items():
        if file_hash(ROOT/relative)!=expected:raise ValueError('Alignment collection source changed')
