...
function automaticDialog(){
 ...
 return `<dialog id="automaticDialog" class="automatic-dialog" data-domain="${esc(domain)}" aria-labelledby="automaticTitle" aria-describedby="automaticReason" data-explanation-id="${esc(card.id)}">${domain==='pong'?'<svg class="automatic-connector" aria-hidden="true"><path/><circle r="4"/></svg>':''}<div class="automatic-speaker"><span class="automatic-avatar" aria-hidden="true">2</span><div><h2 id="automaticTitle">${lang==='zh'?'AI 队友的解释':'Your AI teammate explains'}</h2><div class="hint">Task 2 · ${tr('turn')} ${card.turn}</div></div></div><div id="automaticReason" class="automatic-body">${esc(card.body)}</div><p class="hint">${lang==='zh'?'游戏已暂停。请阅读原因，然后确认并继续。':'The game is paused. Read the explanation, then confirm to continue.'}</p><button id="confirmExplanationButton" class="button full">${lang==='zh'?'确认并继续':'Confirm and continue'}</button></dialog>`;
}
...
function syncAutomaticDialog(){
 ...
 if(!dialog.open){
  ...
  $('confirmExplanationButton')?.focus({preventScroll:true});
 }
...
function observeAutomatic(){
 ...
 if(!card||!element||document.hidden)return;
...
function bind(){if(prolificEntry&&!view?.instance_id)window.ProlificEntry.bind(async next=>{view=next;domain=next.domain;participantId=next.participant_id;await render();},render);if($('startForm'))$('startForm').onsubmit=async e=>{e.preventDefault();if(busy||(!preview&&!release?.study_ready))return;busy=true;updateEntryStatus();adminKey=$('adminInput')?.value||'';try{view=await api('/api/study/session',{domain,participant_id:$('participantInput').value,language:lang,consent:$('consentInput').checked,mode:preview?'preview':'pilot',group:$('groupInput').value},adminKey?{Authorization:'Bearer '+adminKey}:{});participantId=view.participant_id;adminKey='';render();}catch(err){if(err.code==='study_not_ready'){release={...release,study_ready:false};entryFailed=false;entryCheckedAt=-Infinity;}report(err);}finally{busy=false;updateEntryStatus();scheduleEntryCheck();}};
 ...
 if($('confirmExplanationButton'))$('confirmExplanationButton').onclick=()=>{const card=pendingAutomatic();if(card&&!busy&&!animationPending)command('confirm_explanation',{explanation_id:card.id});};
