...
R42_RELEASE_MODULE = "ui.warehouse_alignment_r42_release"
...
def _infer_quick_intent(question):
    ...
    if any(token in text for token in ("哪个任务", "哪一个任务", "朝哪", "目标", "方向",
                                       "which task", "what task", "target", "heading")):
        return "task_direction"
    if any(token in text for token in ("等待", "没动", "不动", "wait", "stood still", "stationary")):
        return "wait_reason"
    if ("为什么" in text or "为何" in text or "why" in text):
...
def _participant_answer(row):
    ...
    return {"id": row["id"], "status": status, "frame": row["frame"],
        ...
        "intent_id": row["intent_id"] if "intent_id" in keys else None,
        "question_sequence": (row["question_sequence"]
                              if "question_sequence" in keys else None)}
...
'''
        ...
        <p class="small" data-i18n="roundPreviewText">本局从全新的第 0 帧开始，包含两个并行配送任务，最多 120 步。</p>
        <button id="beginRoundButton" type="button" class="primary full" data-i18n="beginRound">开始本局</button>
      </section>
...
'''
        ...
        js = js.replace(
            'tutorialText:"先观看两个 AI 完成一次固定协作配送，了解取货、交付、充电和碰撞规则。演示不计入正式任务成绩。"',
            'tutorialText:"观看两个 AI 完成完整的 120 步教学演示，了解取货、交付、充电、碰撞和恢复。演示不计入正式任务成绩。"',
            1,
        )
        js = js.replace(
            'tutorialText:"Watch two AI robots complete one fixed collaborative delivery to learn pickup, delivery, charging, and collision rules. The demo does not count toward task scores."',
            'tutorialText:"Watch two AI robots complete a full 120-turn teaching demonstration of pickup, delivery, charging, collision and recovery. The demo does not count toward task scores."',
            1,
        )
        js = js.replace(
            "frame:shown()?.state?.frame ?? 0});}",
            "frame:shown()?.state?.frame ?? 0,intent_id:intentId});}",
        )
        js = js.replace(
            'document.querySelectorAll("[data-question]").forEach(b=>b.addEventListener("click",()=>{$("questionFocus").value=b.dataset.focus;ask(tr(b.dataset.question),b.dataset.focus);}));',
            'const QUICK_INTENTS={whyAction:"action_reason",whyWait:"wait_reason",whyCollision:"collision_reason",humanInfluence:"human_influence",currentGoal:"task_direction",chargingNeed:"charging_need"};document.querySelectorAll("[data-question]").forEach(b=>b.addEventListener("click",()=>{$("questionFocus").value=b.dataset.focus;ask(tr(b.dataset.question),b.dataset.focus,QUICK_INTENTS[b.dataset.question]);}));',
        )
        js = js.replace(
            '  Object.assign(WORDS.en,{historyLoading:',
            '  Object.assign(WORDS.zh,{roundPreview:"本局预览",roundPreviewText:"本局从全新的第 0 帧开始，包含两个并行配送任务，最多 120 步。",beginRound:"开始本局",tutorialText:"观看两个 AI 完成完整的 120 步教学演示，了解取货、交付、充电、碰撞和恢复。演示不计入正式任务成绩。"});\n'
            '  Object.assign(WORDS.en,{roundPreview:"Round preview",roundPreviewText:"This round starts from a fresh frame 0 with two parallel delivery tasks and a 120-turn limit.",beginRound:"Start round",tutorialText:"Watch two AI robots complete a full 120-turn teaching demonstration of pickup, delivery, charging, collision and recovery. The demo does not count toward task scores."});\n'
            '  Object.assign(WORDS.en,{historyLoading:',
            1,
        )
        js = js.replace(
            '    $("operationPanel").classList.toggle("hidden",!v?.run_id || ["consent","instructions","questionnaire","completed"].includes(p));',
            '    const roundReady=v?.round_ready===true;$("roundPreviewPanel").classList.toggle("hidden",!roundReady);$("roundPreviewTitle").textContent=roundReady?`${p==="task1"?"Task 1":"Task 2"} · ${tr("round")} ${v?.flow?.round_index || 1}`:"—";disable($("beginRoundButton"),locked || !allowed(v,"begin_round"));\n'
            '    $("operationPanel").classList.toggle("hidden",!v?.run_id || roundReady || ["consent","instructions","questionnaire","completed"].includes(p));',
            1,
        )
        js = js.replace(
    ...
    data["styles.css"] += (b"\n#freeplayTab,#freeplaySetup,#savedRunsField,#feedbackPanel,.technical{display:none!important}"
                            ...
                            b" .round-preview{padding:14px 0}.round-preview h3{margin:7px 0 5px}"
                            b"\n")
    return data
...
CREATE TABLE IF NOT EXISTS runs(id TEXT PRIMARY KEY,session_id TEXT NOT NULL,scenario_id TEXT NOT NULL,signature TEXT NOT NULL,
 ...
 started INTEGER NOT NULL DEFAULT 0,created TEXT NOT NULL,end_reason TEXT,provenance TEXT NOT NULL);
...
CREATE TABLE IF NOT EXISTS questions(id TEXT PRIMARY KEY,session_id TEXT NOT NULL,run_id TEXT NOT NULL,frame INTEGER NOT NULL,
 ...
 focus TEXT NOT NULL DEFAULT 'executed',evidence_detail TEXT,intent_id TEXT,question_sequence INTEGER);
...
class OnlineAlignmentStudyStore:
    ...
    def __init__(self, context, *, database=DEFAULT_DATABASE, storage_mode="auto"):
        ...
        if (self.is_diagnostic or self.is_r42) and requested_storage_mode != "ephemeral":
    ...
    def _initialize_database(self):
        ...
        with closing(self.connect()) as db:
            ...
            if "intent_id" not in question_columns:
                db.execute("ALTER TABLE questions ADD COLUMN intent_id TEXT")
            ...
            if "started" not in run_columns:
                db.execute("ALTER TABLE runs ADD COLUMN started INTEGER NOT NULL DEFAULT 1")
            if not exists:
    ...
    def _can_explain(self, db, session):
        ...
        return (run["signature"] == self.signature and run["stage"] == "task1"
                and ("started" not in run.keys() or bool(run["started"])))
    ...
    def _validated_tutorial(self, context):
        ...
        binding_fields = ({"parent_scene_fingerprint", "parent_snapshot_sha256",
        ...
        if (not isinstance(payload, dict) or set(payload) != expected_fields
                ...
                or (not self.is_r42 and
                    payload["bindings"].get("scene_manifest_version")
                    != manifest_version)
        ...
        if (not isinstance(frames, list)
                or (len(frames) != self.horizon + 1 if self.is_r42
                    else not 2 <= len(frames) <= self.horizon + 1)):
        ...
        if self.is_r42:
            deliveries = frames[-1]["metrics"].get("individual_deliveries", [])
            if (len(deliveries) != 2 or min(deliveries) < 1
                    or frames[-1]["state"].get("frame") != self.horizon):
                raise ValueError("complete r4.2 two-robot tutorial required")
    ...
    def _start_run(self, db, sid, scene_index):
        ...
        self._assert_fresh_round(env, metrics)
        provenance = {**self.provenance, "scenario_sha256": _digest(scene),
    ...
    @staticmethod
    def _assert_fresh_round(env, metrics):
        state = _state(env)
        agents = list(_attr(state, "agents", ()))
        tasks = list(_attr(state, "tasks", ()))
        if (int(_attr(state, "frame", -1)) != 0
                or float(metrics.get("score", 1)) != 0
                or int(metrics.get("deliveries", 1)) != 0
                or int(metrics.get("collisions", 1)) != 0
                or int(metrics.get("shutdowns", 1)) != 0
                or len(agents) != 2 or len(tasks) != 2
                or any(float(_attr(agent, "battery", -1)) != 100.0
                       or _attr(agent, "carrying_task_id", None) is not None
                       or int(_attr(agent, "deliveries_completed", -1)) != 0
                       for agent in agents)):
            raise ValueError("formal round must start from a clean frame 0")
    ...
    def _view(self, db, sid):
        ...
        result = {"session_id": _digest({"service": self.signature, "session": sid}),
            ...
            "enrollment": {"mode": ("internal_pilot" if self.is_r42 else
                                      "internal_diagnostic" if self.is_diagnostic
        ...
        if session["stage"] == "instructions":
            ...
            result.update(ended=bool(run["ended"]), done=bool(run["ended"]), end_reason=end_reason,
                round_ready=self.is_r42 and not bool(run["started"]) and not bool(run["ended"]),
        ...
        if session["mode"] == "enrollment":
            ...
            if result["ended"]:
                kinds = ["next"]
            elif self.is_r42 and not bool(run["started"]):
                kinds = ["begin_round"]
    ...
    def command(self, sid, payload):
        ...
        with closing(self.connect()) as db:
            ...
            try:
                ...
                if kind == "start":
                ...
                elif kind == "begin_round":
                    self._begin_round(db, session)
                elif kind == "next":
                    ...
                    run = self._run(db, sid)
                    if self.is_r42 and not bool(run["started"]):
                        raise CommandError("round_not_started", 409)
                    if run["ended"]:
    ...
    def _begin_round(self, db, session):
        if (not self.is_r42 or session["stage"] not in ("task1", "task2")
                or not session["active_run"]):
            raise CommandError("begin_round_not_allowed_in_stage", 403)
        run = self._run(db, session["id"])
        if run["ended"] or run["started"]:
            raise CommandError("round_already_started", 409)
        env = self.runtime.from_snapshot(json.loads(run["snapshot"]))
        self._assert_fresh_round(env, json.loads(run["metrics"]))
        db.execute("UPDATE runs SET started=1 WHERE id=?", (run["id"],))
    ...
    def _next(self, db, session, payload):
        ...
        if stage in ("task1", "task2"):
            bank = (0 if stage == "task1" else 1) if self.is_r42 else \
                (0 if (stage == "task1") == (session["task_order"] == "XY") else 1)
    ...
    def _begin_task1(self, db, session):
        ...
        db.execute("UPDATE sessions SET stage='task1',round_index=0,active_run=NULL WHERE id=?",
            (session["id"],))
        bank = 0 if self.is_r42 else (0 if session["task_order"] == "XY" else 1)
    ...
    def _question(self, db, session, payload):
        ...
        db.execute("""INSERT INTO questions
            (id,session_id,run_id,frame,stage,question,language,status,answer,shown,created,focus,evidence_detail,intent_id,question_sequence)
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (uuid4().hex, session["id"], run["id"], frame, "task1", question.strip(), language,
             "pending", None, None, _utcnow(), focus, None, intent_id, sequence))
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            try:
                ...
                local_origin = (expected_origin.startswith("http://127.0.0.1")
                                or expected_origin.startswith("http://localhost"))
                if origin != expected_origin or not (origin.startswith("https://") or local_origin):
...
def main(argv=None):
    ...
    if (not args.public_origin.startswith("https://")
            and not args.public_origin.startswith("http://127.0.0.1")
            and not args.public_origin.startswith("http://localhost")):
        parser.error("public origin must use HTTPS (except loopback development)")
