"""Real held-out explanation acceptance for the r4.1 diagnostic runtime.

The audit evaluates robot_2 on the frozen 64-scene final-test split with
...
report when a numerical or language gate is missed; only the separate r4.1
...
from backend.warehouse_r41_online_explanation import (
    R41DiagnosticOnlineAlignmentExplainer as OnlineAlignmentExplainer,
)
from backend.warehouse_r41_diagnostic_online_runtime import (
    R41DiagnosticOnlineAlignmentRuntime as OnlineAlignmentRuntime,
)
from env.warehouse.navigation import ACTIONS
from env.warehouse_native.partners import partner_action


VERSION = "warehouse-r41-diagnostic-explanation-audit.v3"
SCENE_MANIFEST_VERSION = "warehouse-r41-diagnostic-conflict-scene-manifest.v3"
RCPD_VERSION = "warehouse-r41-diagnostic-rcpd.v3"
...
def contract() -> dict[str, Any]:
    return {
        ...
        "split": "final_test", "scenes": EXPLANATION_SCENES,
        ...
        "ordinary_fidelity_min": .90, "ordinary_nonwait_min": .90,
...
def _validated_episode_count(base: Sequence[Mapping[str, Any]],
    ...
    row file masquerade as the registered 64 x 3 matrix.  Episode identity
...
def _summarize(base: Sequence[Mapping[str, Any]], pairs: Sequence[Mapping[str, Any]],
    ...
    checks = {
        "complete_64_scene_three_partner_matrix": (
            episodes == EXPLANATION_SCENES * len(PARTNERS)
        ),
        "ordinary_fidelity": bool(base) and fidelity["overall"]["fidelity"] >= .90,
        "ordinary_nonwait_fidelity": bool(nonwait) and fidelity["nonwait"]["fidelity"] >= .90,
...
def audit(*, actor_path: str | Path, protocol_path: str | Path, program_path: str | Path,
          rcpd_report_path: str | Path, scenarios_path: str | Path,
    ...
    rcpd_report_path = _regular_input(rcpd_report_path, "Diagnostic RCPD report")
    ...
    content = deepcopy(scenarios)
    manifest_content_sha256 = content.pop("content_sha256", None)
    runtime = OnlineAlignmentRuntime(
        actor_path, training_protocol_path=protocol_path, manifest_path=scenarios_path,
        ...
        expected_manifest_file_sha256=file_hash(scenarios_path),
        expected_manifest_content_sha256=manifest_content_sha256,
        expected_manifest_semantic_sha256=digest(scenarios),
    )
    explainer = OnlineAlignmentExplainer(program_path,
        expected_program_sha256=file_hash(program_path), runtime=runtime)
    split = scenarios.get("splits", {}).get("final_test")
    rcpd_report = _read_json(rcpd_report_path)
    if (not isinstance(split, list) or len(split) != EXPLANATION_SCENES
            or scenarios.get("version") != SCENE_MANIFEST_VERSION
            or scenarios.get("frozen_actor") != {
            ...
            or rcpd_report.get("version") != RCPD_VERSION
            ...
            or rcpd_report.get("bindings", {}).get("actor_file_sha256")
                != file_hash(actor_path)
            or rcpd_report.get("bindings", {}).get("protocol_file_sha256")
                != file_hash(protocol_path)
            or rcpd_report.get("bindings", {}).get("protocol_content_sha256")
                != runtime.protocol_sha256
            or rcpd_report.get("bindings", {}).get("manifest_file_sha256")
                != file_hash(scenarios_path)
            or rcpd_report.get("bindings", {}).get("manifest_content_sha256")
                != manifest_content_sha256
            or rcpd_report.get("bindings", {}).get("manifest_semantic_sha256")
                != digest(scenarios)):
        raise ValueError("Exact frozen diagnostic final-test explanation split required")
    heldout = {scene.get("fingerprint") for scene in split}
    other = {scene.get("fingerprint") for name, rows in scenarios["splits"].items()
             if name != "final_test" for scene in rows}
    ...
    initial = {"actor": file_hash(actor_path), "protocol": file_hash(protocol_path),
               "program": file_hash(program_path), "rcpd_report": file_hash(rcpd_report_path),
    ...
    bindings = {"actor_sha256": runtime.actor_sha256,
        ...
        "rcpd_report_sha256": file_hash(rcpd_report_path),
        "manifest_file_sha256": file_hash(scenarios_path),
        "manifest_content_sha256": manifest_content_sha256,
        "manifest_semantic_sha256": digest(scenarios),
    ...
    if episodes != EXPLANATION_SCENES * len(PARTNERS):
    ...
    if (producer_sources() != initial["sources"] or file_hash(actor_path) != initial["actor"]
            ...
            or file_hash(rcpd_report_path) != initial["rcpd_report"]
    ...
    read_saved_report(output,
        ...
        expected_bindings=bindings, require_passed=False)
    return report
...
def read_saved_report(output: str | Path, *, expected_report_sha256: str,
    ...
    if (not isinstance(inputs, dict) or inputs.get("version") != VERSION
            ...
            or len(inputs["holdout_fingerprints"]) != EXPLANATION_SCENES
            or len(set(inputs["holdout_fingerprints"])) != EXPLANATION_SCENES
    ...
    expected_report = {
        ...
        "bindings": dict(expected_bindings), "statistics": statistics,
        "base_rows": len(ordinary), "branch_anchor_count": anchor_count,
        "episodes": EXPLANATION_SCENES * len(PARTNERS),
        "zero_nn_overrides": zero_overrides,
        "source_state_unchanged": all(
            row["decision_source_unchanged"] for row in ordinary),
        "counterfactual_isolated": all(
            row["source_unchanged"] for row in interventions)
            and all(row["source_unchanged"] for row in language),
        "program_never_controls_action": controller,
        "execution": {"accounting_complete": True, "pending_operation": None,
                      "counts": expected_execution},
        "evidence_artifacts": artifacts,
    }
    if digest(report) != digest(expected_report):
        raise ValueError("Explanation report differs from immutable rows")
    if require_passed and not statistics["passed"]:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--rcpd-report", type=Path, required=True)
    ...
    report = audit(actor_path=args.actor, protocol_path=args.protocol,
        ...
        scenarios_path=args.scenarios, output=args.output)
...
__all__ = ["VERSION", "SCENE_MANIFEST_VERSION", "RCPD_VERSION", "PARTNERS", "GROUPS", "QUESTIONS", "EXPLANATION_SCENES", "contract",
